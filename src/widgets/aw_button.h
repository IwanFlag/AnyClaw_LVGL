...
inline lv_obj_t* aw_btn_create(lv_obj_t* parent, const char* text,
    ...
    /* Create label first so it can be styled in switch cases */
    ...
    switch (style) {
        ...
        case BTN_OUTLINED:
            lv_obj_set_style_bg_opa(btn, LV_OPA_TRANSP, 0);
            lv_obj_set_style_border_color(btn, g_colors ? g_colors->accent : aw::color_primary(), 0);
            lv_obj_set_style_border_width(btn, 1, 0);
            lv_obj_set_style_text_color(lbl, g_colors ? g_colors->accent : aw::color_primary(), 0);
            break;
    }
