...
static void check_nodejs(SelfCheckItem& item) {
    ...
    if (run_cmd("node --version", output, sizeof(output)) && output[0]) {
        ...
        app_log("[SelfCheck] Node.js: %s", output);
    } else {
        item.ok = false;
        strncpy(item.message, "Node.js not found", sizeof(item.message) - 1);
        app_log("[SelfCheck] Node.js: NOT FOUND");
    }
}
...
static void check_npm(SelfCheckItem& item) {
    ...
    if (run_cmd("npm --version", output, sizeof(output)) && output[0]) {
        ...
        app_log("[SelfCheck] npm: %s", output);
    } else {
        item.ok = false;
        strncpy(item.message, "npm not found or error", sizeof(item.message) - 1);
        app_log("[SelfCheck] npm: NOT FOUND or ERROR");
    }
}
...
static void check_network(SelfCheckItem& item) {
    ...
    if (code > 0) {
        ...
        app_log("[SelfCheck] Network: OK (HTTP %d)", code);
    } else {
        /* Try alternative */
        ...
        if (code > 0) {
            ...
            app_log("[SelfCheck] Network: OK (via OpenRouter)");
        } else {
            item.ok = false;
            strncpy(item.message, "Network unreachable", sizeof(item.message) - 1);
            app_log("[SelfCheck] Network: UNREACHABLE");
        }
    }
}
...
static void check_config_dir(SelfCheckItem& item) {
    ...
    if (!appdata) {
        ...
        app_log("[SelfCheck] Config dir: APPDATA not set");
        return;
    }
    ...
    if (fs::exists(config_dir)) {
        ...
        if (f.is_open()) {
            ...
            fs::remove(test_file);
            item.ok = true;
            app_log("[SelfCheck] Config dir: writable (%s)", config_dir.c_str());
        } else {
            item.ok = false;
            app_log("[SelfCheck] Config dir: NOT writable (%s)", config_dir.c_str());
        }
    } else {
        /* Directory doesn't exist yet - try to create it */
        item.ok = false;
        app_log("[SelfCheck] Config dir: does not exist (%s)", config_dir.c_str());
    }
}
...
SelfCheckResult selfcheck_run() {
    ...
    app_log("[SelfCheck] Running startup self-check...");
    ...
    if (result.all_ok) {
        app_log("[SelfCheck] All checks PASSED");
    } else {
        app_log("[SelfCheck] Some checks FAILED - attempting fixes...");
    }
...
bool selfcheck_fix(SelfCheckResult& result) {
    ...
    if (!result.nodejs.ok) {
        app_log("[SelfCheck] Node.js missing - cannot auto-fix, user must install");
        /* Show message box to guide user */
        MessageBoxA(nullptr,
            "Node.js is not installed or not in PATH.\n\n"
            "Please install Node.js from https://nodejs.org/\n"
            "and restart AnyClaw.",
            "AnyClaw - Node.js Required",
    ...
    if (!result.npm.ok && result.nodejs.ok) {
        app_log("[SelfCheck] Attempting: npm cache clean --force");
        char output[256] = {0};
        if (run_cmd("npm cache clean --force", output, sizeof(output), 10000)) {
            /* Re-check */
            check_npm(result.npm);
            if (result.npm.ok) {
                app_log("[SelfCheck] npm fixed after cache clean");
            } else {
                app_log("[SelfCheck] npm still broken after cache clean");
                fixed = false;
            }
        } else {
            app_log("[SelfCheck] npm cache clean failed");
            fixed = false;
        }
    }
    ...
    if (!result.config_dir.ok) {
        ...
        if (appdata) {
            std::string config_dir = std::string(appdata) + "\\openclaw";
            app_log("[SelfCheck] Attempting to create config dir: %s", config_dir.c_str());
            try {
                if (fs::create_directories(config_dir)) {
                    /* Re-check */
                    check_config_dir(result.config_dir);
                    if (result.config_dir.ok) {
                        app_log("[SelfCheck] Config dir created successfully");
                    } else {
                        app_log("[SelfCheck] Config dir created but still not writable");
                        fixed = false;
                    }
                } else {
                    app_log("[SelfCheck] Failed to create config dir");
                    fixed = false;
                }
            } catch (const std::exception& e) {
                app_log("[SelfCheck] Exception creating config dir: %s", e.what());
                fixed = false;
            }
        }
    }
    ...
    if (!result.network.ok) {
        app_log("[SelfCheck] Network unreachable - check internet connection");
        fixed = false;
    }
