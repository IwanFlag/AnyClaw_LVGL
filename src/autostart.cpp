...
/* REG_AUTOSTART_VALUE defined in paths.h */
...
bool is_auto_start() {
    ...
    DWORD type = 0;
    LONG result = RegQueryValueExW(hKey, REG_AUTOSTART_VALUE, nullptr, &type, (LPBYTE)path, &size);
    RegCloseKey(hKey);
...
void set_auto_start(bool enable) {
    ...
    if (RegOpenKeyExW(HKEY_CURRENT_USER, REG_RUN_KEY, 0, KEY_SET_VALUE, &hKey) != ERROR_SUCCESS) {
        LOG_E("AUTOSTART", "Failed to open registry key");
        return;
    }
    ...
    if (enable) {
        ...
        RegSetValueExW(hKey, REG_AUTOSTART_VALUE, 0, REG_SZ,
                       (const BYTE*)quoted,
                       (DWORD)((wcslen(quoted) + 1) * sizeof(wchar_t)));
        LOG_I("AUTOSTART", "Enabled");
    } else {
        RegDeleteValueW(hKey, REG_AUTOSTART_VALUE);
        LOG_I("AUTOSTART", "Disabled");
    }
