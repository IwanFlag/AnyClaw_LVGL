...
3. OpenClaw未安装: 绿色安装按钮 → GitHub Release优先 → npm install -g openclaw回退
...
4. 下载源回退: GitHub(AnyClaw) → nodejs.org → npmmirror(CN) → 腾讯镜像
5. 网络/本地双源安装: GitHub Release优先 → npm registry回退 → bundled兜底
