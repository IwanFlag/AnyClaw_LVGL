...
- 新建 docs 跟踪文档: [docs/anyclaw_gui_parallel_checklist.md](anyclaw_gui_parallel_checklist.md)
- 写入“首次使用链路改造 v1”完整草案。
...
- 已修复 C2 Dual 开关可逆行为：从 Dual 关闭后，右栏折叠/展开按钮与可见性恢复正常逻辑。
- 已实现 C2 状态持久化：`c2_dual_result` 与 `work_chat_collapsed` 写入/读取配置，重启后保持上次使用习惯。
- 已在 Dual 开关动作后自动保存配置，减少状态丢失。
- 影响代码: [src/ui_main.cpp](../src/ui_main.cpp)

### Delta 2026-04-22 #21

- 已优化 Work 主区宽度上限策略：Dual 开启时主内容可使用更宽区域（大屏显示更充分）。
- 目标：减少“右栏保留后主内容过窄”的阅读与编辑阻塞。
- 影响代码: [src/ui_main.cpp](../src/ui_main.cpp)

### Delta 2026-04-22 #22

- 已修复 Dual 关闭后的右栏高度回退：从全高模式恢复到常规 `220/40` 高度策略。
- 已补齐 Work 右栏状态文案统一刷新：Dual/折叠状态切换后即时一致。
- 已实现手动折叠状态即时持久化：Work Chat 折叠切换后自动写入配置。
- 影响代码: [src/ui_main.cpp](../src/ui_main.cpp)

### Delta 2026-04-22 #23

- 已完成 C2/Work 状态文案国际化：Dual 固定/折叠/就绪/执行中/来自 Chat 镜像均支持中英文切换。
- 已在语言切换流程中追加状态文案刷新，避免出现“界面已切换语言但状态文案仍旧值”。
- 影响代码: [src/ui_main.cpp](../src/ui_main.cpp)

### Delta 2026-04-22 #24

- 已实现 Dual 前后折叠偏好记忆：开启 Dual 时保留原偏好，关闭 Dual 后恢复到之前折叠/展开状态。
- 已调整配置持久化语义：保存的是“用户偏好折叠态”而非 Dual 强制展开时的临时状态。
- 已实现手动折叠同步更新偏好并立即写配置。
- 影响代码: [src/ui_main.cpp](../src/ui_main.cpp)

### Delta 2026-04-22 #25

- 已新增 Work Chat 折叠按钮统一刷新函数，集中处理符号与禁用态，避免 Dual/普通模式分散逻辑。
- 已在 Dual 模式下显式禁用折叠按钮（LV_STATE_DISABLED），提升交互一致性并降低误操作预期差。
- 已简化 `work_chat_toggle_cb`：复用统一策略函数，去除重复的宽高/显隐分支代码。
- 影响代码: [src/ui_main.cpp](../src/ui_main.cpp)

### Delta 2026-04-22 #26

- 已修复聊天状态刷新对 Work 状态标签的误覆盖：`update_chat_status_label` 不再直接写入 Work 状态文案。
- 已将 Work 状态标签刷新统一收口到 C2 策略函数，确保 Dual/折叠/就绪文案在中英文切换后保持一致。
- 影响代码: [src/ui_main.cpp](../src/ui_main.cpp)

### Delta 2026-04-22 #27

- 已补齐 Work 执行态优先级：当 `g_work_waiting_ai` 为真时，Work 状态标签统一显示“正在执行任务”，不再被 Chat 状态刷新链路回退为 Ready/Collapsed。
- 已保持该逻辑继续走 C2 统一状态刷新函数，避免新增分散写入点。
- 影响代码: [src/ui_main.cpp](../src/ui_main.cpp)

### Delta 2026-04-22 #28

- 已补齐 Work 请求失败兜底：当 Chat 容器不可用、Gateway 配置缺失或流式线程启动失败时，自动清理 `g_work_waiting_ai`，防止 Work 状态卡在“执行中”。
- 已将 Work 发送/统一入口的状态写入进一步收口到 `c2_refresh_work_chat_state_label()`，减少直接写状态标签的分散点。
- 影响代码: [src/ui_main.cpp](../src/ui_main.cpp)

### Delta 2026-04-22 #29

- 已移除 Work Chat 面板初始化时对状态标签的硬编码英文文本，改由 `c2_apply_work_panel_policy()` 统一写入状态文案。
- 目标：避免初始化路径绕开 C2 状态策略函数，减少状态来源分叉。
- 影响代码: [src/ui_main.cpp](../src/ui_main.cpp)

### Delta 2026-04-22 #30

- 已新增 `c2_commit_work_panel_policy()` 统一封装“应用策略 + 布局重排 + 可选持久化”，替代 Dual 切换与 Work 折叠回调中的重复调用序列。
- 目标：减少 C2 交互路径中的顺序分叉，降低后续改动时漏调 `save_theme_config` 或 `relayout_panels` 的风险。
- 影响代码: [src/ui_main.cpp](../src/ui_main.cpp)

### Delta 2026-04-22 #31

- 已修复构建阻塞：补齐 `ui_main.cpp` 早期配置持久化路径所需的函数/状态前置声明，并消除重复定义导致的 `C2086`。
- 已修复 `ui_settings.cpp` 的前置声明与 API 兼容问题：补齐 `tr()` 前置声明、补充 `CJK_FONT_SMALL` 宏、将 `lv_event_send` 调整为 `lv_obj_send_event`。
- 已完成一次完整编译验证，`build.bat` 结果 `EXIT_CODE=0`。
- 影响代码: [src/ui_main.cpp](../src/ui_main.cpp), [src/ui_settings.cpp](../src/ui_settings.cpp)

### Delta 2026-04-22 #32

- 已将并行清单文件迁移到 docs 目录：`build/anyclaw_gui_parallel_checklist.md` -> `docs/anyclaw_gui_parallel_checklist.md`。
- 已修复 `LV_SDL_USE_EGL` 宏重复定义告警：移除 `src/lv_conf.h` 的手动定义，改由 LVGL 内部统一推导。
- 已验证打包产物可启动：`build/bin/AnyClaw_LVGL.exe` 启动成功（`WaitForInputIdle=True`），验证后已自动关闭进程。
- 已完成增量构建验证，`build.bat` 结果 `EXIT_CODE=0`。
- 影响代码: [src/lv_conf.h](../src/lv_conf.h), [docs/anyclaw_gui_parallel_checklist.md](anyclaw_gui_parallel_checklist.md)

### Delta 2026-04-22 #33

- 已补齐 C2 状态闭环：流式输出完成后，清理 `g_work_waiting_ai` 的同时立即刷新 Work 状态标签。
- 目标：避免任务刚完成时状态文案短暂滞留在“执行中”，提升双结果视图一致性。
- 已完成编译验证，`build.bat` 结果 `EXIT_CODE=0`。
- 影响代码: [src/ui_main.cpp](../src/ui_main.cpp)

### Delta 2026-04-22 #34

- 已清理仓库中的冗余构建脚本与历史编译产物（保留主入口 `build.bat` / `build.ps1`）。
- 已新增并验证 `build/build_oneclick.bat`：支持一键编译 + 便携包产出。
- 已重写 [docs/BUILD.md](BUILD.md)，统一当前有效的编译打包方法。
- 已完成端到端验证：产物运行成功、打包输出 `build/package/AnyClaw_LVGL_portable.zip`。
- 影响文件: [build/build_oneclick.bat](../build/build_oneclick.bat), [docs/BUILD.md](BUILD.md)
