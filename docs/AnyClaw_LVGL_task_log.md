...
## TASK-STABILITY05: 流式会话收口与健康线程停机加固（2026-04-09）
...
| # | 功能 | 文件 | 状态 |
|---|------|------|------|
| 1 | Chat/Work 发送入口在流式期间统一拒绝重入 | `src/ui_main.cpp` | ✅ |
| 2 | Send/Upload/Voice 与流状态联动禁用/恢复 | `src/ui_main.cpp` | ✅ |
| 3 | health_stop 超时告警 + 回调清理 | `src/health.cpp` | ✅ |
...
## TASK-FTP-RESUME-01: 下载断点续传与失败重试（2026-04-09）
...
| # | 功能 | 文件 | 状态 |
|---|------|------|------|
| 1 | 下载侧续传：REST 优先 + 跳过字节回退 | `src/ftp_client.cpp` | ✅ |
| 2 | 传输配置新增 `resume_download` | `src/ftp_client.h` | ✅ |
| 3 | Work 面板新增 `Retry Last` 按钮 | `src/ui_main.cpp` | ✅ |
| 4 | 失败后可复用上次 FTP 参数重试 | `src/ui_main.cpp` | ✅ |
...
## TASK-KB03: KB V2 检索打分与上下文裁剪（2026-04-09）
...
| # | 功能 | 文件 | 状态 |
|---|------|------|------|
| 1 | 关键词检索按评分排序（频次/位置/文件名命中） | `src/kb_store.cpp` | ✅ |
| 2 | 命中片段截取（围绕关键词）替代固定头部截断 | `src/kb_store.cpp` | ✅ |
| 3 | 上下文片段去重与长度预算控制 | `src/kb_store.cpp` | ✅ |
...
## TASK-REMOTE03: 会话状态迁移守卫与失败原因回显（2026-04-09）
...
| # | 功能 | 文件 | 状态 |
|---|------|------|------|
| 1 | 新增 `try_update_state()`（合法迁移校验） | `src/remote_protocol.h/.cpp` | ✅ |
| 2 | 非法状态迁移拒绝并返回原因 | `src/remote_protocol.cpp` | ✅ |
| 3 | UI 接入状态迁移失败日志（request/accept） | `src/ui_main.cpp` | ✅ |
...
## TASK-PRD-MAP-20260409: PRD 与代码对照复核（本轮）
...
| # | PRD 功能点 | 动作 | 文件 | 结果 |
|---|------------|------|------|------|
| 1 | SEC-UI-01 Security 状态面板应反映真实状态 | 静态文案改动态检测（API Key/Port/Config 可写） | `src/ui_settings.cpp` | ✅ |
| 2 | TM-01 任务管理可视化补强（调度视图） | Task List 增加 Scheduler 汇总（活跃会话/cron） | `src/ui_main.cpp` | ✅ |

### 当前仍未完成的 PRD 需求（继续执行）

| 模块 | PRD 功能编号 | 缺口说明 | 优先级 | 状态 |
|------|--------------|----------|--------|------|
| 远程协作真通道 | REMOTE-01 | 缺少真实桌面/语音数据通道与跨端断线重连 | P0 | 🔧 继续实施 |
| 远程安全闭环 | REMOTE-SAFE-01 | 仍需真实通道上的强制双确认与远端态同步 | P0 | 🔧 继续实施 |
| 本地模型安装闭环 | GEMMA-01 | 缺少下载断点续传与安装后完整可用性自检 | P0 | 🔧 继续实施 |
| 语音工作流补全 | MODE-02 | Voice 独立链路与 Work 端流程入口仍不完整 | P1 | 🔧 继续实施 |
...
| # | 功能 | 文件 | 状态 |
|---|------|------|------|
| 1 | 向导增加“退出向导”按钮并接入统一关闭逻辑 | ui_main.cpp | ✅ |
| 2 | 检测页内置安装进度显示（任务/步骤/进度条/结果） | ui_main.cpp | ✅ |
| 3 | Node/OpenClaw 安装入口改为 `Auto Install`（含 Node/npm bootstrap） | ui_main.cpp | ✅ |
| 4 | 修复 Back 后 Next 残留禁用状态 | ui_main.cpp | ✅ |
| 5 | Gemma 显式 Skip 按钮，不阻塞向导完成 | ui_main.cpp | ✅ |

### 验证
- Windows Release 构建通过（`AnyClaw_LVGL.exe` 生成）。

---

## TASK-LAN-CHAT-01: 局域网群聊中继模式（MVP）
...
| # | 功能 | 文件 | 状态 |
|---|------|------|------|
| 1 | 轻量 LAN 协议封包/解包 | lan_chat_protocol.h/.cpp | ✅ |
| 2 | LAN Host（中继）+ Client（连接） | lan_chat_client.h/.cpp | ✅ |
| 3 | 全局消息 / 私聊 / 建群 / 加群 / 群消息 | lan_chat_client.cpp | ✅ |
| 4 | Work 面板 LAN Chat 操作区与事件流 | ui_main.cpp | ✅ |
...
| # | 功能 | 文件 | 状态 |
|---|------|------|------|
| 1 | FTP 传输模块（上传/下载） | ftp_client.h/.cpp | ✅ |
| 2 | 传输进度回调与取消标志 | ftp_client.cpp | ✅ |
| 3 | Work 面板 FTP 配置与操作按钮 | ui_main.cpp | ✅ |
| 4 | 构建系统接入（源文件 + `wininet` 链接） | CMakeLists.txt | ✅ |
...
## TASK-UI-MODE-TOGGLE-20260409: Chat/Work 合并切换 + Voice 输入区入口
...
| # | 功能 | 文件 | 状态 |
|---|------|------|------|
| 1 | 顶栏模式按钮改为单按钮 `Chat<->Work` 切换 | `src/ui_main.cpp` | ✅ |
| 2 | Voice 按钮移动到聊天输入区（上传按钮左侧） | `src/ui_main.cpp` | ✅ |
| 3 | 顶栏移除设备按钮，仅保留设置入口 | `src/ui_main.cpp` | ✅ |
| 4 | 交互防抖：长任务按钮继续防重复点击 | `src/ui_main.cpp` | ✅ |
...
**状态：** ✅ 已完成
**完成时间：** 2026-04-08
**关联提交：** `7d074f2`, `a831a5a`

### 实现内容

| # | 功能 | 文件 | 状态 |
|---|------|------|------|
| 1 | 长任务统一进度桥接（begin/update/finish） | app.h, ui_main.cpp | ✅ |
| 2 | 左侧新增进度面板（任务名/步骤/进度条/结果） | ui_main.cpp | ✅ |
| 3 | Node.js 缺失自动下载并静默安装（三源回退） | openclaw_mgr.cpp | ✅ |
| 4 | OpenClaw 网络安装增加三 registry 自动切换 | openclaw_mgr.cpp | ✅ |
| 5 | Gemma 下载三源回退 + 慢速/超时切换 | openclaw_mgr.cpp | ✅ |
| 6 | 向导安装改异步线程，避免 UI 阻塞 | ui_main.cpp | ✅ |
| 7 | 新增“退出安装”按钮，支持中途取消 | ui_main.cpp, openclaw_mgr.cpp | ✅ |
| 8 | 安装结果轮询回写 UI（成功/失败/取消） | ui_main.cpp | ✅ |

### 验证

- Windows 原生构建通过（`tools\\windows\\build.bat`）。
- 向导安装流程可显示当前步骤与结果，取消后任务可被中断并回显“已取消”。
...
| TASK-REMOTE01 | 双端远程协作（桌面/语音/控制） | P0 | 🔧 实施中（V1：请求/接受/拒绝/断开状态机UI） |
| TASK-CTRL01 | 控制权模式（用户控制 / AI 控制） | P0 | 🔧 实施中（V1 持久化配置已落地） |
| TASK-LLM01 | 大模型接入模式（Gateway / Direct API） | P0 | ⏸️ Direct API 暂缓（当前固定 Gateway） |
| TASK-GEMMA01 | Gemma 4 本地模型安装选项（可选安装/多选/推荐模型） | P0 | 🔧 实施中（V2：Work 面板+向导集成+三源下载+进度展示+取消机制） |
| TASK-GUI-BUG01 | 聊天界面稳定性修复（附件卡片生命周期/信息显示一致性） | P0 | 🔧 实施中（已修复内存释放+打开失败日志+图标宏冲突） |
| TASK-MODE02 | Voice/Work 模式功能补全（语音链路、工作流面板入口） | P1 | 🔧 实施中（V1：Voice 输入可复用 Chat 发送链路） |
| TASK-SHARE02 | 附件发送队列（进度/失败重试/批量） | P0 | 🔧 实施中（V3：排队/发送/失败可点击重试/文件批量选择） |
| TASK-PROFILE02 | 头像资源选择与AI/用户角色卡展示 | P1 | 🔧 实施中（V1：Work 面板角色卡预览已落地） |
| TASK-REMOTE-SAFE01 | 远程协作安全闸门（双确认/倒计时/红条/一键断开） | P0 | 🔧 实施中（V2：双确认弹窗 + 3秒倒计时 + 红条 + 一键断开） |
| TASK-WIDGET02 | 通用表单插件库（section/field/dropdown/button） | P1 | ✅ 已完成（aw_form.h 已落地并接入 Work 面板） |

---

## PRD-代码差距清单（2026-04-08）

> 本节用于快速追踪“PRD 已定义但代码未闭环”的事项，便于连续开发。

| 模块 | 关联任务 | 当前缺口 | 状态 |
|------|----------|----------|------|
| 远程协作 | TASK-REMOTE01 / TASK-REMOTE-SAFE01 | 真正桌面流与语音链路、跨端会话鉴权（已补请求超时自动重试） | 🔧 实施中 |
| 本地模型（Gemma4） | TASK-GEMMA01 | 内置2B打包策略（已补下载续传与安装后可用性自检） | 🔧 实施中 |
| 本地知识库 | TASK-KB01 | 索引构建、增量更新、检索接入聊天上下文 | 🆕 待实施 |
| 权限系统深化 | TASK-PERM06 / TASK-AUDIT01 | 剩余命令入口统一拦截、审计链式防篡改字段 | 🆕 待实施 |
| 可观测性与体检 | TASK-OBS01 / TASK-BOOT01 | trace_id 全链路、体检修复向导落地 | 🆕 待实施 |
| 特性开关 | TASK-FLAG01 | feature flag 配置源、灰度切换、回滚策略 | 🆕 待实施 |

---

## TASK-REMOTE04: 远程请求超时自动重试（2026-04-09）
...
## TASK-GEMMA02: Gemma 下载续传与安装后校验（2026-04-09）
...
- `src/openclaw_mgr.cpp`
  - 多源下载时，如果目标文件已存在，使用 `curl -C -` 进行断点续传。
  - 下载成功后新增校验阶段：逐个检查所选模型文件是否存在且大于阈值，未通过则整体失败。
  - 下载失败时自动删除残留半包文件，避免下次误判为可用模型。
...
## TASK-MODE02-V2: Voice 到 Work 流程补全（2026-04-09）
...
### 实现内容
- `src/ui_main.cpp`
  - Voice 输入区新增 `Send To Work` 按钮。
  - 将语音转写内容直接写入 Work Prompt，并自动切到 Work 模式。
  - 保留原 `Send To Chat` 路径，实现 Chat/Work 双路分发。
...
| 旧步骤 | 新步骤 | 变更 |
|--------|--------|------|
| Step 0: Language | Step 0: Language | 不变 |
