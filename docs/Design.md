...
1. **输出区（flex-grow）：** 占据主区域，显示对话气泡/AI思考/文件输出。垂直滚动。

2. **控制行（固定高度）：** 位于输出区下方，包含 Agent 选择、Report 按钮、AI行为下拉。

3. **输入区（固定高度）：** 位于底部，包含附件按钮、文本输入框、发送按钮。

**子元素结构：**

| 元素 | 尺寸 | 位置 | 行为 |
|------|------|------|------|
| 左侧面板 | LEFT_PANEL_W × PANEL_H | 根容器左侧 | Session/Cron 列表，2s 刷新 |
| 输出区 | flex-1 | 右侧主区域 | 垂直滚动容器 |
| 用户气泡 | 最大 70% 宽度 | 输出区右侧 | 渐变背景，靠右 |
| AI 气泡 | 最大 70% 宽度 | 输出区左侧 | surface 背景，靠左，左侧 accent 竖线 |
| AI 思考块 | 100% 宽度 | AI 气泡下方 | 可折叠，caption 标题 |
| 文件输出块 | 100% 宽度 | AI 思考下方 | 顶部标签栏，code 字体 |
| 控制行 | 100% × BTN_H | 输出区下方 | 固定高度，按钮组 |
| 输入区 | 100% × CHAT_INPUT_MIN_H | 界面最底部 | 固定高度，自动增长 |

### UI-12: Bot · Work 模式

Work 模式激活时，左 nav 高亮 Bot，Chat/Work 切换器切到 Work，内容区显示 mode_panel_work（mode_panel_chat 隐藏）。

**与 Chat 模式的本质区别：** UI-11 输出区是对话气泡；UI-12 输出区是 **5 个 section 纵向排列**（Runtime Mode / Agent Trace / Observability / Step Stream / Output Area），外加 1 个右侧浮层（Work Chat）。
...
│    │                                                                    │
│ 🦞*│   ┌─ Runtime Mode ────────────────────────────────────────────┐    │
│    │   │ Control mode: [User controls AnyClaw ▼]                   │    │
│    │   │ LLM access:    [Gateway mode (OpenClaw) ▼]                │    │
│    │   │ AI mode:       [Autonomous ▼]         [Show advanced ▐]   │    │
│    │   └──────────────────────────────────────────────────────────┘    │
│    │   ┌─ Agent Trace ─────────────────────────────────────────────┐    │
│    │   │ Next step: ×××                                            │    │
│    │   │ [Agent script textarea]                                   │    │
│    │   └───────────────────────────────────────────────────────────┘    │
│    │   ┌─ Observability ───────────────────────────────────────────┐    │
│    │   │ [Trace timeline textarea]                                  │    │
│    │   │ [Refresh]  [Sort By Latency]  [Toggle Fail-Only]         │    │
│    │   └───────────────────────────────────────────────────────────┘    │
│    │   ┌─ Step Stream ─────────────────────────────────────────────┐    │
│    │   │ ┌──────────────────────────────────────────────────────┐  │    │
│    │   │ │ 📄 读取 config.json                    ✓ 完成       │  │    │
...
│    │                                         └────────────────────┘    │
│    ├────────────────────────────────────────────────────────────────────┤
│    │  [Agent ▼]  [Report]  [AI行为 ▼]                            │ ← ctrl_bar（独立浮层）
│    ├────────────────────────────────────────────────────────────────────┤
│    │  [📎] │ 输入消息...                                    [▶]   │ ← 输入区
└────┴────────────────────────────────────────────────────────────────────┘
```
...
1. **ctrl_bar（固定高度）：** 独立浮层，非 mode_panel_work 子节点。包含 Agent 下拉、Report 按钮、AI行为下拉。在 Chat 和 Work 模式均显示，位置固定在输入区上方。
2. **输入区（固定高度）：** 独立浮层，包含附件按钮、文本框、发送按钮。
3. **Runtime Mode section（内容区宽 × auto）：** mode_panel_work 第 1 个子节点。三个下拉（控制模式/LLM访问/AI交互模式）+ 高级模块开关 + Health Check 按钮。
4. **Agent Trace section（内容区宽 × auto）：** 第 2 个子节点。下一步提示 + Agent script textarea。
5. **Observability section（内容区宽 × auto）：** 第 3 个子节点。Trace 时间线 textarea + 三个操作按钮。
6. **Step Stream section（内容区宽 × flex-1）：** 第 4 个子节点。StepCard 列表，可折叠。**StepCard 状态：** ● 进行中(绿 spinner) / ✓ 完成(绿) / ✗ 失败(红) / ? 等待确认(蓝)。**写操作按钮：** [接受✓] [拒绝✗] [回退↩]
7. **Output Area section（内容区宽 × auto）：** 第 6 个子节点（splitter 第 5 个子节点在中间）。输出标签 + 渲染器内容。
8. **右侧浮层 Work Chat（固定尺寸，靠右）：** 第 7 个子节点。可折叠浮层，包含 chat feed + input。
...
[用户点击左侧导航 Bot，进入 Chat 模式]
   ...
   Ctrl+T 切换到 Work 模式
       │
       ▼
   内容区隐藏 mode_panel_chat，显示 mode_panel_work
   ctrl_bar 和输入区保持不变
       │
       ├─── 设置 Runtime Mode ──→ 下拉选择控制模式/LLM/AI交互模式
       │
       ├─── AI 执行任务 ──→ Step Stream 自动追加 StepCard，实时显示状态
       │
       ├─── 写操作需确认 ──→ StepCard 显示 [接受✓][拒绝✗][回退↩]
       │
       ├─── 可拖动分隔条 ──→ 调整 Step Stream 和 Output Area 的高度分配
       │
       ├─── 右侧浮层 Work Chat ──→ 可折叠，展示 Work 模式下的对话记录
       │
       └─── 输入区发送任务 ──→ 触发新的任务执行，StepCard 追加新步骤
```
...
| 场景 | 行为 |
|------|------|
| AI 执行任务中 | Step Stream 自动追加 StepCard，显示当前步骤 |
| 写操作需确认 | StepCard 显示 [接受✓][拒绝✗][回退↩] 按钮 |
| Terminal 有输出 | 在 Output Area section 追加终端内容 |
| 用户折叠 Work Chat | 右侧浮层收缩，mode_panel_work 占满右侧空间 |

### UI-05: File 模块

File 模块激活时，左侧导航高亮 File，左侧面板（Session/Cron）隐藏，内容区全宽显示 File 内容。
...
**约束：**
- File 内容容器（module_files_panel）父容器为内容区，**不是** left_panel，宽度 = 内容区宽
- left_panel（Session/Cron）在 File 模块下隐藏
...
| 模块 | 左侧面板（Session/Cron） | 右侧内容区 |
|------|------------------------|-----------|
| Bot | ✅ 显示（双栏结构） | ✅ 显示（输出区 + 控制行 + 输入区） |
| File | ❌ 隐藏 | ✅ 全宽显示 File 内容 |
...
[用户点击左侧导航 File，进入 File 模块]
       │
       ▼
    内容区全宽显示 File 内容，左侧面板隐藏
       │
       ├─── 浏览文件列表 ──→ 显示文件路径 + 修改时间
       │                                                              │
       ├─── 点击 [刷新] 按钮 ──→ 重新索引文件，显示更新状态
       │                                                              │
       └─── 点击 [上传] 按钮 ──→ 弹出文件上传菜单（UI-36）
```

**布局结构（内容区共 1 层）：**

1. **File 内容区（全宽）：** 占据整个内容区宽度，left_panel 隐藏。显示文件列表容器。

**子元素结构：**

| 元素 | 尺寸 | 位置 | 行为 |
|------|------|------|------|
| module_files_panel | 内容区宽 × auto | 内容区内 | surface 背景，1px border |
| module_files_view | 100% × flex-1 | panel 内 | textarea，显示文件列表 |
| module_files_state | 100% × auto | panel 底部 | caption 字体，索引状态 |
| 刷新按钮 | auto × BTN_H | state 左侧 | secondary 样式，点击刷新索引 |
| 上传按钮 | auto × BTN_H | state 中间 | accent 渐变，点击打开上传菜单 |
