...
│  │  检测结果                    │    │  📺 检测与安装日志                    │ │
│  │                             │    │                                    │ │
│  │  ● Node.js    v22.14.0 Pass│ │  > 正在检测 Node.js...              │ │
│  │  ● npm         v10.8.0  Pass│ │  > Node.js 已就绪 ✓                 │ │
│  │  ● 网络连通     OK        Pass│ │  > 正在检测 npm...                  │ │
│  │  ● OpenClaw   未安装     Warn │ │  > 正在检测网络连通性...             │ │
│  │  ● Hermes     未安装     Warn │ │  > 网络 OK ✓                        │ │
│  │  ● Claude CLI 未安装     Warn │ │  > 未检测到 OpenClaw                │ │
│  │  ● 磁盘空间   85GB       Pass │ │  > 未检测到 Hermes                  │ │
...
- 双面板总宽: 1100px (86%)，左面板40%(480px)/右面板45%(580px)/间距40px
- 内容区内边距: 左右各 90px (7%)，上边距 28px (3.5%)
- 7 项检测行: 行高 24px，行间距 36px（内容区 6.4%）
- 安装按钮区: y=522~562，间距 16px，水平居中
...
[进入 Step 1]
       │
       ▼
    自动开始 7 项并行检测（Node.js / npm / 网络连通 / OpenClaw / Hermes / Claude Code CLI / 磁盘空间）
       │
       ▼
    ┌─────────────────────────┐
    │  检测结果实时更新到左面板  │
...
2. **双面板区：** 标题区下方左右并排两个矩形容器面板，中间有 16px 间距，两个面板总宽度占内容区宽度的 **86%**（LV_PCT(86)），在内容区内部水平居中。
   - **左面板（检测结果）：** 宽度约为双面板总宽的 **40%**（~410px），内部从上到下排列 7 个检测项状态行。每行包含：状态图标（✅/⏳/⚠️/❌）+ 检测项名称 + 版本号或状态文字。检测项包括 Node.js、npm、网络连通性、OpenClaw、Hermes、Claude Code CLI、磁盘空间。检测项之间垂直间距 28px（内容区 6%）。
