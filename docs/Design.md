...
**约束：**
- File 内容显示在 left_panel 区域内（Session/Cron 隐藏）
- right_panel 空闲
- 搜索框在上方，文件列表占满剩余高度
...
| 元素 | 类型 | 父容器 | 尺寸 | 行为 |
|------|------|--------|------|------|
| module_files_panel | container | module_placeholder | left_panel 全宽 × auto | surface 背景 |
...
| 模块 | left_panel（Session/Cron） | right_panel |
|------|---------------------------|-------------|
| Bot  | ✅ 显示（Session/Cron 列表） | ✅ 显示（输出区 + ctrl_bar + 输入区） |
| File | ❌ 隐藏（module_placeholder 填充） | 空闲 |
