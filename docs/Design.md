...
Task 模块激活时，左侧导航高亮 Task，左侧面板（Session/Cron）隐藏，内容区全宽显示 Task 内容。
...
**约束：**
- Task 内容容器（module_placeholder）父容器为内容区，**不是** left_panel，宽度 = 内容区宽
- left_panel（Session/Cron）在 Task 模块下隐藏
- 内容区（右侧）完全展开，不隐藏（只是没有 Bot 模块的双栏结构）
...
1. **Task 内容区（全宽）：** 占据整个内容区宽度，left_panel 隐藏。显示任务列表容器。

**子元素结构：**

| 元素 | 尺寸 | 位置 | 行为 |
|------|------|------|------|
| module_tasks_panel | 内容区宽 × auto | 内容区内 | surface 背景，1px border |
