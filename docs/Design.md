...
| Chat/Work | 11 | 526 | fill=#252A38 |
| Settings | 11 | 594 | fill=#252A38 |
...
高度 52px，背景 `#0A0C10`，顶部分隔线 `y=594`，颜色 `#3DD68C` @ 6%。按钮垂直居中 `y=610`，高度 36px。
...
- **ctrl_bar**（y=537, h=31）：背景 `#1A1D27`，圆角 rx=7，内含 5 个控件按钮
- **附件芯片行**（y=573, h=20）：芯片 w=148, h=16, rx=3，带 ✕ 关闭按钮
- **分隔线**（y=595）：stroke=`#3DD68C` @ 6%，1px
- **文本输入区**（y=599）：placeholder "输入消息..."，`#6B7280`
- **附件按钮**（x=1080, y=609, w=21, h=21, rx=5）：`+` 符号
- **发送按钮**（cx=1130, cy=612, r=18）：圆形渐变，▶ 符号
...
│  │px  │    │    │  内容区 (y=41, h=605)                                  ││
│  │    │    │    │  ┌─────────────────────┬──────────────────────────┐ ││
│  │    │    │    │  │ left_panel (276px)   │ right_panel (818px)    │ ││
│  │    │    │    │  └─────────────────────┴──────────────────────────┘ ││
│  └────┴────┴───────────────────────────────────────────────────────┘│
└─────────────────────────────────────────────────────────────────────┘
```

左导航栏内部结构（x=0, y=41, w=82px, h=605px，flex COLUMN 布局）：

**分层结构：**

- **left_nav 容器**（x=0, y=41, w=82px, h=605px）：导航栏根容器，flex COLUMN 布局，内边距 0，背景 panel 色。
...
[应用启动]
       │
       ▼
    创建左导航栏 left_nav (x=0, y=41, w=82, h=605)
       │
       ...
       │
       └─── 创建 nav_bot (h=120px, y=526) ──→ 创建 Chat/Work 切换 + Settings
                                        │
...
- nav_top → nav_spacer：nav_top 底部 y=304，flex_grow 填充剩余全部空间
- nav_spacer → nav_bot：nav_bot 顶部 y=526
...
```
right_panel
├── 输出区（flex-grow，垂直滚动，h=496）  ← y=41，底部=537
├── 控制行（固定高度 31px）              ← y=537，gap=0
└── 输入区（固定高度 62px，三行）        ← y=568，底部=630，跨面板对齐任务容器
```
...
|| 元素 | 宽 | 高 | x | y | 计算 |
|------|---:|---:|---:|---:|------|
| left_panel | 276 | 598 | 82 | 48 | x = nav_w(82) + 0(7px间距)，y = TITLE_H(41)+7 |
| right_panel | 818 | 605 | 365 | 41 | x = left_panel右 + 7，y同 |
| 输出区(chat_cont) | 818 | 496 | 365 | 41 | flex-grow 填充，底部与任务容器对齐 |
| 控制行(ctrl_bar) | 818 | 31 | 365 | 537 | y = 输出区底(537)，gap=0 |
| 输入区(chat_input) | 818 | 62 | 365 | 568 | y = 控制行底(568)，gap=0，底部=630 |
...
│  │                                                                               │  │
│  └───────────────────────────────────────────────────────────────────────────────┘  │
│  ┌─ ctrl_bar（控制栏，y=537, h=31）──────────────────────────────────────────────┐  │
│  │  [Agent ▼]  [AI行为 ▼]  [AI托管 ○]  [Voice ○]                     0/2000    │  │  ← 字数统计在右上角
│  └───────────────────────────────────────────────────────────────────────────────┘  │
│  ┌─ chat_input（输入框，y=568, h=62）────────────────────────────────────────────┐  │
│  │  ┌─ 输入框内部 y=573 ───────────────────────────────────────────────────┐  │  │
...
**第四层：左导航内部（x=0, y=41, w=82, h=605）从上到下：**
...
│       │
│  ┌──┐ │  ← Chat/Work 切换，52×52px，圆角9px，x=11, y=526，💬
│  │💬│ │
│  └──┘ │
│  ┌──┐ │  ← Settings 图标，52×52px，圆角9px，x=11, y=594（与左面板底部对齐）⚙️
│  │⚙️│ │
│  └──┘ │
```
...
| Session Card | x=92, y=298, w=256, h=190 | 会话列表，距 AI伙伴容器 10px |
| 任务 Card | x=92, y=504, w=256, h=126 | 所有任务，距 Session Card 16px，距面板底部 16px |
| 右面板 | x=365, y=41, w=818, h=605 | 消息区+控制行+输入框 |
| chat_cont | x=365, y=41, w=818, h=496 | flex-grow 填充，消息列表，底部与任务容器对齐 |
| ctrl_bar | x=365, y=537, h=31 | Agent/AI行为/AI托管/Voice 四个控件，字数统计右上角 |
| chat_input | x=365, y=568, h=62 | 输入框内：芯片行(第一行)+文本区(第二行起)，底部=630 |
| 附件芯片行 | x=378, y=575, h=16 | 输入框内第一行，横排文件芯片+删除X+滚动按钮 |
| 用户气泡 | transform x=980, w=139 | 蓝色渐变，右置，气泡右边界距头像左边界 12px |
| 用户头像 | cx=171, r=14 | 头像右边界=1159，距面板右边界 24px |
| AI头像 | cx=17 | 气泡左侧，间隙21px，SVG 青虫子 |
| 上传按钮 | x=1080, y=609, 21×21 | +号，输入框右下角 |
| 发送按钮 | cx=1130, cy=612, r=18 | 圆形，绿色渐变，输入框右下角 |
...
**第三层：左导航内部（x=0, y=41, w=82, h=605）**

> 导航栏宽度 82px，图标 52×52px，水平居中（x=11）。图标间间距 16px。
...
| Chat/Work 图标 | 52×52 | 11 | 526 | 圆角9px，高亮背景#252A38，💬 |
| Settings 图标 | 52×52 | 11 | 594 | 圆角9px，透明背景，⚙️ |

**第四层：左侧面板（x=82, y=48, w=276, h=598）**

| 控件 | 尺寸 | x | y | 说明 |
|------|------:|---:|---:|------|
| 面板容器 | 276×598 | 82 | 48 | 圆角10px，surface 背景，浅绿边框1px(opacity=0.03) |
| AI伙伴容器 | 256×203 | 92 | 58 | 圆角10px，边框1.5px accent，SVG 青虫子 + 文字 + 状态行 |
...
**第五层：右面板（x=365, y=41, w=818, h=605）**

内部分为三个垂直区域：

**5.1 消息区（chat_cont，y=41，高=496，flex-grow）：**

| 控件 | 尺寸 | x | y | 说明 |
|------|------:|---:|---:|------|
| 消息列表 | 818×496 | 365 | 41 | clipPath 裁剪，flex-grow 填充 |
...
**5.2 控制栏（ctrl_bar，y=537，高=31）：**

| 控件 | 尺寸 | x | y | 说明 |
|------|------:|---:|---:|------|
| 控制栏容器 | 818×31 | 365 | 537 | 圆角7px，surface 背景 |
| Agent 下拉 | 69×22 | 372 | 542 | 圆角11px，accent透明底，文字 accent色，"Agent ▼"，text-anchor 居中 |
| AI行为 下拉 | 69×22 | 446 | 542 | 圆角11px，深灰底，文字 text-primary，"AI行为 ▼"，text-anchor 居中 |
| AI托管 开关 | 69×22 | 520 | 542 | 圆角11px，左侧灰圆滑块，"AI托管"，text-anchor 居中，文字灰色（关闭态） |
| Voice 开关 | 69×22 | 594 | 542 | 圆角11px，左侧灰圆滑块，"Voice"，text-anchor 居中，文字灰色（关闭态） |
| 字数统计 | auto | 1135 | 556 | "0/2000"，text-dim色，ctrl_bar 右上角 |

> **控件间距（x轴）：** Agent(372)→AI行为(446) gap=74，AI行为→AI托管(520) gap=3，AI托管→Voice(594) gap=74，Voice→（边距）gap=541。
>
> **AI托管/Voice 为开关样式**（默认关闭，灰色圆圈滑块在左侧），尺寸统一为 69×22。

**5.3 输入区（chat_input，y=568，高=62）：**

输入框内部署附件芯片行为第一行，用户输入从第二行开始。整体高度固定 62px，底部与任务容器底部对齐（y=630）。

**输入框内部结构（y=573，高=57）：**

| 控件 | 尺寸 | x | y | 说明 |
|------|------:|---:|---:|------|
| 输入框容器 | 698×57 | 372 | 573 | 圆角5px，深灰背景，浅绿边框(opacity=0.06) |
| 附件芯片行 | 698×20 | 378 | 575 | 第一行，粘贴文件后自动生成，横排显示 |
| 分隔线 | 698×1 | 372 | 595 | 芯片行与文本输入区分隔，浅绿6%opacity |
| 文本输入区 | — | 380 | 599 | 第二行起，光标默认在此位置 |
| 上传按钮 | 21×21 | 1080 | 609 | 圆角5px，深灰背景，+号，输入框右下角 |
| 发送按钮 | r=18 | 1130 | 612 | 圆形，绿色渐变，白色 ▶，输入框右下角 |

**附件芯片行内部（y=575，高=16）：**

|| 控件 | 尺寸 | x | y | 说明 |
|------|------:|---:|---:|------|
| 文件芯片 | 动态 | 378 | 575 | 横排，圆角3px，深灰背景，左📄+文件名+右删除✕ |
| 删除按钮 | ⌀10 | 芯片右-5 | 583 | 芯片右侧圆形X按钮，点击移除该附件 |
...
**相邻关系：**
- 标题栏 → 内容区（y=41，无缝衔接）
- 左导航 → 间距(7px) → 左侧面板（x=82）
- chat_cont → ctrl_bar（y=537，紧贴，gap=0）
- ctrl_bar → chat_input（y=568，紧贴，gap=0）
- chat_input 底部 → 任务容器底部（y=630，完全对齐）
- 左面板任务容器 → 右面板 chat_input（底部同为 630px，跨面板对齐）
