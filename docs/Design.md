...
> 所有 SVG 坐标、控件尺寸、间距均基于主窗口逻辑像素（**1280×800**）为比例值。
...
| §8.2 | Skill 管理 | §12 主界面 → 左导航 Skill 图标 → Skill 管理面板 | UI-65 |
| §8.4 | 知识库 | §12 主界面 → 左导航 KB 图标 → 知识库面板 | UI-60 |
...
| §16 | 配置界面 | 模型管理 | PRD §7 |
| §17 | 扩展界面 | Skills/KB 双入口：左导航图标 + Settings → C3 | UI-43, UI-60, UI-65 |
...
尺寸：16/20/32/48px（PNG，托盘 API 使用位图）。**必须提供 2x 高分辨率版本**（32/40/64/96px），文件名加 `@2x` 后缀，如 `tray_active_32@2x.png`。高 DPI 屏幕（≥200%）自动使用 2x 版本。

托盘菜单字体：`-12` → `-24`（放大 2 倍），菜单项高度 `24px` → `48px`，LED 指示灯 `4px` → `8px`。见 **UI-63**。
...
> 本节定义 AnyClaw LVGL 所有 UI 界面的 SVG 线框图统一规范，保证跨界面控件尺寸和风格一致。所有 SVG 均遵循本文，不可随意发挥。
> **规范版本：v1 | 更新：2026-04-26**

#### 5.4.1 基础规范

**用途：** SVG 线框图用于在 Design.md 中直观展示每个 UI 界面的布局和控件，是需求评审和开发对接的视觉语言。所有界面在编码实现前必须先有对应 SVG 确认。

**比例参考尺寸：** 1280 × 800（主窗口逻辑像素）。SVG 作为比例示意图，所有坐标/尺寸均为 1280×800 比例值，实际渲染由代码按窗口缩放决定。viewBox 和 width/height 统一用 `0 0 1280 800`。

**主题：** V1 Matcha（默认主打主题）。

**界面分类（版本标记规则）：**
- **App 界面**（如 UI-11/12/03 等）：右下角加版本标记 `Matcha v1 · UI-XX`，font-size=8, fill=#6B7280, opacity=0.4，anchor=end，位置 x=1172, y=656
- **非 App 界面**（如 UI-01 启动检查页等纯功能页）：不加版本标记

#### 5.4.2 主题色板（V1 Matcha）

| 语义 Token | 色值 | 用途 |
|-----------|------|------|
| `bg` | `#0F1117` | 窗口最底层背景 |
| `surface` | `#13151E` | 面板、输入区、低层容器 |
| `panel` | `#1A1D27` | 浮层面板、弹窗 |
| `raised` | `#1E2230` | 气泡、浮层、tooltip |
| `overlay` | `#252A38` | hover 态、选中态 |
| `text_primary` | `#E8ECF4` | 主文字、标题 |
| `text_secondary` | `#8B92A8` | 次要文字、说明 |
| `text_tertiary` | `#6B7280` | 占位符、弱文字 |
| `accent` | `#3DD68C` | 主强调色、品牌色、成功 |
| `accent_hover` | `#2BB673` | 主按钮 hover |
| `accent_active` | `#1FA35E` | 主按钮 active |
| `accent_subtle` | `#3DD68C1A` | 背景着色（10% opacity） |
| `accent_secondary` | `#6C9FFF` | 辅助蓝、链接 |
| `danger` | `#EF4444` | 错误、失败 |
| `danger_hover` | `#DC2626` | 危险按钮 hover |
| `warning` | `#FFBE3D` | 警告、思考中 |
| `info` | `#6C9FFF` | 链接、信息 |
| `blush` | `#FF9EB5` | 腮红点缀（opacity 0.7） |

#### 5.4.3 字体规范

| 场景 | 字号 | 字重 | 颜色 |
|------|------|------|------|
| 标题栏 Logo 文字 | 13px | 600 | `#E8ECF4` |
| 窗口标题 | 13px | — | `#E8ECF4` |
| 正文 | 12px | — | `#E8ECF4` |
| 次要说明 | 11px | — | `#8B92A8` |
| 弱/占位 | 10px | — | `#6B7280` |
| 代码 | 11px | — | `#8B92A8` |
| 徽章文字 | 9px | — | 与状态对应 |
| 版本标记 | 8px | — | `#6B7280`, opacity=0.4 |

字体栈：`system-ui, -apple-system, "HarmonyOS Sans SC", "Microsoft YaHei", sans-serif`
代码字体：`Consolas, "Cascadia Code", monospace`
字体优先级：`Plus Jakarta Sans` > `HarmonyOS Sans SC` > `system-ui`

#### 5.4.4 Header（标题栏）

高度 41px，背景 `#0A0C10`，无圆角（rx=0），底部 1px 分隔线 `y=41`，颜色 `#3DD68C` @ 6%。

**Logo（大青虫子 SVG，36×36）：** 不使用 emoji，SVG 路径如下：

```
<circle cx="27" cy="17" r="8" fill="#3DD68C"/>
<line x1="21" y1="9" x2="19" y2="5" stroke="#3DD68C" stroke-width="1.5" stroke-linecap="round"/>
<line x1="33" y1="9" x2="35" y2="5" stroke="#3DD68C" stroke-width="1.5" stroke-linecap="round"/>
<circle cx="19" cy="5" r="2.5" fill="#3DD68C"/>
<circle cx="35" cy="5" r="2.5" fill="#3DD68C"/>
<ellipse cx="23" cy="15" rx="2.5" ry="3" fill="#0F1117"/>
<ellipse cx="31" cy="15" rx="2.5" ry="3" fill="#0F1117"/>
<circle cx="24" cy="14" r="1" fill="#FFFFFF"/>
<circle cx="32" cy="14" r="1" fill="#FFFFFF"/>
<ellipse cx="20" cy="19" rx="3" ry="1.5" fill="#FF9EB5" opacity="0.7"/>
<ellipse cx="34" cy="19" rx="3" ry="1.5" fill="#FF9EB5" opacity="0.7"/>
<path d="M 24 21 Q 27 24 30 21" stroke="#0F1117" stroke-width="1.2" fill="none" stroke-linecap="round"/>
```

**窗口控制按钮（3 个，右对齐）：**

| 按钮 | x | y | w | h | rect 填充 | 文字颜色 |
|------|---|---|---|---|----------|---------|
| `[—]` | 1085 | 24 | — | — | 无 rect | `#6B7280` |
| `[□]` | 1106 | 12 | 30 | 17 | `#252A38`, rx=4 | `#6B7280` |
| `[✕]` | 1142 | 12 | 30 | 17 | `#3DD68C` @ 20%, rx=4 | `#3DD68C` |

#### 5.4.5 左 NAV（左侧导航栏）

宽度 82px，高度 759px（800 - 41），背景 `#0A0C10`。图标圆角矩形 w=52, h=52, rx=9，x=11。

| 图标 | x | y | 高亮态 |
|------|---|---|-------|
| Bot | 11 | 48 | fill=#252A38 |
| File | 11 | 116 | fill=#252A38 |
| KB | 11 | 184 | fill=#252A38 |
| Skill | 11 | 252 | fill=#252A38 |
| Chat/Work | 11 | 696 | fill=#252A38 |
| Settings | 11 | 764 | fill=#252A38 |

非高亮态：fill=transparent。图标文字居中，font-size=17，`#6B7280`。

#### 5.4.6 面板（Panel / Card）

背景 `#13151E`，圆角 rx=10，边框 stroke=`#3DD68C` @ 3%，stroke-width=1。内边距 pad=24px（四边相同）。

嵌套面板（如 AI 伙伴容器）：stroke-width 可增至 1.5。

#### 5.4.7 按钮

| 类型 | 填充 | 文字 | 高度 | 圆角 | 最小宽度 |
|------|------|------|------|------|---------|
| Primary | `#3DD68C` | `#0F1117`, 600 | 40px min | rx=11 | 90px |
| Secondary | transparent | `#3DD68C` | 40px min | rx=11 | 90px |
| Ghost | transparent | `#8B92A8` | 40px min | rx=11 | — |
| Danger | `#EF4444` | `#FFFFFF`, 600 | 40px min | rx=11 | 90px |
| 发送（圆形） | 渐变 `#4ADE80→#22C55E` | `#0F1117` | r=18, 圆形 | — | — |

**Primary 按钮叠加态：**
- Hover：bg 变为 `#2BB673`，shadow_md
- Active：bg 变为 `#1FA35E`，scale(0.97)
- Disabled：bg=`#1A1D2780`，text=`#6B728080`，opacity=0.4

#### 5.4.8 状态徽章（Badge）

宽度 80px，高度 18px，圆角 rx=9，dot r=3.5 与文字间距 4px，文字居中 font-size=9。

| 状态 | dot 颜色 | 填充 | 文字颜色 |
|------|---------|------|---------|
| Pass | `#3DD68C` | `#3DD68C` @ 12% | `#3DD68C` |
| Fail | `#EF4444` | `#EF4444` @ 12% | `#EF4444` |
| Unknown | `#6B7280` | `#6B7280` @ 12% | `#6B7280` |
| Checking | `#6B7280` | `#252A38` | `#6B7280` |

#### 5.4.9 Footer（底部栏）

高度 52px，背景 `#0A0C10`，顶部分隔线 `y=748`，颜色 `#3DD68C` @ 6%。按钮垂直居中 `y=764`，高度 36px。

#### 5.4.10 输入框

**单行输入框：**
背景 `#13151E`，边框 stroke=`#3DD68C` @ 6%，圆角 rx=5，高度 44px 基准。

**多行输入框（chat_input）：** 拆分为三个区域：
- **ctrl_bar**（y=661, h=31）：背景 `#1A1D27`，圆角 rx=7，内含 5 个控件按钮
- **附件芯片行**（y=698, h=20）：芯片 w=148, h=16, rx=3，带 ✕ 关闭按钮
- **分隔线**（y=720）：stroke=`#3DD68C` @ 6%，1px
- **文本输入区**（y=724）：placeholder "输入消息..."，`#6B7280`
- **附件按钮**（x=1140, y=738, w=21, h=21, rx=5）：`+` 符号
- **发送按钮**（cx=1190, cy=742, r=18）：圆形渐变，▶ 符号

#### 5.4.11 消息气泡

**用户气泡：** 渐变 `#3B82F6 → #2563EB`，圆角 rx=16，阴影 filter，右侧对齐。

**AI 气泡：** 背景 `#1E2230`，左侧 3px 绿色竖线（accent），圆角 rx=12，阴影 filter。

#### 5.4.12 思考气泡

背景渐变 `#1F1A0A → #141208`（defs linearGradient），边框 stroke=`#FFBE3D` @ 20%，圆角 rx=7。内部包含思考标签（🔍 思考过程）和折叠提示（点击折叠）。
...
Panel 内 pad=24px。行内容垂直居中（行高 28px，内元素 line-height=28 居中）。

#### 5.4.14 弹窗（Modal）

遮罩 `#000000` @ 50-60%，背景 `#1A1D27`，边框 stroke=`#3DD68C` @ 15%，圆角 rx=12，阴影 shadow_lg。

#### 5.4.15 开关（Switch）

宽度 36px（min），高度 = 宽度 × 56%，圆角 = 高度 / 2。
关闭态：轨道 `#373C55`，滑块 `#8B92A8`。
开启态：轨道 `#3DD68C`，滑块白色。

#### 5.4.16 进度条

轨道背景 `surface`，填充 accent 渐变，高度 GAP_SMALL × 1.5，圆角 = 高度 / 2。

#### 5.4.17 Tab 切换

高度 66px（基准 LP_ROW_H × 1.5）。当前项：文字 accent，底部 2px accent 下划线。非当前：文字 `#8B92A8`。

#### 5.4.18 步骤指示器

已完成：accent 圆点 + 内部 check 图标。当前：accent 圆点 + pulse 动画 + glow。未完成：边框圆点，`#6B7280`。连接线 2px，已完成段 accent，未完成段 border。

#### 5.4.19 加载指示器

脉冲圆点：3 个圆点，accent 色，依次脉冲 1.2s 循环。旋转圆环：2px 描边，accent 色，0.8s 线性旋转。

#### 5.4.20 骨架屏

背景 `raised`，shimmer 动画 1.5s 循环。

#### 5.4.21 Toast 通知

右下角，slide-in + fade-in 200ms ease_out。堆叠间距 44px。背景 `raised`，阴影 shadow_md。

#### 5.4.22 空状态

SVG 图标 ICON_LARGE + text_tertiary，居中。文案 body + text_tertiary，居中。最大宽度 窗口宽 × 19%。具体图标：Chat 空用 garlic_48 + lobster_32 并排；其他场景用对应 Lucide 图标（layers/clock/puzzle/search/file-text）。
...
```
1. <defs>        — 渐变、filter、clipPath、animation
2. bg rect       — 全屏背景 #0F1117
3. Header        — 标题栏（41px）
4. Left NAV      — 左侧导航栏（82px宽）
5. Content       — 主内容区（panels / cards）
6. ctrl_bar      — Chat 输入控件条（31px）
7. chat_input    — Chat 输入框（62px）
8. 浮层/弹窗     — Modal / Tooltip / Toast
9. 版本标记      — 仅 App 界面（右下角 x=1172, y=656）
```

**group 组织原则：** 按功能模块用 `<g>` 分组，transform 统一加在 group 上。复用 defs 中的渐变/filter。透明度统一用小数（`fill-opacity="0.03"`），不用百分号。

#### 5.4.24 绘图流程

1. 读 Design.md 对应 UI 章节 → 理解布局结构和坐标表
2. 确认界面类型：App 界面（有内容面板）还是功能页（启动页/检查页）
3. 确定 viewBox=`0 0 1280 800`，建立 SVG 骨架
4. 按分层顺序：bg → Header → Left NAV → Content → ctrl_bar/chat_input → 浮层
5. 逐层填入元素，用 `<g>` 按功能组织
6. 参照 §6 控件库验算每个控件的色值、尺寸、圆角
7. App 界面加版本标记；非 App 界面不加
8. 自我检查：量一遍关键坐标是否与 Design.md 表格一致

#### 5.4.25 常见错误

| 错误 | 正确做法 |
|------|---------|
| 用 emoji 代替大青虫子 SVG | 用 §5.4.4 规定的 SVG 路径 |
| 状态文字用 OK/Err/N/A | 用 Pass / Fail / Unknown |
| Badge 宽度用 72px | 用 80px |
| 窗口控制只用 2 个 | 用 3 个：`[—][□][✕]` |
| Header 高度用 30px | 用 41px |
| 按钮高度写 36px | 用 40px（Primary 最小值） |
| 面板边框透明度写 4% | 用 3%（opacity 0.03） |
| 版本标记 opacity 写 30% | 用 0.4（40%），小数不用百分号 |
| 面板 stroke-width 全用 1 | 嵌套面板用 1.5 |
| chat_input 只画一个 rect | 需分 ctrl_bar + 芯片行 + 分隔线 + 文本区 |
...
| Cron 空 | `clock` (ICON_LARGE, text_tertiary) | "暂无任务 — 点击 + 创建" | [+按钮] |
...
> **代码额外检测项（不计入 8 项，不显示在 UI）：** Config Directory（配置目录可写性）、Workspace（工作区目录可写性）。

---

**布局（1280×800 比例图，以主窗口比例为值）：**

```
┌──────────────────────────────────────────────────────────────────┐
│  🐛 AnyClaw  │                                    │  [—][□][✕] │  ← Win32 TITLE_BAR=48px（原生窗口标题栏，非LVGL控件）
├──────────────────────────────────────────────────────────────────┤
│                                                              │
│                         检测项 8 项                             │
│                                                              │
...
│  │  ● OpenClaw      v2.1.0                ● OK          │   │  ← 8行总高=348px, 居中
...
> **说明：** UI-01 Boot Check 页面为全屏 overlay（`g_loading_overlay`），覆盖在原生 Win32 窗口之上，无独立 LVGL 标题栏控件。窗口拖拽由 main.cpp 的 Win32 `WM_NCHITTEST` 处理（`TITLE_BAR_HEIGHT=48`）。内容区从 y=0 开始。
...
| 区域 | 坐标 | 尺寸 | 说明 |
|------|------|------|------|
| HEADER | x=0, y=0, w=1280, h=41 | 41px（5.1%） | 标题栏，含 logo + 窗口控制按钮 |
| CONTENT | x=0, y=41, w=1280, h=614 | 614px | 检测项列表区，LV_PCT(86) |
| FOOTER | x=0, y=655, w=1280, h=145 | 145px | 按钮栏，[向导修复] + [进入界面] |

**检测项列表（CONTENT 内，panel 宽 1100px 水平居中）：**
...
1. **顶部栏（HEADER，41px，占窗口高 5.1%）：** 最上方一条细横条区域。左侧显示品牌图标 + "AnyClaw"，右侧显示窗口控制按钮。底部有一条分隔线（1px 宽）将标题栏与下方内容区分开。

2. **内容区（y=41~655，高 614px）：** 内容区内部没有任何背景色或边框，内部垂直居中放置检测列表 panel，panel 本身相对内容区水平居中（两边各 7% margin）。

3. **检测列表 panel（宽 LV_PCT(86%)=1100px，高 500px，y=50）：** panel 是一个带背景色和圆角的矩形容器。宽度为窗口宽度的 86%，panel 内部有 24px 的均匀内边距（上右下左四边相同）。panel 内部从上到下排列 8 行检测项，第 1 行距 panel 顶部 24px，最后一行距 panel 底部 24px。每行高度 40px，行与行之间有 4px 垂直间距（gap），因此 8 行总高为 8×40 + 7×4 = 348px，加上上下各 24px 内边距 = 396px，panel 总高 500px，内含 104px 额外垂直空间使内容不拥挤。

4. **Footer 按钮区（y=655~800，高 145px）：** 位于 panel 下方，按钮区内部水平排列两个按钮：左侧"向导"按钮靠左边缘，右侧"下一步"按钮靠右边缘，两按钮之间留白。按钮垂直居中于 Footer 区域内。

**三列布局（每行内部从左到右 3 列）：**

- **列1 - 检查项名称（固定 232px 宽）：** 位于每行最左侧，显示检测项的名称文字（如 "Node.js"），左对齐。
- **列2 - 版本/说明（固定 268px 宽）：** 位于列1右侧，显示该项的详细状态或版本号（如 "v22.14.0"），左对齐。
...
- **UI-01（BootCheck）：** 非阻断，显示 10 项环境检测结果，有 Fail 也可进入主界面（受限）或向导
...
**SVG 线框图：** `ui/matcha_v1/UI-04-Step0-lang.svg`
...
1. **标题区：** 内容区顶部居中放置两行文字。上行为中文标题「请选择你的语言」（18px，粗体），下行为英文副标题「Please select your language」（14px，text_secondary 颜色）。标题区下方有 24px 垂直间距。

2. **选项按钮区：** 标题区下方水平排列两个大按钮（语言选项卡片），两个按钮之间有 24px 水平间距，两按钮在内容区内部水平居中。每个按钮 200px 宽 × 140px 高，圆角 12px。按钮内部垂直居中排列：上方为语言图标（🇨🇳 或 🇺🇸，28px），中间为语言名称（「中文」或「English」），下方为英文副标签（「简体中文」或「English」）。选中卡片右上角显示绿色圆圈+✓图标。三种状态：默认（border 1px accent@15%）、hover（border 1px accent@30%）、选中（border 2px accent + 右上角✓）。

3. **提示区：** 选项按钮区下方居中显示一行提示。默认状态：「○ 请选择一项以继续」（text_secondary 色）。选中后：「● 已选择: 中文」或「● 已选择: English」（accent 色，左侧实心圆点）。

**Footer 按钮状态：**
- [退出向导]：左侧，secondary 样式，始终可用
- [上一步]：Step0 禁用态（透明度 50%，文字变暗），不可点击
- [下一步]：accent 样式，选中语言后可用
...
│                                                                              │
│                         🔧 检测与安装                                         │
│            运行环境检测 / OpenClaw · Hermes · Claude Code 安装                │
│  ──────────────────────────────────────────────────────────────────────────  │
│  ┌─────────────────────────────┐    ┌────────────────────────────────────┐ │
│  │  检测结果                    │    │  📺 检测与安装日志                    │ │
│  │                             │    │                                    │ │
│  │  ● Network    google.com OK Pass│  > 正在检测 Network...             │ │
│  │  ● Node.js    v22.14.0 Pass│  │  > Network OK ✓                    │ │
│  │  ● npm         v10.8.0  Pass│  │  > 正在检测 Node.js...              │ │
│  │  ● OpenClaw   v1.2.0   Pass│  │  > Node.js 已就绪 ✓                 │ │
│  │  ● Hermes     未安装     Warn │  │  > 正在检测 npm...                  │ │
│  │  ● Claude     未安装     Warn │  │  > npm 就绪 ✓                       │ │
│  │  ● Config Dir 可写       Pass│  │  > Config Dir OK ✓                │ │
│  │  ● 磁盘空间   85GB       Pass │  │  > 磁盘空间充足 ✓                  │ │
│  └─────────────────────────────┘    │  > 全部检测完成                     │ │
│                                      └────────────────────────────────────┘ │
│                                                                              │
│           [ 从网络下载安装（推荐）]                                             │
...
- 面板高度: h=374，末行y=428，距底部536px
- 安装按钮区: y=552，按钮h=40，距底部655px
...
[进入 Step 1]
       │
       ▼
    自动开始 8 项并行检测（Network / Node.js / npm / OpenClaw / Hermes / Claude / Config Dir / Disk Space）
       │
       ▼
    ┌─────────────────────────┐
    │  检测结果实时更新到左面板  │
...
1. **标题区：** 内容区顶部水平居中放置标题「🔧 检测与安装」（18px 粗体）和副标题「运行环境检测 / OpenClaw · Hermes · Claude Code 安装」（13px，text_secondary 颜色）。标题区下方有分隔线。

2. **双面板区：** 标题区下方左右并排两个矩形容器面板，中间有 16px 间距，两个面板总宽度占内容区宽度的 **86%**（LV_PCT(86)），在内容区内部水平居中。
   - **左面板（检测结果）：** 宽度约为双面板总宽的 **40%**（~410px），内部从上到下排列 8 个检测项状态行。每行包含：状态图标（✅/⏳/⚠️/❌）+ 检测项名称 + 版本号或状态文字。检测项顺序：Network / Node.js / npm / OpenClaw / Hermes / Claude / Config Dir / Disk Space。检测项之间垂直间距 36px（内容区 6%）。
   - **右面板（检测与安装日志）：** 宽度约为双面板总宽的 **58%**（~594px），内部为一个终端风格日志展示区（monospace 字体，surface 背景色），实时显示检测和安装过程的命令行输出。日志自动滚动到底部，支持选中复制。
...
│  进阶能力（非阻断，可后续在设置中配置）                                       │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐                   │
│  │ 💬 飞书 Bot   │  │ 🖥️ 桌面控制  │  │ 🔊 语音交互  │   [x=90,w=1100]  │
│  └──────────────┘  └──────────────┘  └──────────────┘                   │
├──────────────────────────────────────────────────────────────────────────────┤
│  [ 退出向导 ]                                            [ 开始使用 ]      │
└──────────────────────────────────────────────────────────────────────────────┘
...
[进入 Step 3]
    ...
    └───────────────┬────────────────────┘
                    │
         用户点击 [开始使用]（Footer 右下角）
                    │
                    ▼
    保存配置（语言 + 昵称 + 时区 + Leader 模式 + wizard_completed=true）
                    │
                    ▼
             关闭向导 → 进入主界面（Bot · Chat 模式）
```

#### 文字描述

**StepBar（y=41~89）：** 4 个居中圆点（cx=280/520/760/1000），圆点之间用细线连接（已完成段绿色 #3DD68C，未完成段灰色 #252A38）。已完成步骤圆点内显示 ✓，当前步骤（完成）圆点用橙色 #FF7F50 高亮（实心圆 + 外圈）。StepBar 背景色 #0F1117 与内容区一致。

**内容区（y=89~655）：** 从上到下垂直排列，所有控件左右边界对齐 x=90、宽度 1100：
1. **标题区：** 居中标题「🎉 恭喜！设置完成」（20px 粗体）+ 副标题「最后一步：确认配置并进入 AnyClaw」（13px，text_secondary）。副标题下方有分隔横线（y=170）。
2. **昵称：** 标签「昵称」（11px），下方输入框（高 40px，rx=8），placeholder 为「你的昵称...」。
3. **时区：** 标签「时区」（11px），下方下拉框（高 40px），默认「Asia/Shanghai」，右侧 ▼ 指示器。
4. **Leader 模式：** 标签「Leader 模式」（11px），下方两个垂直排列的单选选项卡片（各高 44px）：
   - 选项1：橙色边框背景（选中态），左侧 ◉ Radio，内文字「启用 Leader 模式（多 Agent 并行，推荐）」，右侧「推荐」标签。
   - 选项2：深色背景，左侧 ○ Radio，内文字「跳过（单 Agent，仅 OpenClaw）」。
   - 选项下方提示文字「Skip 将降级为单 Agent 模式（仅 OpenClaw）。」
5. **进阶能力：** 标签「进阶能力（非阻断，可后续在设置中配置）」（10px，居中），下方三个能力卡片横向排列（各宽 300px、高 50px）：「💬 飞书 Bot」「🖥️ 桌面控制」「🔊 语音交互」。

**Footer（y=655~800）：** 背景色 #13151E，无分隔横条。底部左下角「退出向导」按钮（透明边框风格），右下角「开始使用」按钮（accent 实心填充）。
...
> 父容器：窗口。坐标：x=0，y=TITLE_H=41px，w=44px（固定值，不缩放），h=PANEL_H=759px。
> icon-only，hover 300ms 显示 tooltip。
> **v2.2.12 更新：** 新增 KB（知识库）和 Skill 到顶部模块区，共 6 个图标，分两区排列（nav_top 4个 + nav_spacer + nav_bot 2个）。
...
│  │px  │    │    │  内容区 (y=41, h=759)                                  ││
│  │    │    │    │  ┌─────────────────────┬──────────────────────────────┐ ││
│  │    │    │    │  │ left_panel (276px)   │ right_panel (898px)    │ ││
│  │    │    │    │  └─────────────────────┴──────────────────────────────┘ ││
│  └────┴────┴───────────────────────────────────────────────────────┘│
└─────────────────────────────────────────────────────────────────────┘
```

左导航栏内部结构（x=0, y=41, w=44px, h=759px，flex COLUMN 布局）：

**分层结构：**

- **left_nav 容器**（x=0, y=41, w=44px, h=759px）：导航栏根容器，flex COLUMN 布局，内边距 0，背景 panel 色。
- **nav_top 子容器**（flex_grow=0, h 由内容撑起 = 4×36 + 3×gap=12×3 = 180px）：flex COLUMN + START 排列，包含 Bot / Workspace / KB / Skill 四个模块图标按钮，之间各隔 gap=12px。
- **nav_spacer 透明填充**（flex_grow=1）：占据 nav_top 底部到 nav_bot 顶部之间所有剩余空间，使 nav_top 贴顶、nav_bot 贴底。
- **nav_bot 子容器**（flex_grow=0, h 由内容撑起 = 2×36 + 12 = 84px）：flex COLUMN + END 排列，包含 Chat/Work 切换按钮和 Settings 图标，之间隔 gap=12px，容器底部与 left_nav 底部对齐。

**按钮尺寸：** 36×36px，圆角 8px。x 在 nav_top/nav_bot 宽度内居中（flex CROSS_AXIS=CENTER）。

```
┌───────┐
│  🐛   │  ← Bot 图标，36×36，圆角8，y=48（nav_top 顶部）
│───────┤
│  🖥   │  ← Workspace 图标，36×36，圆角8，y=96（gap=12）
│───────┤
│  📚   │  ← KB 图标，36×36，圆角8，y=144（gap=12）
│───────┤
│  🧩   │  ← Skill 图标，36×36，圆角8，y=192（gap=12）
│       │  ← nav_spacer，flex_grow=1，填充剩余全部空间
│       │
│  💬   │  ← Chat/Work，36×36，圆角8，y 靠 nav_bot 顶部
│───────┤
│  ⚙️   │  ← Settings 图标，36×36，圆角8，y 贴底
└───────┘
```
...
[应用启动]
    ...
    创建左导航栏 left_nav (x=0, y=41, w=77, h=759)
       ...
       │                                                        显示 Skill 管理内容
       │
       ├─── 创建 nav_spacer (flex_grow=1) ──→ 仅填充，无内容
       │
       └─── 创建 nav_bot (h=120px, y=680) ──→ 创建 Chat/Work 切换 + Settings
                                        │
...
> ⚠️ 实际代码使用百分比/flex 布局，以下为 1280px 基准屏的典型值。精确值随窗口大小自动缩放。

||| 控件 | 宽 | 高 | x | y | 计算公式 |
|------|---:|---:|---:|---:|---------
| left_nav | 82px | 759 | 0 | 41 | `NAV_W_PCT(6%) × 1280 = 77px` |
| nav_top | 82px | 256 | 0 | 41 | 4×52 + 3×16 = 256px |
| nav_spacer | 82px | flex | 0 | 304 | y = nav_top bottom |
| nav_bot | 82px | 126 | 0 | 680 | 2×52 + 22 = 126px，flex END 贴底 |

**第二层：顶部品牌 Logo**

| 控件 | 尺寸 | x | y | 说明 |
|------|------:|---:|---:|------|
| 品牌 Logo | 36×36 | 居中(23) | 5 | ICON_LARGE，固定显示，不受导航图标影响 |

**第三层：顶部模块按钮区（nav_top，h=256px，y=41）**

| 控件 | 尺寸 | x | y | 说明 |
|------|------:|---:|---:|------|
| Bot 按钮 | 52×52 | 居中(11) | 48 | nav_top 顶部，SVG 青虫子高亮 |
| File 按钮 | 52×52 | 居中(11) | 116 | `y = 48 + 52 + 16 = 116` |
| KB 按钮 | 52×52 | 居中(11) | 184 | `y = 116 + 52 + 16 = 184` |
| Skill 按钮 | 52×52 | 居中(11) | 252 | `y = 184 + 52 + 16 = 252` |
| nav_top 底部边界 | — | — | 297 | `y = 252 + 52 = 297` |

**第四层：中间填充区（nav_spacer，flex_grow=1）**

| 控件 | 尺寸 | x | y | 说明 |
|------|------:|---:|---:|------|
| nav_spacer 容器 | 82×flex | 0 | 304 | flex_grow=1，高度随窗口自动伸缩（典型屏≈222px） |
...
| hover | bg=hover_overlay + tooltip（KB/Skill 尤为重要，图标较小需文字说明） |
...
| 元素 | 说明 |
|------|------|
| 🐛（SVG 青虫子） | 品牌 Logo，固定，36×36 SVG 图形（圆头+触角+腮红+微笑） |
| AnyClaw · Chat | 当前模块名 + 子状态（随左导航切换），13px |
| ─ □ ✕ | 窗口控制按钮，最小化/最大化/关闭 |
...
```
right_panel
├── 输出区（flex-grow，垂直滚动，h=620）  ← y=41，底部=661
├── 控制行（固定高度 31px）              ← y=661，gap=0
└── 输入区（固定高度 62px，三行）        ← y=692，底部=754，跨面板对齐任务容器
```

**Bot 模块通用子元素（LVGL 像素值，基准 1280×800）：**

|| 元素 | 宽 | 高 | x | y | 计算 |
|------|---:|---:|---:|---:|------|
| left_panel | 276 | 759 | 84 | 41 | x = nav_w(77) + 7，左面板与导航栏间距7px |
| right_panel | 898 | 759 | 367 | 41 | x = left_panel右 + 7，y同 |
| 输出区(chat_cont) | 898 | 620 | 367 | 41 | flex-grow 填充，消息列表 |
| 控制行(ctrl_bar) | 898 | 31 | 367 | 661 | y = 输出区底(661)，gap=0 |
| 输入区(chat_input) | 898 | 62 | 367 | 692 | y = 控制行底(692)，gap=0，底部=754 |
...
```
┌────────────────────────────────────────────────────────────────────────────┐
│ HEADER (y=0~41, h=41)  [─][□][✕]  AnyClaw · Workstation               │
├────┬──────────────────────────────┬──────────────────────┬───────────────┤
│NAV │ 左面板 (x=40, w=270)       │ 中部 (x=310, w=570)  │ 右 (x=920,w=360)
│x=0 │ 文件浏览器（可折叠）        │ ┌─ Tab栏 (y=41,h=33)──┐│ AI对话(可折叠)
│w=40│ [AnyClaw ▼][+]            │ │main.cpp│utils.h│🌐feishu│+││ Markdown输出
│    │ D:\workspace\AnyClaw       │ └────────────────────┘│ ctrl_bar(661)
│🤖  │ 🔍 搜索...                │ ┌─ 编辑器 (y=74,h=356)─┐│ chat_input(692)
│🖥  │ src/                      │ │ 1 │ #include...     ││               │
│    │  📄 ui_main.cpp           │ │...│                 ││ ◀折叠         │
│📚  │  📄 boot_check.cpp        │ └────────────────────┘│               │
│🧩  │ docs/                     │ ┌─ 分隔线 (y=430,h=6) ─┐│               │
│    │                          │ │     ⋮                ││               │
│⚙️  │(左下角)                  │ ├─ 终端 (y=436,h=364)─┤│               │
└────┴──────────────────────────┴──────────────────────┴───────────────┘
```
...
| 区域 | x | y | w | h | 说明 |
|------|---|---|---|---|------|
| HEADER | 0 | 0 | 1280 | 41 | logo + 标题 + 窗口控制 |
| NAV | 0 | 41 | 40 | 759 | 左导航，5图标（40px宽，图标28×28） |
| 左面板 | 40 | 41 | 270 | 759 | 项目浏览器（216×1.25） |
| 中部 | 310 | 41 | 570 | 759 | 编辑器+终端 |
| 右面板 | 920 | 41 | 360 | 759 | AI对话 |

**左 NAV（x=0, w=40）：**
5 个图标从上到下：🤖 Bot | 🖥 Workspace(激活,accent左边框) | 📚 KB | 🧩 Skill | ⚙️ Settings(左下角)
图标 28×28，圆角 rx=6，gap=8px，图标按钮 rx=6。
图标 36×36，gap=12px，图标按钮 rx=8。

**左面板 - 项目浏览器（x=44, w=270）：**
| 元素 | x | y | w | h | 说明 |
|------|---|---|---|---|------|
| 面板背景 | 44 | 41 | 270 | 759 | fill=#13151E |
| 标题"文件" | 60 | 62 | - | - | font=11, bold |
| 分隔线 | 44 | 70 | 270 | 1 | |
| 项目下拉 | 52 | 76 | 140 | 24 | rx=4, "AnyClaw"+"▼" |
| 新建[+] | 198 | 76 | 22 | 22 | rx=4 |
| 路径 | 52 | 108 | 250 | 16 | fill=#1A1D27 |
| 搜索框 | 52 | 132 | 250 | 22 | rx=4 |
| 项目标签 | 52 | 170 | - | - | font=9, dim |
| 文件树项 | 52 | 178~282 | 250 | 22 | rx=3, src/ + ui_main.cpp(激活) + boot_check.cpp + docs/ + ui/ |
| 折叠按钮 | 314 | 360 | 14 | 40 | ◀, 在中部门口 |

**中部（x=314, w=606）：**

*上部 - 文件编辑区（y=41~430, h=389）：*
| 元素 | x | y | w | h | 说明 |
|------|---|---|---|---|------|
| Tab栏背景 | 314 | 41 | 606 | 33 | fill=#0A0C10 |
| Tab: main.cpp | 314 | 41 | 100 | 33 | rx=0, fill=#13151E |
| ✕ | 406 | 50 | 8 | 14 | |
| Tab: utils.h | 414 | 41 | 80 | 33 | |
| ✕ | 486 | 50 | 8 | 14 | |
| Tab: 🌐 feishu | 494 | 41 | 100 | 33 | |
| ✕ | 586 | 50 | 8 | 14 | |
| +新建 | 602 | 48 | 24 | 20 | |
| 编辑区背景 | 314 | 74 | 606 | 356 | fill=#0F1117 |
| 行号栏 | 314 | 74 | 36 | 356 | fill=#0A0C10 |
| 行号 | 328 | 98~278 | - | - | monospace 11, dim色 |
| 代码 | 358 | 98~258 | - | - | syntax色 |
| 滚动条 | 900 | 74 | 20 | 356 | fill=#0A0C10 |

*分隔线（y=430, h=6）：*
| 元素 | x | y | w | h | 说明 |
|------|---|---|---|---|------|
| 分隔背景 | 314 | 430 | 606 | 6 | fill=#0A0C10 |
| 分隔线 | 314 | 430 | 606 | 2 | fill=#252A38 |
| 手柄 | 602 | 430 | 20 | 6 | fill=#1A1D27, ⋮ |

*下部 - 终端区（y=436~800, h=364）：*
| 元素 | x | y | w | h | 说明 |
|------|---|---|---|---|------|
| Tab栏背景 | 314 | 436 | 606 | 28 | fill=#0D1117 |
| 标签"终端" | 326 | 453 | - | - | font=10, dim |
| 终端1 Tab | 358 | 439 | 70 | 22 | fill=#13151E, 激活有底部accent线 |
| ✕ | 420 | 447 | 8 | 14 | |
| 终端2 Tab | 428 | 439 | 70 | 22 | fill=#1E2230 |
| ✕ | 490 | 447 | 8 | 14 | |
| +新建 | 504 | 441 | 22 | 18 | |
| 终端内容 | 314 | 464 | 606 | 336 | fill=#0D1117 |

**右面板（x=920, w=360）：**
| 元素 | x | y | w | h | 说明 |
|------|---|---|---|---|------|
| 面板背景 | 920 | 41 | 360 | 759 | fill=#0F1117 |
| AI消息(markdown) | 932 | 58 | 336 | 160 | fill=#1E2230, accent左边框3px |
| 用户消息气泡 | 932 | 228 | 280 | 40 | fill=#3B82F6, rx=16 |
| AI消息2 | 932 | 278 | 336 | 80 | accent左边框3px |
| 思考状态 | 932 | 368 | 336 | 28 | fill=#1A1D27, 圆点loading |
| ctrl_bar | 920 | 661 | 360 | 31 | fill=#1A1D27, 5控件 |
| chat_input | 920 | 692 | 360 | 62 | fill=#1A1D27 |
| 折叠按钮 | 920 | 360 | 14 | 40 | ◀, 在右部门口 |

**ctrl_bar 控件（y=666, h=22, 间距74px）：**
| 控件 | x | w | 样式 |
|------|---|---|------|
| Agent ▼ | 927 | 69 | accent选中, 文字绿色 |
| AI行为 ▼ | 1001 | 69 | 默认 |
| AI托管 | 1075 | 69 | 灰色圆点 |
| Voice | 1149 | 69 | 灰色圆点 |
| 字数统计 | 1268 | - | 右对齐, dim |

**chat_input（y=697~749）：**
| 元素 | x | y | w | h | 说明 |
|------|---|---|---|---|------|
| 输入框 | 927 | 697 | 315 | 52 | fill=#13151E |
| 附件芯片1 | 933 | 702 | 148 | 16 | |
| 附件芯片2 | 1087 | 702 | 100 | 16 | |
| 分隔线 | 927 | 722 | 315 | 1 | |
| 文字输入提示 | 935 | 738 | - | - | "输入消息..." |
| 附件+ | 1213 | 733 | 21 | 21 | |
| 发送按钮 | 1257 | 742 | r=18 | - | accent圆形 |

**折叠行为：**
- 左面板折叠：点击 ◀(x=280) → 左面板 w=0，Center w=280+640=920，Right x=920
- 右面板折叠：点击 ◀(x=920) → 右面板 w=0，Center w=640+360=1000，Right 消失
- 中部分隔线拖拽：上下比例可调，默认 editor h=356, terminal h=364
...
- 菜单字体 `-24`（24pt），菜单项高度 `48px`（放大 2 倍旧版 `24px`）
- LED 指示灯半径 `8px`（放大 2 倍旧版 `4px`）
...
**第一层：窗口骨架（设计基准 1280×800）**

```
┌──────────────────────────────────────────────────────────────────────────────────────────┐
│  标题栏  y=0, h=41   🦞 AnyClaw · Chat                              [─]  [□]  [✕]      │
├──────┬───────────────────────────────────────────────────────────────────────────────────┤
│ 导航  │  左面板 x=84, y=41, w=276, h=759   │  右面板 x=367, y=41, w=898, h=759          │
│x=0   │                                │                                                    │
│y=41  │                                │                                                    │
│w=77  │                                │                                                    │
│h=759 │                                │                                                    │
...
**第二层：左面板内部（x=84, y=41, w=276, h=759）从上到下：**

```
│  ┌─ AI伙伴容器 x=92, y=58, w=256, h=203 ─────────────────────────────────────┐  │
│  │  🦞 AnyClaw        [🔊 Voice ON]      [Agent Status ● Running]           │  │
│  │     你的 AI 伙伴                          Model: MiniMax-M2.7                │  │
│  └───────────────────────────────────────────────────────────────────────────────┘  │
│  ── 分隔线 x=92, y=272, w=260 ──────────────────────────────────────────────────   │
│  ┌─ 任务列表 x=92, y=286 ───────────────────────────────────────────────────────┐  │
│  │ 任务列表  (0)                                                                 │  │
│  └───────────────────────────────────────────────────────────────────────────────┘  │
│  ┌─ Session Card x=92, y=298, w=256, h=190 ──────────────────────────────────────┐  │
│  │ 会话                      (0)                                                  │  │
│  │───────────────────────────────                                                 │  │
│  │ [空状态占位]                                                                         │  │
│  └───────────────────────────────────────────────────────────────────────────────┘  │
│  ┌─ 任务 Card x=92, y=504, w=256, h=126 ────────────────────────────────────────┐  │
│  │ 所有任务                  (0)                                                  │  │
│  │───────────────────────────────                                                 │  │
│  │ [空状态占位]                                                                         │  │
│  └───────────────────────────────────────────────────────────────────────────────┘  │
...
**第三层：右面板内部（x=367, y=41, w=898, h=759）从上到下：**

```
│  ┌─ chat_cont（消息区，flex-grow，h=620）─────────────────────────────────────┐  │
│  │                                                                               │  │
│  │  👤  帮我查一下今天的天气                        12:28  → │              │  │
│  │       右侧头像靠右边界，头像右边界=1183，气泡左置，气泡/头像间隙8px  │  │
│  │                                                                               │  │
│  │  🦞 AI消息气泡（完整结构）                                                    │  │
...
│  │                                                                               │  │
│  └───────────────────────────────────────────────────────────────────────────────┘  │
│  ┌─ ctrl_bar（控制栏，y=661, h=31）──────────────────────────────────────────────┐  │
│  │  [Agent ▼]  [AI行为 ▼]  [AI托管 ○]  [Voice ○]                     0/2000    │  │  ← 字数统计在右上角
│  └───────────────────────────────────────────────────────────────────────────────┘  │
│  ┌─ chat_input（输入框，y=692, h=62）────────────────────────────────────────────┐  │
│  │  ┌─ 输入框内部 y=700 ───────────────────────────────────────────────────┐  │  │
│  │  │  [📄 LICENSES.chromium.html ✕] [📄 report.pdf ✕]                │  │  ← 第一行：附件芯片
│  │  │  ─────────────────────────────────────────────────────────────────    │  │  │
│  │  │  输入消息...                                                   [+] [▶]  │  │  │  ← 第二行起：用户输入
│  │  │  用户输入从此开始                                                    │  │  │
│  │  └────────────────────────────────────────────────────────────────────────┘  │  │
...
**第四层：左导航内部（x=0, y=41, w=77, h=759）从上到下：**

```
│  ┌──┐ │  ← Bot 图标，52×52px，圆角9px，x=11(居中), y=48（与左面板顶部对齐）🦞SVG
│  │🦞│ │
│  └──┘ │
│  ┌──┐ │  ← File 图标，52×52px，圆角9px，x=11, y=116，📁
│  │📁│ │
│  └──┘ │
│  ┌──┐ │  ← KB 图标，52×52px，圆角9px，x=11, y=184，📚
│  │📚│ │
│  └──┘ │
│  ┌──┐ │  ← Skill 图标，52×52px，圆角9px，x=11, y=252，🧩
│  │🧩│ │
│  └──┘ │
│ (flex)│  ← nav_top，flex_grow=1，填充中部按钮与底部按钮之间
│       │
│  ┌──┐ │  ← Chat/Work 切换，52×52px，圆角9px，x=11, y=696，💬
│  │💬│ │
│  └──┘ │
│  ┌──┐ │  ← Settings 图标，52×52px，圆角9px，x=11, y=764（与左面板底部对齐）⚙️
│  │⚙️│ │
│  └──┘ │
```
...
|------|--------|------|
| 窗口 | 1280×800 | viewBox |
| 标题栏 | y=0, h=41 | 底部线条分隔，左侧 SVG 青虫子 + "AnyClaw · Chat" |
| 左导航 | x=0, y=41, w=77, h=759 | Bot 高亮（SVG 青虫子），6个图标，上下部对齐面板 |
| 左面板 | x=84, y=41, w=276, h=759 | 左侧面板主体 |
| AI伙伴容器 | x=94, y=58, w=256, h=203 | 顶部 SVG 青虫子 + Voice开关 + 右侧 Agent Status + Model |
| Agent Status | x=94, y=240, w=256, h=20 | 绿色LED + Running，紧贴容器底部 |
| Model | x=92, y=258, w=256, h=16 | "Model: MiniMax-M2.7"，紧贴 Agent Status 下方 |
| Session Card | x=94, y=298, w=256, h=190 | 会话列表，距 AI伙伴容器 10px |
| 任务 Card | x=94, y=504, w=256, h=190 | 所有任务，距 Session Card 16px，距面板底部 16px |
| 右面板 | x=367, y=41, w=898, h=759 | 消息区+控制行+输入框 |
| chat_cont | x=367, y=41, w=898, h=620 | flex-grow 填充，消息列表 |
| ctrl_bar | x=367, y=661, h=31 | Agent/AI行为/AI托管/Voice 四个控件，字数统计右上角 |
| chat_input | x=367, y=692, h=62 | 输入框内：芯片行(第一行)+文本区(第二行起)，底部=754 |
| 附件芯片行 | x=380, y=700, h=16 | 输入框内第一行，横排文件芯片+删除X+滚动按钮 |
| 用户气泡 | transform x=980, w=139 | 蓝色渐变，右置，气泡右边界距头像左边界 12px |
| 用户头像 | cx=171, r=14 | 头像右边界=1159，距面板右边界 24px |
| AI头像 | cx=17 | 气泡左侧，间隙21px，SVG 青虫子 |
| 上传按钮 | x=1140, y=738, 21×21 | +号，输入框右下角 |
| 发送按钮 | cx=1190, cy=742, r=18 | 圆形，绿色渐变，输入框右下角 |
...
[用户点击左导航 Bot，进入 Chat 模式]
    ...
    显示 Bot · Chat 界面：左导航 Bot 高亮 + 左面板状态行 + 右面板输出区
       ...
       │
       ├─── 点击 Agent ▼ ──→ 选择 Agent 下拉菜单
       │
       ├─── 点击 AI行为 ▼ ──→ 选择 AI行为 下拉菜单（自主/Ask/Plan）
       │
       ├─── 点击 AI托管 ○ ──→ 切换 AI托管 开关（默认关闭）
       │
       ├─── 点击 Voice ○ ──→ 切换 Voice 开关（默认关闭）
       │
       └─── 点击左导航 Chat/Work 切换 ──→ Chat ↔ Work 模式切换
```
...
| 控件 | 宽 | 高 | x | y | 计算公式 |
|------|---:|---:|---:|---:|---------|
| 窗口 | 1280 | 800 | 0 | 0 | WIN_DEFAULT_W × WIN_DEFAULT_H |
| 标题栏 | 1280 | 41 | 0 | 0 | TITLE_H_PCT(6%) × 800 ≈ 41px |
| 左导航 | 77 | 759 | 0 | 41 | NAV_W_PCT(6%) × 1280 ≈ 77px |
| 间距 | 7 | — | 82 | — | PANEL_GAP = 7px |
| 左侧面板 | 276 | 759 | 84 | 41 | w = 1280 - 77 - 7 - 898 |
| 右侧面板 | 898 | 759 | 367 | 41 | 1280 - 77 - 7 - 898 |

**第二层：标题栏内部（y=0，h=41）**

| 控件 | 尺寸 | x | y | 说明 |
|------|------:|---:|---:|------|
| 品牌 SVG 图标 | 36×36 | 8 | 5 | 垂直居中，SVG 青虫子（圆头+触角+腮红+微笑） |
| 模块名文字 | ~180宽 | 44 | 23 | "AnyClaw · Chat"，text-primary色，13px |
| 最小化按钮 | 30×17 | 1085 | 12 | ─，text-dim色，圆角4px |
| 最大化按钮 | 30×17 | 1106 | 12 | □，text-dim色，圆角4px，浅绿边框 |
| 关闭按钮 | 30×17 | 1142 | 12 | ✕，accent色，圆角4px，透明绿底 |

**第三层：左导航内部（x=0, y=41, w=77, h=759）**

> 导航栏宽度 77px，图标 52×52px，水平居中（x=11）。图标间间距 16px。

| 控件 | 尺寸 | x | y | 说明 |
|------|------:|---:|---:|------|
| Bot 图标 | 52×52 | 11 | 48 | 圆角9px，高亮背景#252A38，SVG 青虫子（圆头+身体+触角+腮红+微笑） |
| File 图标 | 52×52 | 11 | 116 | 圆角9px，透明背景，📁 |
| KB 图标 | 52×52 | 11 | 184 | 圆角9px，透明背景，📚 |
| Skill 图标 | 52×52 | 11 | 252 | 圆角9px，透明背景，🧩 |
| Chat/Work 图标 | 52×52 | 11 | 696 | 圆角9px，高亮背景#252A38，💬 |
| Settings 图标 | 52×52 | 11 | 764 | 圆角9px，透明背景，⚙️ |

**第四层：左侧面板（x=84, y=41, w=276, h=759）**

| 控件 | 尺寸 | x | y | 说明 |
|------|------:|---:|---:|------|
| 面板容器 | 276×759 | 84 | 41 | 圆角10px，surface 背景，浅绿边框1px(opacity=0.03) |
| AI伙伴容器 | 256×203 | 94 | 58 | 圆角10px，边框1.5px accent，SVG 青虫子 + 文字 + 状态行 |
| Agent Status | 256×20 | 92 | 240 | 绿色LED + "Running"，紧贴容器底部内侧 |
| Model | 256×16 | 92 | 258 | "Model: MiniMax-M2.7"，紧贴 Status 下方 |
| 分隔线 | 256×1 | 92 | 272 | accent色4%opacity，AI伙伴容器与列表分隔 |
| 任务列表标题 | auto | 92 | 286 | "任务列表 (0)"，text-primary + text-dim |
| Session Card | 256×190 | 92 | 298 | 圆角7px，表面背景，内部分隔 |
| 任务 Card | 256×126 | 92 | 504 | "所有任务 (0)"，距 Session Card 16px，距面板底 16px |

**AI伙伴容器内部（x=92, y=58, w=256, h=203）：**

| 控件 | 尺寸 | x | y | 说明 |
|------|------:|---:|---:|------|
| 容器背景 | 256×203 | 92 | 58 | 圆角10px，accent边框1.5px |
| SVG 青虫子 | 56×70 | 100 | 65 | 中心 cx=128 cy=110，大圆头r28+小圆身+触角+腮红+微笑 |
| 名字标签 | auto | 200 | 102 | "AnyClaw"，accent色 bold 12px |
| 描述标签 | auto | 200 | 118 | "你的 AI 伙伴"，text-dim色 10px |
| Voice 开关 | 72×24 | 270 | 88 | ON 状态：accent边框+accent文字"Voice"+喇叭图标+滑块在右侧 |
| Agent Status | 256×20 | 92 | 240 | LED + "Agent Status" + "Running"，accent色 LED |
| Model | 256×16 | 92 | 258 | "Model: MiniMax-M2.7"，text-dim色 |

**SVG 青虫子图形规范（AI伙伴/NAV/消息区头像统一）：**

```
构成：圆头 + 触角（2根短线+圆点）+ 眼睛（椭圆黑瞳+白点高光）+ 腮红（粉色椭圆）+ 微笑 + （全身版含身体+投影）
...
| 眼睛 | #0F1117 | 2个椭圆黑瞳 |
| 高光 | #FFFFFF | 黑瞳内小白点 |
| 腮红 | #FF9EB5 | 2个粉色椭圆，opacity=0.7 |
| 微笑 | #0F1117 | 圆头底部贝塞尔曲线 |
| 身体（全身版）| #3DD68C | 小圆角矩形 |
| 投影 | #000000 | 身体下方椭圆，opacity=0.2 |
```

**第五层：右面板（x=367, y=41, w=898, h=759）**

内部分为三个垂直区域：

**5.1 消息区（chat_cont，y=41，高=620，flex-grow）：**

| 控件 | 尺寸 | x | y | 说明 |
|------|------:|---:|---:|------|
| 消息列表 | 898×620 | 367 | 41 | clipPath 裁剪，flex-grow 填充 |
| 用户气泡 | transform | 980 | 71 | 蓝色渐变背景，右侧头像，左上角时间戳 |
| AI气泡 | 最大431宽 | 403 | 随内容 | 浅灰背景，左侧绿色竖线，SVG 青虫子头像 |

**AI气泡内部结构：**

| 区域 | 内容 | 条件 |
|------|------|------|
| AI头像 | 28px圆形，SVG 青虫子 | 始终显示 |
| 时间戳 | "12:28"，左上 | 始终显示 |
| AI正在思考标题栏 | "AI 正在思考..." + 黄点动画 | 思考/输出中显示动画，已完成无动画 |
| 思考内容 | "思考过程"（可折叠） | 始终显示 |
| 终端区 | $ curl ... + JSON输出 | 存在终端输出时显示 |
| 文件链接 | filename.pdf + 点击打开文件 | 存在文件输出时显示 |
| 结论文本 | 最终回复 | 始终显示 |
...
| 区域 | 内容 |
|------|------|
| 时间戳 | "12:28"，在气泡内右上角，与气泡右边界对齐 |
| 气泡 | 蓝色渐变背景，右侧头像，左上角内容 |
| 文本 | "帮我查一下今天的天气"，白色 |

**5.2 控制栏（ctrl_bar，y=661，高=31）：**

| 控件 | 尺寸 | x | y | 说明 |
|------|------:|---:|---:|------|
| 控制栏容器 | 898×31 | 367 | 661 | 圆角7px，surface 背景 |
| Agent 下拉 | 69×22 | 374 | 662 | 圆角11px，accent透明底，文字 accent色，"Agent ▼"，text-anchor 居中 |
| AI行为 下拉 | 69×22 | 448 | 662 | 圆角11px，深灰底，文字 text-primary，"AI行为 ▼"，text-anchor 居中 |
| AI托管 开关 | 69×22 | 522 | 662 | 圆角11px，左侧灰圆滑块，"AI托管"，text-anchor 居中，文字灰色（关闭态） |
| Voice 开关 | 69×22 | 596 | 662 | 圆角11px，左侧灰圆滑块，"Voice"，text-anchor 居中，文字灰色（关闭态） |
| 字数统计 | auto | 1137 | 680 | "0/2000"，text-dim色，ctrl_bar 右上角 |

> **控件间距（x轴）：** Agent(374)→AI行为(448) gap=74，AI行为→AI托管(522) gap=3，AI托管→Voice(596) gap=74，Voice→（边距）gap=269。
>
> **AI托管/Voice 为开关样式**（默认关闭，灰色圆圈滑块在左侧），尺寸统一为 69×22。

**5.3 输入区（chat_input，y=692，高=62）：**

输入框内部署附件芯片行为第一行，用户输入从第二行开始。整体高度固定 62px，底部与任务容器底部对齐（y=754）。

**输入框内部结构（y=700，高=54）：**

| 控件 | 尺寸 | x | y | 说明 |
|------|------:|---:|---:|------|
| 输入框容器 | 752×57 | 372 | 700 | 圆角5px，深灰背景，浅绿边框(opacity=0.06) |
| 附件芯片行 | 752×20 | 378 | 702 | 第一行，粘贴文件后自动生成，横排显示 |
| 分隔线 | 752×1 | 372 | 720 | 芯片行与文本输入区分隔，浅绿6%opacity |
| 文本输入区 | — | 380 | 724 | 第二行起，光标默认在此位置 |
| 上传按钮 | 21×21 | 1140 | 738 | 圆角5px，深灰背景，+号，输入框右下角 |
| 发送按钮 | r=18 | 1190 | 742 | 圆形，绿色渐变，白色 ▶，输入框右下角 |

**附件芯片行内部（y=702，高=16）：**

|| 控件 | 尺寸 | x | y | 说明 |
|------|------:|---:|---:|------|
| 文件芯片 | 动态 | 378 | 702 | 横排，圆角3px，深灰背景，左📄+文件名+右删除✕ |
| 删除按钮 | ⌀10 | 芯片右-5 | 710 | 芯片右侧圆形X按钮，点击移除该附件 |
| 横向滚动 | 仅溢出显示 | — | — | 附件超出宽度时，左右两侧显示 `<` `>` 滚动按钮 |

> 粘贴/拖拽文件进入输入框 → 自动生成芯片插入第一行，光标自动跳到第二行。支持 Ctrl+V 粘贴文件。

**相邻关系：**
- 标题栏 → 内容区（y=41，无缝衔接）
- 左导航 → 间距(7px) → 左侧面板（x=84）
- chat_cont → ctrl_bar（y=661，紧贴，gap=0）
- ctrl_bar → chat_input（y=692，紧贴，gap=0）
- chat_input 底部 → 任务容器底部（y=754，完全对齐）
- 左面板任务容器 → 右面板 chat_input（底部同为 754px，跨面板对齐）
...
| 用户头像 | 气泡右侧 8px，右边界对齐 | ICON_LARGE=24×24 | 🔵 图标，右边界 = right_panel 右边界（x=1183），头像 cx = 236，组 transform x = 933（头像右边界 933+236+14=1183，与面板右边界对齐） |
...
│      │                               │                                            │
├──────┤                               ├──────────────────────────────────────────────┤
│ 💬/⚙️│                               │  [Agent▼][AI行为▼][AI托管▼][Voice][Report]   │
│      │                               │  [+] 输入任务描述...                      [▶]│
└──────┴───────────────────────────────┴────────────────────────────────────────────┘
```
...
[用户点击 Boot Check 按钮]
    ...
    自动开始环境检测（逐项检测）
       ...
       ├─── 检测项失败(●→✗) ──→ 显示红色叉
       │
       └─── 全部检测完成 ──→ 进度条 8/8，可关闭弹窗
```

**文字描述：**

Boot Check 进度条在用户点击 Boot Check 按钮后弹出，显示环境检测进度。顶部显示"Boot Check 环境体检"标题；中间是进度条（已填充/总宽度）和计数器（X/10）；下方是 10 个检测项列表，每项显示：状态图标●/✓/✗ + 检测项名称 + 结果文字。全部检测完成后进度条变为 10/10，弹窗可关闭。
...
| 进度条 | body_x+16, body_y+52, w-32, h=24 | lv_bar，填充比例=已完成数/10 |
| 计数器 | body_x+16, body_y+52, 右对齐, h=24 | "X/10" |
| 检测项列表 | body_x+16, body_y+84, w-32, h=10×40=400 | 10行，每行40px高 |
...
**Voice 开关控制（独立维度，来自 PRD §4.7）：**

Voice 属于独立开关维度，不与 Agent/AI行为/AI托管互斥。

| 维度 | 选项 | 说明 |
|------|------|------|
| Voice 模式 | OFF（默认）/ ON | OFF 时纯文本；ON 时 AI 语音+文字同时输出 |

**AI 人物容器喇叭开关：**

| 控件 | 默认值 | 交互 | 状态视觉 |
|------|------|------|------|
| 喇叭开关 | ON | 点击切换 AI 语音播报能力 | ON：accent色边框+喇叭图标+文字"Voice"+滑块右侧；OFF：灰色边框+文字"OOF"+滑块左侧 |

1. `SPK OFF`：AI 保持文字输出，不触发 TTS（灰色滑块在左）。
2. `SPK ON`：AI 在文字渲染完成后触发 TTS，文字+语音同时输出（绿色滑块在右）。
3. 该开关位于左侧 AI 人物容器右上角，与控制行 Voice 开关为同一状态的不同入口。

**文本优先渲染规则（必须实现）：**

1. 用户语音：先 STT 出文本，再进入 Chat 输出区用户气泡，再触发发送。
2. AI 回复：先流式/完整文本进入 Chat 输出区 AI 气泡，再触发 TTS 播放。
3. 文本与语音同源：TTS 内容必须取自已显示文本快照，避免口播与屏显不一致。
4. 失败降级：STT/TTS 任一路失败时，保留文本会话能力，不得阻断输入输出。
...
[用户进入 Chat/Work 模式]
    ...
    显示控制行（Agent下拉 / AI行为下拉 / AI托管下拉 / Voice按钮 / Report按钮）
      ...
      │
      └─── 切换 Voice 开关 ──→ 关闭/打开语音模式
              │
              ├── 打开：用户语音转文字进入输出区，再走发送
              ├── 打开：AI 文本先显示，再做 TTS 播放
              └── 关闭：停止自动 TTS，保留纯文本对话

             └─── 点击 AI 人物容器喇叭 ──→ SPK ON/OFF
              │
              ├── OFF：AI 仅文字
              └── ON：AI 文字 + 语音
```

    **Voice 状态视觉（UI-11 控制行）：**

    | 状态 | 控件样式 | 行为提示 |
    |------|---------|---------|
    | Off | 次按钮底色 + 中性文字 | `Voice Off` |
    | On | 强调色底 + 反白文字 | `Voice On` |
    | Capturing | On 样式 + 轻微脉冲 | `Listening...` |
    | Speaking | On 样式 + 点阵动效 | `Speaking...` |
    | Error | 警告色边框 | `Voice error` |

  **最小化/托盘联动（Voice 后台策略）：**

  1. 窗口最小化或隐藏到托盘后，语音状态机不断链。
  2. 当 `Voice=On` 且 `SPK=On` 时，AI 文本完成后继续后台播报。
  3. 恢复窗口后，聊天区需可回看完整“用户文本 -> AI 文本 -> 播报状态”链路。
...
## §17 扩展界面（KB · Skill 双入口：左导航图标 或 Settings → C3 Agent 中心）

> 需求 → PRD §8 Agent 能力 → KB/Skill **左导航图标**（主要入口）或 Settings → C3 Agent 中心（二级入口）

### UI-60: 知识库面板

**进入方式：** 左导航 📚 KB 图标 **或** Settings → C3 Agent 中心 → KB Tab（双入口）
...
**进入方式：** 左导航 🧩 Skill 图标 **或** Settings → C3 Agent 中心 → Skill Tab（双入口）
