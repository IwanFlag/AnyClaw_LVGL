...
```
┌─────────────────────────────────────────────────────────────────────────────────────┐
│  标题栏  y=0, h=48   Bot · Chat                              [─] [□] [✕]           │
├──────┬──────────────────────────────────────────────────────────────────────────────┤
│      │                                                                            │
│ 导航  │  左面板 x=95, y=56, w=340, h=698  │  右面板 x=443, y=56, w=1000, h=698    │
│ x=0  │                               │                                            │
│ y=48 │                               │                                            │
│ w=87 │                               │                                            │
│ h=752│                               │                                            │
...
**第二层：左面板内部（x=95, y=56, w=340, h=698）从上到下：**

```
│  ┌─ 状态行 x=107, y=68, w=316, h=30 ─────────────────────────────────────────────┐  │
│  │ [Agent Status]  [●]  Running...                                               │  │
│  └───────────────────────────────────────────────────────────────────────────────┘  │
│  ┌─ 模型名 x=107, y=102 ────────────────────────────────────────────────────────┐  │
│  │ Model: MiniMax-M2.7                                                           │  │
│  └───────────────────────────────────────────────────────────────────────────────┘  │
│  ── 分隔线 x=107, y=132, w=316, h=1 ─────────────────────────────────────────────────  │
│  ┌─ task_title x=107, y=144 ───────────────────────────────────────────────────┐  │
│  │ 任务列表  (0)                                                                 │  │
│  └───────────────────────────────────────────────────────────────────────────────┘  │
│  ┌─ Session Card x=107, y=189, w=316, h=170 ──────────────────────────────────┐  │
│  │ 会话                      (0)                                                │  │
│  │───────────────────────────────                                               │  │
│  │ [空状态占位]                                                                    │  │
│  └───────────────────────────────────────────────────────────────────────────────┘  │
│  ┌─ Cron Card x=107, y=371, w=316, h=170 ───────────────────────────────────┐  │
│  │ Cron                    (0)                                                  │  │
│  │───────────────────────────────                                               │  │
│  │ [空状态占位]                                                                    │  │
│  └───────────────────────────────────────────────────────────────────────────────┘  │
...
**第三层：右面板内部（x=443, y=56, w=1000, h=698）从上到下：**

```
│  ┌─ ctrl_bar x=CHAT_GAP, y=0, w=1000, h=CTRL_BAR_H ───────────────────────┐  │
│  │ [Agent ▼]  [Report]  [AI行为 ▼]                                            │  │
│  └───────────────────────────────────────────────────────────────────────────────┘  │
│  ┌─ mode_panel_chat x=CHAT_GAP, y=CHAT_GAP+CTRL_BAR_H ─────────────────────┐  │
│  │  ┌─ chat_cont（消息区）─ 撑满上方剩余空间 ─────────────────────────────────┐ │  │
│  │  │                                                                               │ │  │
│  │  │   🔵 帮我查一下今天的天气                            12:28              │ │  │
│  │  │                                                                               │ │  │
│  │  │   🦞 今天天气晴，26°C，待办3项未完成。               12:28           │ │  │
│  │  │                                                                               │ │  │
│  │  │   ┌─ AI 思考 ───────────────────────────────────────────────────┐ │ │  │
│  │  │   │ 调用天气API获取数据中...                                   │ │ │  │
│  │  │   └─────────────────────────────────────────────────────────────┘ │ │  │
...
| 控件 | x | y | w | h | 备注 |
|------|---:|---:|---:|---:|------|
| ctrl_bar | 16 | 56 | 976 | 36 | pr 内部 y=0，+pr.y=56 |
| mode_panel_chat | 16 | 92 | 976 | 566 | y=56+36，h=698-36-gap×2 |
| chat_cont | 16 | 92 | 976 | ~500 | flex-grow 填充 |
| chat_input | 16 | input_y | ~910 | 36 | mode_panel_chat 底部 |
| btn_send | 886 | input_y+gap | 58 | 58 | chat_input 右侧 |
...
| Cron Card | 316×170 | 107 | 371 | card_y + card_h + 12px gap |
...
| 控件 | 尺寸 | x | y | 说明 |
|------|------:|---:|---:|------|
| 消息区容器 | 1000×591 | 443 | 56 | RIGHT_PANEL_W × 85% |
| 用户气泡 | 最大700宽 | 716 | 随内容 | CHAT_BUBBLE_PCT=70%，靠右对齐 |
| AI 气泡 | 最大700宽 | 443 | 随内容 | CHAT_BUBBLE_PCT=70%，靠左对齐 |
| AI 思考块 | 1000×~变高 | 443 | 紧接气泡 | 折叠显示标题，点击展开 |
| 文件输出块 | 1000×~变高 | 443 | 紧接思考块 | 100%宽，紧接上方 |
...
| 控件 | 尺寸 | x | y | 说明 |
|------|------:|---:|---:|------|
| 输入区容器 | 1000×36 | 443 | 659 | y=56+591+12 |
| 上传按钮 | 29×29 | 455 | 664 | BTN_UPLOAD_PCT(30%)×36=29，垂直居中 |
| 输入框 | ~910×36 | 496 | 659 | 左边=455+29+12，右边到右面板边缘 |
| 发送按钮 | 43×43 | 900 | 664 | 右边缘-43，垂直居中 |

**4.4 控制行（y=695，高=36）**

| 控件 | 尺寸 | x | y | 说明 |
|------|------:|---:|---:|------|
| 控制行容器 | 1000×36 | 443 | 695 | 输入区下方，y=659+36 |
| Agent 下拉 | ~120×28 | 455 | 699 | 按钮高=36×58%=21，实际28px，垂直居中 |
| Report 按钮 | ~72×28 | 581 | 699 | gap=6px |
| AI行为 下拉 | ~120×28 | 659 | 699 | gap=6px |

**控件间相邻关系总结：**
- 标题栏 → 内容区（y=48，无缝衔接）
- 左导航 → 间距(8px) → 左侧面板（x=87+8=95）
- 左侧面板 → 右侧面板（x=443，无 splitter）
- 消息区 → 分隔线（y=647，紧贴消息区下方+gap）
- 分隔线 → 输入区（y=659，紧贴分隔线下方+12px）
- 输入区 → 控制行（y=695，紧贴输入区下方）
- 上传按钮 → 输入框（x=496，紧贴上传按钮右边缘+12px）
...
```
┌─────────────────────────────────────────────────────────────────────────────────────┐
│ 窗口: 0,0 → 1443,800（标题栏 h=48，剩余 y≥56 用于内容）                                │
│                                                                                     │
│ 第一层: 左右面板（窗口内容区从 y=56 开始）                                             │
│ ┌───────┬───────────────────────────────────────────────────────────────────────┐  │
│ │ 左面板│                       右面板 (Bot 主界面)                                 │  │
...
│ │ h=698 │ 第二层: 右面板内部从上到下                                               │  │
│ │       │ ┌─────────────────────────────────────────────────────────────────┐  │  │
│ │ ...   │ │  ctrl_bar: y=0, h=40 (Agent▼ | Report | AI行为▼)                  │  │  │
│ │       │ ├─────────────────────────────────────────────────────────────────┤  │  │
│ │       │ │  mode_panel_chat: y=46, h=646 (铺满剩余空间)                       │  │  │
│ │       │ │  ├── chat_cont: flex-grow（消息流区域，上方撑满）                   │  │  │
│ │       │ │ │                                                                  │  │  │
│ │       │ │ ├── [用户气泡]                                                       │  │  │
│ │       │ │ ├── [AI 气泡]                                                        │  │  │
│ │       │ │ └── [StepCard 轨迹]（Task 模式时）                                   │  │  │
│ │       │ │                                                                  │  │  │
│ │       │ └── chat_input: 底部固定 (输入框 + [📎][▶])                          │  │  │
│ │       │                                                                  │  │  │
│ │       └─────────────────────────────────────────────────────────────────┘  │  │
...
| ctrl_bar | x=6, y=0, w=988, h=40 | 控制栏（Agent▼/Report/AI行为▼），在 mode_panel_chat 上方 |
| mode_panel_chat | x=6, y=46, w=988, h=646 | 铺满 ctrl_bar 下方全部区域 |
| chat_cont | flex-grow | 消息流区域（mode_panel_chat 内，上方撑满） |
| chat_input | 底部固定 | 输入框（mode_panel_chat 内底部，Shift+Enter 换行，Ctrl+Enter 任务） |

> **层级关系**：`ctrl_bar` 是 `right_panel` 的直接子控件（y=0）；`mode_panel_chat` 也是 `right_panel` 的直接子控件（y=46，在 ctrl_bar 下方）；`chat_cont` 和 `chat_input` 都是 `mode_panel_chat` 的子控件。
