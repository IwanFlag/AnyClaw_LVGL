...
[应用启动]
       │
       ▼
    创建左导航栏 left_nav (x=0, y=48, w=87, h=702)
       │
       ├─── 创建 nav_top (h=212px) ──→ 创建 Bot + File + KB + Skill 按钮
       │                                        │
       │                                        ├─── 点击 Bot ──→ 高亮 Bot，
       │                                        │              隐藏其他模块，
       │                                        │              显示 Bot 内容（Chat/Work）
       ...
       │
       └─── 创建 nav_bot (h=100px, y=650) ──→ 创建 Chat/Work 切换 + Settings
                                        │
...
|| 控件 | 宽 | 高 | x | y | 计算公式 |
|------|---:|---:|---:|---:|---------
| left_nav | ~71px | 605 | 0 | 41 | `NAV_W_PCT=6%`（1190屏≈71px，1920屏≈115px）|
| nav_top | ~71px | ~212 | 0 | 41 | 内容撑起：4×52 + 3×12 |
| nav_spacer | ~71px | flex_grow=1 | 0 | ~253 | 占据所有剩余空间（1190屏≈273px）|
| nav_bot | ~71px | ~104 | 0 | ~526 | 内容撑起：2×52 + 0×12，flex END 贴底 |
...
**第三层：顶部模块按钮区（nav_top，h≈212px，y=41）**

|| 控件 | 尺寸 | x | y | 说明 |
|------|------|---:|---:|------|
| Bot 按钮 | ~52×52 | 居中(~10) | 48 | nav_top 顶部，nav_btn_sz = NAV_W×70% |
| File 按钮 | ~52×52 | 居中(~10) | 116 | `y = 48 + 52 + 16 = 116` |
| KB 按钮 | ~52×52 | 居中(~10) | 184 | `y = 116 + 52 + 16 = 184` |
| Skill 按钮 | ~52×52 | 居中(~10) | 252 | `y = 184 + 52 + 16 = 252` |
...
|| 元素 | 宽 | 高 | x | y | 计算 |
|------|---:|---:|---:|---:|------|
| left_panel | 276 | 598 | 90 | 48 | x = nav_w(71) + 8 + 11，y = TITLE_H+7 |
| right_panel | 818 | 598 | 373 | 48 | x = left_panel右 + 7，y同 |
| 输出区 | 818 | ~507 | 373 | 48 | right_panel高 × CHAT_FEED_H_PCT(85%) |
| Splitter | 6 | ~507 | 373 | 48 | SPLITTER_W_PCT(0.5%)，可拖动 |
| 控制行 | 818 | 31 | 373 | 561 | y = 输出区底+6 |
| 输入区 | 818 | 62 | 373 | 592 | y = 控制行底+0 |
...
│  │ [空状态占位]                                                                         │  │
│  └───────────────────────────────────────────────────────────────────────────────┘  │
│  ┌─ Cron Card x=92, y=504, w=256, h=126 ────────────────────────────────────────┐  │
│  │ 定时任务                  (0)                                                  │  │
│  │───────────────────────────────                                                 │  │
│  │ [空状态占位]                                                                         │  │
│  └───────────────────────────────────────────────────────────────────────────────┘  │
...
| AI伙伴容器 | x=92, y=58, w=256, h=203 | 顶部人物 + 底部 Agent Status + Model |
...
| Cron Card | x=92, y=504, w=256, h=126 | 定时任务，距 Session Card 16px，距面板底部 16px |
...
| 用户头像 | 1143 | 93 | r=14 | — | 头像右边界=1159，距面板右边界 24px |
| 用户气泡 | 980 | 71 | 139 | 45 | 蓝色渐变，气泡右边界距头像左边界 12px |
...
> **注意：** 本节为 1450×800 基准坐标参考，实际运行缩放至 1190×646。

|| 控件 | 尺寸 | x | y | 说明 |
|------|------:|---:|---:|---:|------|
| AI伙伴容器 | 256×203 | 92 | 58 | 顶部人物 + 底部 Agent Status + Model |
| Agent Status | 256×20 | 92 | 240 | 绿色LED + Running，紧贴容器底部 |
| Model | 256×16 | 92 | 258 | "Model: MiniMax-M2.7" |
| Session Card | 256×190 | 92 | 298 | 会话列表，距 AI伙伴容器 10px |
| Cron Card | 256×126 | 92 | 504 | 定时任务，距 Session Card 16px，距面板底部 16px |
...
**相邻关系：**
- AI伙伴容器底部 → Agent Status：y+37px（58+203→240）
- Agent Status → Model：y+18px（240→258）
- Model → 分隔线：y+14px（258→272）
- 分隔线 → 任务列表标题：y+14px（272→286）
- 任务列表 → Session Card：y+12px（286→298）
- Session Card → Cron Card：y+16px（298+190→504）
- Cron Card 底部 → 左面板底部：16px（504+126=630，左面板底部=646）
