...
Work 模式采用**单列滚动表单布局**（VSCode 设置面板风格），所有设置区块和操作区在同一垂直列中排列。
...
│                                                                  │
│  ┌ Agent Trace ──────────────────────────────────────────────┐  │
│  │ 最近的 5 条 Agent 执行记录（可折叠）                          │  │
│  └───────────────────────────────────────────────────────────┘  │
│                                                                  │
│  ┌ Step Stream ───────────────────────────────────────────────┐  │
│  │ 📄 读取 config.json                          ✓ 完成(绿)    │  │
│  │ ✏️ 写入 main.cpp                          ✓ 完成(绿)    │  │
│  │ ⚡ 执行 npm install                        ● 进行中(绿spinner)│  │
│  │                                                        [接受✓][拒绝✗][回退↩] │  │
│  └───────────────────────────────────────────────────────────┘  │
│                                                                  │
│  ┌ Output Area ──────────────────────────────────────────────┐  │
...
**核心区别于 Chat 模式：**
- Chat = 对话气泡流（User/AI 消息垂直排列）
- Work = 单列表单（Runtime/Trace/StepStream/Output 等可折叠区块 + 浮动聊天面板）
...
[用户点击左侧导航 Bot → 切换到 Work 模式]
       │
       ▼
    显示 Work 界面：单列表单（Runtime/Trace/StepStream/Output/Chat 等区块）
       │
       ├─── 填写 Runtime Mode 控制参数 ──→ 控制模式/LLM/AI交互模式
       │
       ├─── 输入任务描述，点击 [▶] ──→ AI 开始执行，Step Stream 实时更新
       │                                   ● 进行中 / ✓ 完成 / ✗ 失败
       │
       ├─── 写操作卡片出现 [接受/拒绝/回退]  ──→ 确认/取消/撤销写入
       │
       ├─── 查看 Output Area 终端输出 ──→ npm install 等执行结果
       │
       └─── 任务完成后折叠 Step Stream ──→ 查看最终结果
```

**布局结构（单列滚动，内容区从上到下共 3 层）：**

1. **Step Stream 区（flex-grow）：** 占据主区域，垂直排列 StepCard。实时更新每个步骤状态。
2. **控制行（固定高度）：** 位于表单下方，包含 Agent 选择、Report 按钮、AI行为下拉。
3. **输入区（固定高度）：** 位于底部，包含附件按钮、文本输入框、发送按钮。
...
| 元素 | 尺寸 | 位置 | 行为 |
|------|------|------|------|
| Step Stream 表单区块 | 100% × auto | 表单顶部区域 | 垂直排列 StepCard，状态实时更新 |
| StepCard | 100% × auto | Step Stream 内 | 状态图标 + 描述 + 操作按钮（接受/拒绝/回退） |
| Output Area 区块 | 100% × auto | Step Stream 下方 | Terminal 输出，code 字体 |
| Work Chat Panel | 浮动，38% 宽度 | 表单右下角 | 浮动聊天面板，可折叠 |
| 控制行 | 100% × CTRL_BAR_H | 表单底部 | 固定高度，按钮组 |
| 输入区 | 100% × CHAT_INPUT_MIN_H | 表单最底部 | 固定高度，自动增长 |
...
**操作流程（用户视角）：**
```
[用户点击左侧导航 Task，进入 Task 模块]
       │
       ▼
    左侧面板显示 Task 内容，右侧面板隐藏
       │
       ├─── 查看任务队列列表 ──→ ● 进行中/✓ 已完成/○ 队列中
       │                                                              │
       ├─── 点击 [新建任务] 按钮 ──→ 弹出任务输入框
       │                                                              │
       └─── 任务执行状态自动刷新 ──→ 列表实时更新（2s 间隔）
```

**布局结构（内容区共 1 层）：**

1. **Task 内容区（LEFT_PANEL_W）：** 占据左侧面板，完全隐藏右侧面板。显示任务列表容器。

**子元素结构：**

| 元素 | 尺寸 | 位置 | 行为 |
|------|------|------|------|
| module_tasks_panel | ≤ LEFT_PANEL_W × PANEL_H | left_panel 内 | surface 背景，1px border |
| module_tasks_view | 100% × flex-1 | panel 内 | textarea，显示任务队列 |
| module_tasks_state | 100% × auto | panel 底部 | caption 字体，同步状态 |
| 新建任务按钮 | auto × BTN_H | state 右侧 | accent 渐变，点击弹出输入 |
...
1. **File 内容区（LEFT_PANEL_W）：** 占据左侧面板，完全隐藏右侧面板。显示文件列表容器。

**子元素结构：**

| 元素 | 尺寸 | 位置 | 行为 |
|------|------|------|------|
| module_files_panel | ≤ LEFT_PANEL_W × PANEL_H | left_panel 内 | surface 背景，1px border |
...
[用户任意界面]
    ...
    检测键盘按键
       │
       ├─── Ctrl+1/2/3 ──→ 切换左导航模块（Bot/Tasks/Files）
       │
       ├─── Ctrl+Tab ──→ 循环切换模块（Bot→Tasks→Files→Bot）
       │
