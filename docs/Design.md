...
| 阶段 | Step | 内容 |
|------|------|------|
| 阶段 A（首次可用闭环） | UI-04 | 语言选择 — 中文/English 两个按钮 |
| 阶段 A（首次可用闭环） | UI-05 | 环境就绪 — 自动检测 + 一键修复 + 状态摘要 |
| 阶段 A（首次可用闭环） | UI-06 | 模型 & API Key — 推荐模型 + Provider + API Key 输入 |
| 阶段 A（首次可用闭环） | UI-09 | 完成页 — 进入主界面主按钮 + 进阶能力推荐卡 |
| 阶段 B（进阶能力引导，非阻断） | UI-07 | 本地模型 — Enable Switch + 模型选择 + Skip |
| 阶段 B（进阶能力引导，非阻断） | UI-08 | 连接 IM — 渠道卡片 + 配置/Skip |
| 阶段 B（进阶能力引导，非阻断） | UI-09 扩展 | Leader 模式与可选运行时管理 |

**双阶段规则：**
- 阶段 A 只允许一个主决策点/屏，避免并行决策。
- 阶段 B 仅通过推荐任务清单触发，不阻断主流程。
- 阶段 A 完成目标：3 分钟内达到“可聊天、可执行、可见反馈”。

### UI-05 详细: 环境就绪（自动检测 + 一键修复）
...
| ⏳ 修复中 | 蓝色 loader | "修复中..." |
| ⚠️ 需处理 | 黄色 alert-circle | "需处理" / "需要修复" |
