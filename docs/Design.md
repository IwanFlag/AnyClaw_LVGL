...
- 标题栏（48px）：显示 `🧄 Bot · Chat`
- 内容区：分为 left_panel（~25%）和 right_panel（~75%）

**第二层：left_panel（固定宽度，最小 160px）**
- 内容：Session 列表 + Cron 列表
- 行为：2s 自动刷新，固定宽度，左侧固定显示

**第三层：right_panel 内部分为三个子层（自上而下）**

1. **输出区（flex-grow，垂直滚动）**
   - 用户气泡：最大 70% 宽度，靠右，渐变背景
   - AI 气泡：最大 70% 宽度，靠左，surface 背景，左侧 accent 竖线
   - AI 思考块：100% 宽度，紧接 AI 气泡下方，caption 标题，可折叠
   - 文件输出块：100% 宽度，紧接思考块下方，code 字体，顶部语言标签
