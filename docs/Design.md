...
| AI伙伴容器 | x=92, y=58, w=256, h=203 | 顶部 SVG 青虫子 + 底部 Agent Status + Model |
...
| 维度 | 选项 | 说明 |
|------|------|------|
| Voice 模式 | 关闭（默认）/ 打开 | 关闭时纯文本；打开后进入语音链路，但严格“文本优先” |

**AI 人物容器喇叭开关（新增）：**

| 控件 | 默认值 | 交互 |
|------|------|------|
| Speaker（SPK ON/OFF） | ON | 点击切换 AI 语音播报能力 |

1. `SPK OFF`：AI 保持文字输出，不触发 TTS。
2. `SPK ON`：AI 在文字渲染完成后触发 TTS。
3. 该开关位于左侧 AI 人物容器内，不与控制行 Voice 开关冲突。
