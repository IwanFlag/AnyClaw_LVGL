...
Work 模式与 Chat 模式共用同一双栏布局（见 UI-11），唯一区别是右侧输出区：UI-11 输出区显示对话气泡，UI-12 输出区在此基础上额外显示 StepCard 列表 + Terminal 输出。
...
**UI-11 与 UI-12 的唯一区别：** UI-11 输出区只显示对话气泡；UI-12 输出区在此基础上额外显示 StepCard 列表 + Terminal 输出（Chat 对话气泡仍然保留）。

**StepCard 状态：** ● 进行中(绿 spinner) / ✓ 完成(绿) / ✗ 失败(红) / ? 等待确认(蓝)
**写操作按钮：** [接受 ✓] [拒绝 ✗] [回退 ↩]

**操作流程（与 UI-11 的差异）：**
- UI-11 的操作流程全部适用于 UI-12（左侧面板/控制行/输入区行为完全相同）
- 额外行为：执行任务时，输出区自动追加 StepCard，实时显示各步骤状态

**StepCard 特有行为：**

| 场景 | 行为 |
|------|------|
| AI 执行任务中 | 输出区追加 StepCard，显示当前步骤 |
| 写操作需确认 | StepCard 显示 [接受✓][拒绝✗][回退↩] 按钮 |
| Terminal 有输出 | 在最后一条 StepCard 下方追加 terminal 输出块 |

### UI-05: File 模块

File 模块激活时，左 nav 高亮 File，左侧面板（Session/Cron）隐藏，module_placeholder 填充整个 left_panel 区域，内容区（right_panel）空闲。
...
│    │  │  State: ready                                             │   │
│    │  ├─────────────────────────────────────────────────────────────┤   │
│    │  │  [Scan Workspace] [Open Asset Dir] [Sync Managed] [Open WS]│   │
│    │  ├─────────────────────────────────────────────────────────────┤   │
│    │  │  [All]  [Fonts]  [Icons]  [Sounds]                       │   │
│    │  │  🔍 Search path keyword...  [Ext: Any ▼]  [Clear Filters]│   │
│    │  ├─────────────────────────────────────────────────────────────┤   │
│    │  │  Resource snapshot... (textarea, scrollable)              │   │
│    │  └─────────────────────────────────────────────────────────────┘   │
└────┴─────────────────────────────────────────────────────────────────────┘
```

**约束：**
- module_placeholder 父容器为 left_panel，填充整个 left_panel 区域（Session/Cron 列表隐藏）
- 内容区（right_panel）在 File 模块下空闲，不被 File 内容占用
- File 内容宽度 = left_panel 宽度，不是内容区宽度

**Bot / File 布局对比：**

| 模块 | 左侧面板（Session/Cron） | 主内容区（right_panel） |
|------|--------------------------|-------------------------|
| Bot  | ✅ 显示（Session/Cron 列表） | ✅ 显示（输出区 + ctrl_bar + 输入区，见 UI-11） |
| File | ❌ 隐藏（module_placeholder 填充 left_panel） | 空闲 |
...
[用户点击左侧导航 File，进入 File 模块]
       │
       ▼
   left_panel 显示 module_placeholder（Resource Hub 面板）
   right_panel 空闲
       │
       ├─── 点击 [Scan Workspace] ──→ 重新索引工作区文件，状态更新
       ├─── 点击 [Open Asset Dir] ──→ 在文件管理器打开 assets 目录
       ├─── 点击 [Sync Managed] ──→ 同步托管段落，显示成功/失败状态
       ├─── 点击 [Open Workspace] ──→ 在文件管理器打开工作区根目录
       ├─── 点击 [All/Fonts/Icons/Sounds] ──→ 按类型过滤文件列表
       ├─── 输入搜索词 / 选择扩展名下拉 ──→ 实时过滤文件列表
       └─── 点击 [Clear Filters] ──→ 重置所有过滤器
```
...
| 元素 | 父容器 | 尺寸 | 行为 |
|------|--------|------|------|
| module_placeholder | left_panel | left_panel 全宽 × PANEL_H | 填充 left_panel，Session/Cron 隐藏 |
| module_files_panel | module_placeholder | left_panel 宽 × auto | surface 背景，1px border，radius_lg |
| 状态标签 | module_files_panel | 100% × auto | caption 字体，显示索引状态 |
| 四个操作按钮 | module_files_panel | 各 auto × BTN_H | secondary/accent 样式 |
| 过滤器标签行 | module_files_panel | 100% × auto | [All][Fonts][Icons][Sounds] 切换按钮 |
| 搜索行 | module_files_panel | 100% × auto | 搜索框 + 扩展名下拉 + Clear 按钮 |
| module_files_view | module_files_panel | 100% × 180px | textarea，显示文件路径列表 |
