...
```
┌────┬──────────────────────────────────────────────────────────────────┐
│ 🧄 │ 🧄 Bot · Chat    [ Chat | Work ]            ●  [⚙] [✕]       │ 标题栏 TITLE_H
├────┼──────────────────────────────────────────────────────────────────┤
│    │                                                                  │
│ 🦞 │  ← Bot                                                          │
│ ⚡ │  ← 任务    内容区（跟随左导航切换模块）                            │
...
**Session 面板（左侧 LEFT_PANEL_W）：** Session 列表（上）+ Cron 列表（下），可拖拽调整宽度。
**消息区（右侧）：** 用户消息靠右，AI 回复靠左，最大宽度 70%。
**输入区（底部固定）：** 📎 附件 + 🎤 语音 + 多行输入 + 发送按钮(BTN_SEND 圆形)。
...
```
  ┌──────────────────────────────┐
  │  [ Chat ]  [ Work ]          │  ← 标题栏内, 窗口宽×10% × LP_ROW_H, 胶囊式
  └──────────────────────────────┘
...
```
  ...
  输入行: 📎附件 + 🎤语音 + 多行输入 + 字数 + 发送按钮(BTN_SEND圆形)
