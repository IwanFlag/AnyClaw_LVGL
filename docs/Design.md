...
Bot 模块左 nav 高亮时，内容区分左右两栏：

```
┌─────────────────────────────────────────────────────────────────┐
│ left_panel (~25%)          │ right_panel (~75%)                │
│                            │                                   │
│ Session 列表               │ 输出区（flex-grow）                │
│ + Cron 列表                │     ↕ 垂直滚动                    │
│                            │                                   │
│ 固定宽度，最小 160px        │ 控制行（固定高度）                 │
│ 2s 自动刷新                │ 输入区（固定高度）                  │
└─────────────────────────────────────────────────────────────────┘
```
...
| 元素 | 尺寸 | 位置 | 行为 |
|------|------|------|------|
| left_panel | LEFT_PANEL_W × PANEL_H | 根容器左侧，固定宽度 | Session/Cron 列表，2s 刷新 |
| right_panel | flex-1，剩余空间 | left_panel 右侧 | 根据 Chat/Work 模式显示不同内容 |
| 控制行 | 100% × BTN_H | right_panel 底部上方 | 固定高度，按钮组：[Agent ▼] [Report] [AI行为 ▼] |
| 输入区 | 100% × CHAT_INPUT_MIN_H | right_panel 最底部 | 固定高度，自动增长，附件按钮 + 输入框 + 发送按钮 |
...
|------|---------------|-----------------|
| Bot  | Session 列表 + Cron 列表 | 输出区 + 控制行 + 输入区 |
...
| 文件列表 | lv_textarea | module_files_panel | 100% × 剩余高度 | 只读，滚动，显示文件绝对路径列表 |
...
[系统托盘图标被创建]
    ...
    显示托盘图标（根据 Agent 状态显示对应颜色）
       ...
       │                 │
       │                 ├─── Restart OpenClaw ──→ 重启 OpenClaw Gateway
       │                 │
...
**窗口骨架：** 继承 §12 12.4 Bot 模块通用布局（left_panel ~25% + right_panel ~75%）
...
```
[用户点击左侧导航 Bot，进入 Chat 模式]
       │
       ▼
    显示主界面：左侧面板 + 右侧输出区 + 控制行 + 输入区
       │
       ├─── 点击左侧面板 Session/Cron 项 ──→ 右侧输出区加载对应会话记录
       │
       ├─── 在输入区输入消息，按发送 ──→ 显示用户气泡 + AI思考 + 流式响应
       │
       ├─── 点击 AI 思考块标题 ──→ 展开/折叠思考内容
       │
       ├─── 点击文件输出块标签（terminal/python） ──→ 切换语言标签
       │
       └─── 点击控制行 Agent/Report/AI行为 下拉 ──→ 展开操作菜单
```
...
| 标题栏 | 1450 | 48 | 0 | 0 | WIN_W × TITLE_H_PCT(6%)，最小 36px |
| 内容区 | 1450 | 752 | 0 | 48 | WIN_H - TITLE_H |
| 左导航 | 87 | 752 | 0 | 48 | WIN_W × NAV_W_PCT(6%)，最小 36px |
| 间距 | 12 | — | 87 | — | PANEL_GAP |
| 左侧面板 | 240 | 450 | 99 | 56 | 固定值，左边顶到 x=99，右边到 x=339 |
| 右侧面板 | 1099 | 450 | 339 | 56 | 左边顶到 x=339，右边到 x=1438 |

**第二层：标题栏内部（y=0，h=48）**

| 控件 | 尺寸 | x | y | 计算公式 |
|------|-----:|---:|---:|---------|
| 品牌图标 | 38×38 | 8 | 5 | TITLE_H × 80% = 48×0.8，垂直居中（(48-38)/2=5） |
| 窗口标题文字 | ~200宽 | 52 | 0 | 图标右侧，body字体 |
| 模型下拉 | ~145宽 | 右侧堆叠 | 5 | WIN_W × 10%，右对齐到 x≈1305 |
| 窗口控制按钮组 | 28×28×3 + 间隙 | 右到左排列 | 10 | 按钮高=48×58%=28，垂直居中 |

**窗口控制按钮组（标题栏右侧，从右往左）：**
- 关闭按钮：28×28，右边距=28px，gap=6px
- 最大化按钮：28×28，紧贴关闭按钮左侧
- 最小化按钮：28×28，紧贴最大化按钮左侧
- 组右边界到标题栏右边缘：28px（WC_BTN_MARGIN_PCT=20%×28）

**第三层：左侧面板（x=99，y=56，w=240，h=450）**

| 控件 | 尺寸 | x | y | 计算公式 |
|------|-----:|---:|---:|---------|
| 面板容器 | 240×450 | 99 | 56 | 宽固定，高=内容区高-间距 |
| 状态行（LED+文字） | 240×36 | 99 | 56 | LP_ROW_H_PCT(8%)×450=36，最小24px |
| 分隔线 | 240×1 | 99 | 92 | 状态行下方 |
| Session 列表 | 240×~变高 | 99 | 93 | 垂直滚动，起点紧接分隔线 |
| 分隔线 | 240×1 | 99 | ~变高 | Session 和 Cron 之间 |
| Cron 列表 | 240×~变高 | 99 | ~变高 | 垂直滚动 |

**第四层：右侧面板（x=339，y=56，w=1099，h=450）**
...
| 控件 | 尺寸 | x | y | 计算公式 |
|------|-----:|---:|---:|---------|
| 消息区容器 | 1099×382 | 339 | 56 | RIGHT_PANEL_W × CHAT_FEED_H_PCT(85%)，高=450×0.85=382 |
| Splitter（分隔条） | 6×382 | 339 | 56 | 可拖动，0.5%窗口宽，最小3px |
| 用户气泡 | 最大766宽（1099×70%） | 316+间隙 | 随内容 | CHAT_BUBBLE_PCT=70%，靠右对齐到右边缘 |
| AI 气泡 | 最大766宽 | 339 | 随内容 | CHAT_BUBBLE_PCT=70%，靠左对齐到左边缘 |
| AI 思考块 | 1099×~变高 | 339 | 紧接气泡 | 100%宽，紧接上方气泡/思考块 |
| 文件输出块 | 1099×~变高 | 339 | 紧接思考块 | 100%宽，紧接上方 |

**气泡内边距：** 8px（CHAT_INPUT_PAD_ALL）
**消息间距：** 6px（CHAT_GAP_MIN），最小3px

**4.2 控制行（y=444，h=36）**
| 控件 | 尺寸 | x | y | 计算公式 |
|------|-----:|---:|---:|---------|
| 控制行容器 | 1099×36 | 339 | 444 | 消息区下方，y=56+382+6+间隙 |
| Agent 下拉 | ~120×28 | 12 | 4 | 按钮高=36×58%=21，实际28px，垂直居中 |
| Report 按钮 | ~72×28 | +6+120 | 4 | gap=6px |
| AI行为 下拉 | ~120×28 | +6+72 | 4 | gap=6px |

**4.3 输入区（y=492，h=96+）**
| 控件 | 尺寸 | x | y | 计算公式 |
|------|-----:|---:|---:|---------|
| 输入区容器 | 1099×~96 | 339 | 492 | y=444+36+12 |
| 上传按钮 | 29×29 | 12 | 居中 | BTN_UPLOAD_PCT(30%)×96=29，最小24px |
| 输入框 | ~1037×96 | 49 | 0 | 左边=上传按钮+12+间隙，右边到右面板边缘 |
| 发送按钮 | 43×43 | 右边缘-43 | 居中 | BTN_SEND_PCT(45%)×96=43，最小32px |

**输入框内边距：** 8px（CHAT_INPUT_PAD_ALL）
**输入框与发送按钮间距：** 12px（GAP）

**控件间相邻关系总结：**
- 标题栏 → 内容区（y=48，无缝衔接）
- 左导航 → 间距(12px) → 左侧面板（x=99，紧贴间距右边缘）
- 左侧面板 → Splitter(6px) → 右侧面板（x=339，紧贴Splitter右边缘）
- 消息区 → Splitter(6px) → 控制行（y=444，紧贴Splitter下边缘）
- 控制行 → GAP(12px) → 输入区（y=492，紧贴控制行下边缘+12px间距）
- 上传按钮 → 输入框（x=49，紧贴上传按钮右边缘+12px间隙）
- 输入框 → 发送按钮（x=右边缘-43，紧贴输入框右边缘+12px间隙）
...
[用户进入 Chat 界面]
    ...
    显示空状态界面（UI-19）
       │
       ├─── 在输入区输入消息并发送 ──→ 系统判定请求类型（Q&A 或 Task）
       │                                                              │
       ├─── Q&A 判定 ──→ 显示单列消息流（用户气泡 + AI 气泡）
       │                                                              │
       ├─── Task 判定 ──→ 显示双区视图（对话摘要 + StepCard 轨迹 + 结果块）
       │                                                              │
...
1. **输出区（flex-grow）：** 根据请求类型显示不同视图。Q&A 为单列消息流，Task 为双区视图。

2. **输入区（固定高度）：** 位于底部，包含附件按钮、文本输入框、发送按钮。

**子元素结构：**

| 元素 | 尺寸 | 位置 | 行为 |
|------|------|------|------|
| 输出区容器 | 100% × flex-1 | 输入区上方 | Q&A 单列 / Task 双区切换 |
| 消息流容器 | 100% 宽度 | Q&A 视图内 | 垂直滚动，气泡列表 |
| 对话摘要区 | 50% 宽度 | Task 视图左侧 | Task 请求的摘要显示 |
| StepCard 轨迹区 | 50% 宽度 | Task 视图右侧 | 垂直排列步骤卡片 |
| 结果块 | 100% 宽度 | Task 视图底部 | Task 执行结果 |
| 输入区 | 100% × CHAT_INPUT_MIN_H | 界面底部 | 自动增长高度 |
...
用户发送消息后，系统创建用户消息气泡。气泡靠右对齐，最大宽度 70%，超出部分自动换行。背景使用深绿色渐变 `#2B5C3E → #1F4A30`，文字为白色，四个圆角使用 `radius_xl`，底部应用 `shadow_md` 阴影增强层次感。气泡插入消息流容器后，视图自动滚动到最新消息处。气泡右上角显示发送时间戳。
...
[AI 生成响应文本]
       │
       ▼
[创建 AI 消息气泡] ← 分配 raised 背景 #1E2230
       │
       ▼
[左侧绘制 accent 竖线] ← GAP_SMALL×0.5 宽度
       │
       ▼
[Markdown 渲染] ← 支持粗体/斜体/代码/列表
       │
       ▼
[设置最大宽度 70%] ← 超出换行
       │
...
AI 响应文本生成完成后，系统创建 AI 消息气泡。气泡靠左对齐，背景为 raised 色 `#1E2230`，左侧边缘有 `GAP_SMALL×0.5` 宽度的 accent 色竖线作为视觉标记。气泡内容支持 Markdown 渲染，包括粗体、斜体、代码块、列表等格式。气泡最大宽度 70%，超出部分自动换行。插入消息流后，视图自动滚动到最新消息位置。
...
- 3 个圆点，直径 GAP_SMALL×1.5，accent 色
- 依次脉冲效果，周期 1.2s
...
[AI 开始流式输出]
       │
       ▼
[显示打字指示器] ← 3 个圆点，直径 GAP_SMALL×1.5
       │
       ├──► [第1个圆点 脉冲] ──► [延迟 400ms]
       │                            │
       ├──► [第2个圆点 脉冲] ──► [延迟 400ms]
...
当 AI 开始流式输出响应时，界面上显示打字指示器。该指示器包含 3 个圆点，直径为 `GAP_SMALL×1.5`，颜色为 accent 色。三个圆点依次进行脉冲动画效果，间隔 400ms，完整周期 1.2s，周而复始直到 AI 输出完成。当 AI 响应输出完毕后，打字指示器隐藏，随即显示完整的 AI 消息内容。
...
用户点击输入框时，输入框获得焦点并显示 `focus_glow` 发光边框。用户在输入过程中，底部栏实时显示字数统计。输入完成后，用户可点击右侧 accent 渐变圆形的发送按钮 ▶，或按下 Enter 键发送消息。若发送内容为空，则提示用户输入内容。发送前系统调用权限系统校验目录/文件访问权限，校验失败则显示权限不足提示。校验成功后，消息发送到 Agent，输入框清空并重置高度，用户消息气泡插入消息流容器。
...
用户点击输入区的附件按钮 📎 后，弹出菜单让用户选择「选择文件」或「选择目录」。选择后调用 Win32 原生对话框，用户完成选择后系统获取文件/目录路径及元信息。附件卡片根据类型显示不同图标：文件显示 📄 + 文件名 + 大小，目录显示 📁 + 名称 + 文件数，图片额外显示缩略图预览（最大 120×120px）。卡片嵌入消息气泡中。用户点击附件卡片时，系统先进行权限校验，校验失败提示权限不足，校验成功则打开附件：目录调用资源管理器，文件调用系统默认程序。
...
用户首次进入 Chat 界面或当前无对话记录时，显示空状态页面。页面中央展示品牌吉祥物 🧄🦞 组合动画或静态图标，下方显示欢迎文案如"有什么我能帮你？"。页面底部水平排列 3-5 个快捷建议气泡，内容为预设的高频问题模板（如"帮我整理一下文件"）。
...
搜索功能用于在大量聊天记录中快速定位历史消息。搜索界面分为三个区域：顶部为搜索输入框，支持实时关键词过滤；左侧或顶部为筛选标签栏，可按"全部/用户/AI/文件/目录"等维度过滤；下方为滚动结果列表，每条结果显示时间戳、发送者角色和内容摘要。

**交互流程：**
1. 用户点击搜索图标或直接点击搜索框
2. 搜索界面展开，显示输入框和筛选选项
3. 用户输入关键词，结果列表实时更新
4. 点击结果项，列表自动滚动到对应消息位置并高亮显示
5. 点击关闭按钮或返回，搜索界面收起，恢复聊天列表
...
| 左侧面板 | Session/Cron 列表 | StepCard 步骤流（同样 2s 刷新） |
| 写操作 | 无 | [接受✓][拒绝✗][回退↩] 按钮 |
...
**第一层：窗口骨架（继承 §12 12.4）**
- 左导航栏（56px）：Bot 图标高亮，状态 icon=accent
- 标题栏（48px）：显示 `🧄 Bot · Work`
- 内容区：分为 left_panel（~25%）和 right_panel（~75%）

**第二层：left_panel（固定宽度，最小 160px）**
- 内容：StepCard 步骤流列表（替代 Chat 的 Session/Cron）
- 行为：2s 自动刷新，实时显示各步骤状态

**第三层：right_panel 内部分为三个子层（自上而下）**

1. **输出区（flex-grow，垂直滚动）**
   - 对话气泡：同 Chat 模式（保留用户/AI 对话历史）
   - StepCard 列表：按执行顺序排列，显示操作类型图标 + 描述 + 状态
   - Terminal 输出块：跟在最后 StepCard 下方

2. **控制行（固定高度）**
   - 内容：`[Agent ▼]  [Report]  [AI行为 ▼]`
   - 位置：输出区下方，始终可见
...
| 元素 | 尺寸 | 位置 | 行为 |
|------|------|------|------|
| 左侧面板 | LEFT_PANEL_W × PANEL_H | 根容器左侧 | StepCard 步骤流，2s 刷新 |
| 输出区 | flex-1 | 右侧主区域 | 垂直滚动容器 |
| 对话气泡 | 最大 70% 宽度 | 输出区 | 同 Chat 模式 |
| StepCard | 100% 宽度 | 输出区 | 操作类型图标 + 描述 + 状态 + 操作按钮 |
| Terminal 输出块 | 100% 宽度 | 最后 StepCard 下方 | code 字体，顶部 terminal 标签 |
| 控制行 | 100% × BTN_H | 输出区下方 | 固定高度，按钮组 |
| 输入区 | 100% × CHAT_INPUT_MIN_H | 界面最底部 | 固定高度，自动增长 |
...
1. **StepCard 容器（宽 100%，高 auto）：** 位于 Work 输出区，按执行顺序纵向排列。显示操作类型图标、名称、状态和输出摘要。

| 元素 | 尺寸 | 位置 | 行为 |
|------|------|------|------|
| 图标 | 16x16 | 左侧 | 📄 表示读操作 |
| 操作名称 | auto | 图标右侧 | 显示文件名或命令 |
| 状态指示 | 12x12 | 右侧 | ● 进行中(绿 spinner) / ✓ 完成(绿) / ✗ 失败(红) / ? 等待确认(蓝) |
| 输出摘要 | 宽 100% | 下方 | 可折叠，显示 JSON/文本摘要，展开后高 ≤120px |
...
1. **StepCard 容器（宽 100%，高 auto）：** 位于 Work 输出区。显示写操作图标、文件名、变更数量和三个操作按钮。

| 元素 | 尺寸 | 位置 | 行为 |
|------|------|------|------|
| 图标 | 16x16 | 左侧 | ✏️ 表示写操作 |
| 操作名称 | auto | 图标右侧 | 显示文件名 |
| 变更摘要 | auto | 名称下方 | 显示修改数量（如 "修改内容: 3 处变更"）|
| 接受按钮 | 60x32 | 底部左侧 | #3DD68C 绿色，点击后永久保存修改 |
| 拒绝按钮 | 60x32 | 底部居中 | #FF6B6B 红色，点击后放弃修改 |
| 回退按钮 | 60x32 | 底部右侧 | #6C9FFF 蓝色，点击后回退到修改前版本 |
...
[用户查看执行命令 StepCard]
    ...
    显示命令执行结果（成功/失败）
       │
       ├─── 状态: 成功(✓) ──→ 自动进入下一个 StepCard
       │
       ├─── 状态: 失败(✗) ──→ 显示错误信息 + 重试按钮
       │                        │
       │                        └─── 点击「重试」─→ 重新执行命令
       │
       └─── 状态: 进行中(●) ──→ 显示 spinner 动画
```
...
1. **StepCard 容器（宽 100%，高 auto）：** 位于 Work 输出区。显示命令执行图标、命令内容、状态和操作按钮。

| 元素 | 尺寸 | 位置 | 行为 |
|------|------|------|------|
| 图标 | 16x16 | 左侧 | ⚡ 表示执行命令 |
| 命令内容 | auto | 图标右侧 | 显示执行的命令（如 "npm install"）|
| 状态指示 | 12x12 | 右侧 | ● 进行中(绿 spinner) / ✓ 完成(绿) / ✗ 失败(红) / ? 等待确认(蓝) |
| 错误信息 | 宽 100% | 下方 | 失败时显示错误日志，高 ≤80px |
| 重试按钮 | 80x32 | 底部居中 | 失败时显示，点击后重新执行命令 |
...
[AI 执行任务产生输出]
    ...
    SSE 事件携带输出类型
       │
       ├─── type=terminal ──→ TerminalRenderer（终端样式）
       │
       ├─── type=preview ──→ PreviewRenderer（文档预览）
       │
       ├─── type=diff ──→ DiffRenderer（代码 diff）
       │
       └─── type=table ──→ TableRenderer（表格渲染）
```
...
1. **输出面板容器（宽 100%，高 100%）：** 左右分栏布局，左侧终端输出，右侧文档预览（可选）。

| 元素 | 尺寸 | 位置 | 行为 |
|------|------|------|------|
| 终端渲染器 | 宽 50% | 左侧 | 显示命令执行输出，等宽字体，黑色背景 |
| 文档预览 | 宽 50% | 右侧 | 显示 Markdown/JSON 预览，可选显示 |
| 渲染器自动切换 | auto | 根据类型 | 根据 SSE 事件类型自动选择渲染器 |
...
1. **空状态容器（居中）：** 垂直水平居中显示。包含图标、标题、说明文字和按钮。

| 元素 | 尺寸 | 位置 | 行为 |
|------|------|------|------|
| 图标 | 48x48 | 顶部 | ⚡ 闪电图标 |
| 标题 | auto | 图标下方 | "AI 工作台已就绪" |
| 说明文字 | auto | 标题下方 | 两行引导文字 |
| 查看示例按钮 | 120x36 | 底部 | 点击后填充示例任务到输入框 |
...
1. **Diff 容器（宽 100%，高 auto）：** 位于 Work 输出区。显示文件修改的完整 diff 内容，带行号和颜色高亮。

| 元素 | 尺寸 | 位置 | 行为 |
|------|------|------|------|
| 文件名/图标 | auto | 顶部左侧 | 显示文件名和 ✏️ 图标 |
| 状态 | auto | 顶部右侧 | ✓ 已完成 |
| Diff 代码区 | 宽 100% | 中间 | 显示带行号的完整代码，删除行红色背景，新增行绿色背景 |
| 接受按钮 | 80x32 | 底部左侧 | #3DD68C 绿色，确认应用修改 |
| 拒绝按钮 | 80x32 | 底部居中 | #FF6B6B 红色，放弃修改 |
| 回退按钮 | 80x32 | 底部右侧 | #6C9FFF 蓝色，回退原文件 |
...
1. **方案容器（宽 100%，高 auto）：** 居中显示。包含标题、步骤说明、风险评估和三个操作按钮。

| 元素 | 尺寸 | 位置 | 行为 |
|------|------|------|------|
| 标题 | auto | 顶部 | "执行方案" + 📋 图标 |
| 步骤说明 | auto | 标题下方 | 显示执行步骤序号 |
| 风险/耗时 | auto | 步骤下方 | 显示风险等级和预计耗时 |
| 确认执行按钮 | 100x36 | 底部左侧 | #3DD68C 绿色，点击后开始执行 |
| 修改方案按钮 | 100x36 | 底部居中 | #6C9FFF 蓝色，点击后编辑方案 |
| 取消按钮 | 80x36 | 底部右侧 | #FF6B6B 红色，关闭方案展示 |
...
1. **输入容器（宽 100%，高 80px）：** 位于 Work 模式底部。包含多行输入框和两个按钮。

| 元素 | 尺寸 | 位置 | 行为 |
|------|------|------|------|
| 多行输入框 | 宽 100% | 顶部 | 多行文本输入，placeholder "输入任务描述..." |
| Boot Check 按钮 | 100x32 | 底部左侧 | 🔧 图标，点击后打开环境检测进度条 |
| 发送按钮 | 48x48 | 底部右侧 | ▶ 圆形按钮，点击后发送任务 |
...
1. **Boot Check 容器（宽 400px，高 auto）：** 居中弹窗显示。包含标题、进度条和检测项列表。

| 元素 | 尺寸 | 位置 | 行为 |
|------|------|------|------|
| 标题 | auto | 顶部 | "Boot Check 环境体检" |
| 进度条 | 宽 100% | 标题下方 | 显示检测进度（6/9 形式） |
| 检测项（通过） | auto | 进度条下方 | ● 图标 + 项目名 + ✓ 绿色 |
| 检测项（失败） | auto | 进度条下方 | ● 图标 + 项目名 + ✗ 红色 |
| 检测项（进行中）| auto | 进度条下方 | ○ 图标 + "检测中" |
| 检测项（等待） | auto | 进度条下方 | ○ 图标 + "等待" |
...
[用户在任何界面]
    ...
    左 NAV 底部显示模式切换按钮
       │
       ├─── Hover 按钮 ──→ Tooltip 显示当前模式
       │
       └─── 点击按钮 ──→ 切换 Chat ↔ Work 模式
                            │
                            └── 界面 < 150ms 切换完成
```
...
1. **模式切换按钮（40x40）：** 固定于左导航栏底部。icon-only 样式。

| 元素 | 尺寸 | 位置 | 行为 |
|------|------|------|------|
| 切换图标 | 24x24 | 居中 | [↔] 图标，hover 时显示 tooltip |
| Tooltip | auto | 图标上方 | 显示 "Chat" 或 "Work" |
| 点击区域 | 40x40 | 整个按钮 | 点击后 < 150ms 切换模式 |
...
**操作流程：**
```
[用户进入向导 Step 4 - 本地模型安装]
       │
       ▼
    显示本地模型安装选项
       │
       ├─── 启用/关闭 Gemma 本地推理开关 ──→ 开关切换
       │
       ├─── 选择模型版本 ──→ ☑/☐ 复选框选择
       │                      │
       │                      └── 选中后显示下载进度
...
1. **安装容器（宽 100%，高 auto）：** 向导步骤内容区。包含开关、模型选择列表和下载进度。

| 元素 | 尺寸 | 位置 | 行为 |
|------|------|------|------|
| 开关 | 40x20 | 顶部 | 启用/禁用 Gemma 本地推理 |
| 模型列表 | 宽 100% | 开关下方 | 显示可选模型及内存要求 |
| 下载进度条 | 宽 100% | 选中后显示 | 显示下载百分比和来源 |
| Skip 按钮 | 80x32 | 底部右侧 | 跳过此步骤 |
...
[用户进入向导 Step 5 - 连接 IM]
       │
       ▼
    显示 IM 平台列表（Telegram/Discord/WhatsApp）
       │
       ├─── Telegram 已连接（🟢）──→ 显示已连接状态
       │                              │
       │                              └── 可点击断开或重新配置
       │
       ├─── Discord 未配置 + 点击「配置」──→ 打开 Discord 配置面板
       │
       ├─── WhatsApp 未配置 + 点击「配置」──→ 显示扫码绑定二维码
       │
       └─── 点击 Skip ⏭ ──→ 跳过 IM 配置，进入下一步骤
```
...
1. **IM 平台列表容器（宽 100%，高 auto）：** 向导步骤内容区。包含多个 IM 平台卡片。

| 元素 | 尺寸 | 位置 | 行为 |
|------|------|------|------|
| 标题 | auto | 顶部 | "在手机上随时指挥 AnyClaw 📱" |
| Telegram 卡片 | 宽 100% | 列表第1行 | 显示连接状态（🟢已连接），可操作 |
| Discord 卡片 | 宽 100% | 列表第2行 | 显示未配置 + 配置按钮 |
| WhatsApp 卡片 | 宽 100% | 列表第3行 | 显示未配置 + 配置按钮 + 扫码绑定 |
| Skip 按钮 | 80x32 | 底部右侧 | 跳过此步骤 |

**交互细节：**
- 已连接平台显示绿色状态灯，未配置平台显示空心圆
- WhatsApp 支持扫码绑定，扫描后自动完成配置
- Skip 跳过不影响后续向导步骤
- IM 连接状态实时同步到配置
...
[用户进入应用授权界面]
       │
       ▼
    显示待授权应用列表（Outlook/Google Calendar 等）
       │
       ├─── 点击应用右侧「授权」──→ 发起 OAuth 授权流程
       │                              │
       │                              ├─── 授权成功 ──→ 更新状态为已授权
       │                              │
       │                              └─── 授权失败 ──→ 显示错误提示
...
1. **应用授权容器（宽 100%，高 auto）：** 扩展界面内容区。包含应用列表和授权按钮。

| 元素 | 尺寸 | 位置 | 行为 |
|------|------|------|------|
| 标题 | auto | 顶部 | "📱 应用授权" |
| Outlook 行 | 宽 100% | 列表第1行 | 显示图标、名称、推荐方式、授权/拒绝按钮 |
| Google Calendar 行 | 宽 100% | 列表第2行 | 显示图标、名称、推荐方式、授权/拒绝按钮 |
| 优先级说明 | auto | 底部 | "交互方式优先级: CLI > MCP > COM > Skill" |

**交互细节：**
- 每个应用显示推荐的交互方式（CLI/MCP/COM/Skill）
- 授权按钮发起对应协议的授权流程
- 拒绝按钮仅记录拒绝状态，不影响其他应用
- 支持批量授权/拒绝操作
...
| 元素 | 尺寸 | 位置 | 行为 |
|------|------|------|------|
| 遮罩 | 100%x100% | 整个屏幕 | 50% opacity，防止误操作 |
| 标题 | auto | 弹窗顶部 | "🔐 AnyClaw 权限确认" |
| 命令预览 | 宽 100% | 内容区 | 显示待执行的命令内容 |
| 权限类型 | auto | 命令下方 | 显示权限项名称（如 EXEC_SHELL） |
| 拒绝按钮 | 80x32 | 底部左侧 | 红色（#FF6B6B），拒绝操作 |
| 此次授权按钮 | 100x32 | 底部中间 | 蓝色（#6C9FFF），仅本次有效 |
| 永久授权按钮 | 100x32 | 底部右侧 | 绿色（#3DD68C），永久保存 |
| 超时倒计时 | auto | 底部 | ⏱ 60秒超时自动拒绝 |
