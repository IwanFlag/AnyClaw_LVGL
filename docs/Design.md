...
左导航栏内部结构（x=0, y=41, w=77px, h=759px，flex COLUMN 布局）：

**分层结构：**

- **left_nav 容器**（x=0, y=41, w=77px, h=759px）：导航栏根容器，flex COLUMN 布局，内边距 0，背景 panel 色。
- **nav_top 子容器**（flex_grow=0, h 由内容撑起 = 4×52 + 3×gap=16×3 = 256px）：flex COLUMN + START 排列，包含 Bot / File / KB / Skill 四个模块图标按钮，之间各隔 gap=16px。
- **nav_spacer 透明填充**（flex_grow=1）：占据 nav_top 底部到 nav_bot 顶部之间所有剩余空间，使 nav_top 贴顶、nav_bot 贴底。
- **nav_bot 子容器**（flex_grow=0, h 由内容撑起 = 2×52 + 16 = 120px）：flex COLUMN + END 排列，包含 Chat/Work 切换按钮和 Settings 图标，之间隔 gap=16px，容器底部与 left_nav 底部对齐。

**按钮尺寸：** 52×52px，圆角 9px。x 在 nav_top/nav_bot 宽度内居中（flex CROSS_AXIS=CENTER）。
...
│  │  📚   │  │
│  └───────┘  │
│  ┌───────┐  │  ← Skill 图标，52×52，圆角9，y=252（gap=16）
│  │  🧩   │  │
│  └───────┘  │
│             │  ← nav_spacer，flex_grow=1，填充剩余全部空间
│             │
│  ┌───────┐  │  ← Chat/Work，52×52，圆角9，y 靠 nav_bot 顶部
│  │  💬   │  │
│  └───────┘  │
│  ┌───────┐  │  ← Settings 图标，52×52，圆角9，y 贴底
│  │  ⚙️   │  │
│  └───────┘  │
└─────────────┘
```
...
**控件间相邻关系：**
- 左面板顶部 → Bot：y=48，无缝对齐
- Bot → File：y=48+52+16=116px
- File → KB：y=116+52+16=184px
- KB → Skill：y=184+52+16=252px
- nav_top → nav_spacer：nav_top 底部 y=297，flex_grow 填充剩余全部空间
- nav_spacer → nav_bot：nav_bot 顶部 y=680
...
- 圆角从 9px → 8px
- 间距从 16px → 12px
- 6 个图标总高 328px，4 个顶部 + 2 个底部，中间 flex 填充
...
├────┬──────────────────────────────┬──────────────────────┬───────────────┤
│NAV │ 左面板 (x=64, w=216)        │ 中部 (x=280, w=640) │ 右 (x=920,w=360)
│x=0 │ 文件浏览器（可折叠）        │ ┌─ Tab栏 (y=41,h=33)──┐│ AI对话(可折叠)
│w=64│ [AnyClaw ▼][+]            │ │main.cpp│utils.h│🌐feishu│+││ Markdown输出
...
| NAV | 0 | 41 | 64 | 759 | 左导航，5图标 |
| 左面板 | 64 | 41 | 216 | 759 | 项目浏览器 |
...
| 元素 | x | y | w | h | 说明 |
|------|---|---|---|---|------|
| 面板背景 | 64 | 41 | 216 | 759 | fill=#13151E |
| 标题"文件" | 80 | 62 | - | - | font=11, bold |
| 分隔线 | 64 | 70 | 216 | 1 | |
| 项目下拉 | 72 | 76 | 116 | 24 | rx=4, "AnyClaw"+"▼" |
| 新建[+] | 194 | 76 | 22 | 22 | rx=4 |
| 路径 | 72 | 108 | 196 | 16 | fill=#1A1D27 |
| 搜索框 | 72 | 132 | 196 | 22 | rx=4 |
| 项目标签 | 72 | 170 | - | - | font=9, dim |
| 文件树项 | 72 | 178~282 | 196 | 22 | rx=3, src/ + ui_main.cpp(激活) + boot_check.cpp + docs/ + ui/ |
| 折叠按钮 | 280 | 360 | 14 | 40 | ◀, 在中部门口 |
...
| 元素 | x | y | w | h | 说明 |
|------|---|---|---|---|------|
| Tab栏背景 | 280 | 41 | 640 | 33 | fill=#0A0C10 |
| Tab: main.cpp | 280 | 41 | 100 | 33 | rx=0, fill=#13151E |
| ✕ | 372 | 50 | 8 | 14 | |
| Tab: utils.h | 380 | 41 | 80 | 33 | |
| ✕ | 452 | 50 | 8 | 14 | |
| Tab: 🌐 feishu | 460 | 41 | 100 | 33 | |
| ✕ | 552 | 50 | 8 | 14 | |
| +新建 | 568 | 48 | 24 | 20 | |
| 编辑区背景 | 280 | 74 | 640 | 356 | fill=#0F1117 |
| 行号栏 | 280 | 74 | 36 | 356 | fill=#0A0C10 |
| 行号 | 294 | 98~278 | - | - | monospace 11, dim色 |
| 代码 | 324 | 98~258 | - | - | syntax色 |
| 滚动条 | 900 | 74 | 20 | 356 | fill=#0A0C10 |

*分隔线（y=430, h=6）：*
| 元素 | x | y | w | h | 说明 |
|------|---|---|---|---|------|
| 分隔背景 | 280 | 430 | 640 | 6 | fill=#0A0C10 |
| 分隔线 | 280 | 430 | 640 | 2 | fill=#252A38 |
| 手柄 | 590 | 430 | 20 | 6 | fill=#1A1D27, ⋮ |

*下部 - 终端区（y=436~800, h=364）：*
| 元素 | x | y | w | h | 说明 |
|------|---|---|---|---|------|
| Tab栏背景 | 280 | 436 | 640 | 28 | fill=#0D1117 |
| 标签"终端" | 292 | 453 | - | - | font=10, dim |
| 终端1 Tab | 324 | 439 | 70 | 22 | fill=#13151E, 激活有底部accent线 |
| ✕ | 386 | 447 | 8 | 14 | |
| 终端2 Tab | 394 | 439 | 70 | 22 | fill=#1E2230 |
| ✕ | 456 | 447 | 8 | 14 | |
| +新建 | 470 | 441 | 22 | 18 | |
| 终端内容 | 280 | 464 | 640 | 336 | fill=#0D1117 |
