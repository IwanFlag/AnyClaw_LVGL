...
> 父容器：窗口。坐标：x=0，y=TITLE_H=48px，w=NAV_W=87px，h=PANEL_H=752px。
...
│  │    │    │  └─────────────────────┴──────────────────────────┘ ││
│  │    │    │  PANEL_H = WIN_H - TITLE_H - PANEL_GAP = 752         ││
│  └────┴────┴───────────────────────────────────────────────────────┘│
└─────────────────────────────────────────────────────────────────────┘

左导航栏内部结构（x=0, y=48, w=87, h=752）：

┌───────┐
│  🧄   │  ← 品牌 Logo，ICON_LARGE=32px，x=居中(27.5), y=8
│       │
│  ┌──┐ │
│  │🦞│ │  ← Bot 图标，60×60px，圆角10px，x=居中(13.5), y=56
│  └──┘ │
│  ┌──┐ │
│  │📁│ │  ← File 图标，60×60px，圆角10px，x=居中(13.5), y=123
│  └──┘ │
│       │
│ (flex)│  ← nav_top，flex_grow=1，填充 Bot/File 与 nav_bot 之间
│       │
│ ┌────┐│
│ │ > │ │  ← Chat/Work 切换，60×60px，x=居中(13.5), y=564+34=598
│ └────┘│  nav_quick_h=188px，Session/Settings 各60px，居中于188px
│ ┌────┐│  free_space=68px，每次分配 half=34px
│ │ ⚙ │ │  ← Settings 图标，60×60px，x=居中(13.5), y=564+124=688
│ └────┘│
└───────┘
```
...
| 控件 | 宽 | 高 | x | y | 计算公式 |
|------|---:|---:|---:|---:|---------|
| left_nav | 87 | 752 | 0 | 48 | `NAV_W = WIN_W×6%`，`PANEL_H = WIN_H-TITLE_H-GAP` |
...
**第三层：中部模块按钮（nav_top，flex_grow=1）**

| 控件 | 尺寸 | x | y | 说明 |
|------|------|---:|---:|------|
| Bot 按钮 | 60×60 | 居中(13.5) | 56 | `nav_btn_sz = 87×70%=60px`，圆角10px |
| File 按钮 | 60×60 | 居中(13.5) | 123 | `y = 56 + 60 + 7 = 123` |
| Bot/File 间距 | — | — | — | `nav_gap = 87×8%=7px` |

**第四层：底部快捷区（nav_bot，h=188px，y=564）**

| 控件 | 尺寸 | x | y | 说明 |
|------|------|---:|---:|------|
| nav_bot 容器 | 87×188 | 0 | 564 | `nav_quick_h = PANEL_H×25%=752×25%=188px` |
| Chat/Work 切换 | 60×60 | 居中(13.5) | 598 | y=564+(188-60)/2+34=598 |
| Settings 按钮 | 60×60 | 居中(13.5) | 688 | y=598+60+7+34=688 |
| free_space | 68 | — | — | 188-60×2-7×1=68px，half=34px 均分上下 |

**状态样式：**

| 状态 | 样式 |
|------|------|
| 激活 | icon=accent色 + 左侧竖线(4px) + accent_subtle背景 |
| hover | bg=hover_overlay + tooltip |
| 默认 | icon=text_secondary，背景透明 |

**控件间相邻关系：**
- Logo → nav_top：y=8+32+16=56px（16px gap）
- Bot → File：y=56+60+7=123px（7px gap）
- nav_top → nav_bot：flex_grow 填充，中部按钮组与底部图标组自然分隔
- nav_bot 内：Session → Settings，y=598+60+7=688px
...
[用户点击左侧导航 File，进入 File 模块]
   ...
   left_panel 显示 File 内容（搜索框 + 文件列表）
      ...
      ├─── 点击"打开项" ──→ 打开当前选中项
      ├─── 点击"定位" ──→ 在资源管理器中定位选中项
      ├─── 长按行/点击"菜单" ──→ 打开上下文菜单（新建文件夹/重命名/删除）
...
**窗口骨架：** 继承 §12 12.4 Bot 模块通用布局（left_panel ~25% + right_panel ~75%，v2.2.11 移除 splitter）
...
├────┼─────────────────────────────────────────────────────────────┤
│ 🦞*│                                                             │
│    │   ┌─ left_panel (x=95, y=56, w=340, h=698) ────────────┐   │
│    │   │  [Agent Status]  [●]  Running...     ← 状态行 h=30 │   │
│    │   │  Model: MiniMax-M2.7                ← 模型名 h=16   │   │
│    │   │  ─────────────────────────────────────────────────│   │
│    │   │  任务列表  (0)                    ← task_title    │   │
│    │   │  ┌───────────────────────────────────────────────┐│   │
│    │   │  │ 会话                                 (0)       ││   │
...
│    │   │  │ [空状态占位]                                  ││   │ ← list h=120
│    │   │  └───────────────────────────────────────────────┘│   │
│    │   │              Auto-refresh: 2s        ← hint       │   │
│    │   └───────────────────────────────────────────────────┘   │
│    │                                                             │
│    │   [对话气泡 + 控制行 + 输入区...]  ← right_panel           │
└────┴─────────────────────────────────────────────────────────────┘
```
...
[用户点击左导航 Bot，进入 Chat 模式]
       │
       ▼
    显示 Bot · Chat 界面：左导航 Bot 高亮 + left_panel (Session/Cron)
       │
       ├─── 左面板状态行 ──→ LED 绿色 + "Running" / 红色 + "Error"
       │
       ├─── 点击 Session 卡片项 ──→ 右侧加载该会话历史记录
       │                           │
       │                           └── 空会话 ──→ 显示「暂无任务」
       │
       ├─── 点击 Cron 卡片项 ──→ 右侧加载该定时任务历史
       │
       ├─── 在输入区发消息 ──→ 显示用户气泡 + AI思考(折叠) + 流式回复
       │
       ├─── 点击 AI思考标题 ──→ 展开/折叠思考内容
       │
       ├─── 点击控制行下拉 ──→ Agent/Report/AI行为 选择菜单
       │
...
| 左侧面板 | 340 | 698 | 95 | 56 | avail_w×25%=1363×25%=340，PANEL_H=WIN_H-TITLE_H-FOOTER_H-24 |
| 右侧面板 | 1000 | 698 | 443 | 56 | 右侧紧贴左侧面板右边缘 |
...
| 窗口标题文字 | ~200宽 | 52 | 0 | "Bot · Chat"，body字体 |
| 模型下拉 | ~145宽 | ~右 | 5 | WIN_W×10% |
| 窗口控制按钮组 | 28×28×3 | 右侧堆叠 | 10 | 从右往左排列 |

**第三层：左侧面板（x=95, y=56, w=340, h=698）**

| 控件 | 尺寸 | x | y | 说明 |
|------|------:|---:|---:|------|
| 面板容器 | 340×698 | 95 | 56 | 圆角 radius_lg，边框 1px |
| 状态行 lp_status_row | 316×30 | 107 | 68 | Agent Status + LED + 状态文字 |
| 模型名 label | ~200×16 | 107 | 102 | "Model: MiniMax-M2.7" |
| 分隔线 lp_separator | 316×1 | 107 | 132 | sep_y = GAP+30+16+rg = 68 |
| 任务标题 lp_task_title | ~200×20 | 107 | 144 | "任务列表" |
| Session Card | 316×170 | 107 | 189 | card_y = sep_y+1+rg+20 |
| Cron Card | 316×170 | 107 | 371 | card_y+170+12 |
| hint label | auto | 中间 | ~690 | "Auto-refresh: 2s" |

**Session/Cron Card 内部（每张 card 内部结构一致）：**

| 控件 | 尺寸 | x | y | 说明 |
|------|------:|---:|---:|------|
| Card 容器 | 296×170 | 119 | — | w=LEFT_PANEL_W-GAP×2=316，h=170，surface背景 |
| header row | 296×20 | 119 | — | flex_row，标题左/计数右 |
| 标题 label | auto | — | — | "会话" / "Cron"，accent色 |
| 计数 badge | auto | — | — | "(N)"，text_dim色 |
| 分隔线 | 296×1 | 119 | — | card内分隔 |
| list textarea | 296×120 | 119 | — | 只读，多行，行间距2px，滚动 |

**相邻关系：**
- 状态行 → 模型名：y+4px（30+4=34，实际 y=102 = 68+34）
- 模型名 → 分隔线：y+14px（16+14=30，sep_y=68+30=98，代码实际=132）
- 分隔线 → 任务标题：y+12px（1+12=13，task_y=105，代码实际=144）
- Session Card → Cron Card：y+12px gap（189+170+12=371）
- Cron Card → hint：y+12px（371+170+12=553 → 底部对齐到698）
...
| 控件 | 尺寸 | x | y | 说明 |
|------|------:|---:|---:|------|
| 消息区容器 | 1000×593 | 443 | 56 | RIGHT_PANEL_W×CHAT_FEED_H_PCT(85%) |
| 用户气泡 | 最大700宽 | 716 | 随内容 | CHAT_BUBBLE_PCT=70%，靠右 |
| AI 气泡 | 最大700宽 | 443 | 随内容 | CHAT_BUBBLE_PCT=70%，靠左 |
| AI 思考块 | 1000×~变高 | 443 | 紧接气泡 | 折叠显示标题，点击展开 |
| 文件输出块 | 1000×~变高 | 443 | 紧接思考块 | 100%宽 |

**4.2 控制行（y=661，h=36）**

| 控件 | 尺寸 | x | y | 说明 |
|------|------:|---:|---:|------|
| 控制行容器 | 1000×36 | 443 | 661 | 消息区下方，y=56+593+12 |
| Agent 下拉 | ~120×28 | 455 | 665 | 按钮高=36×58%=21，实际28px |
| Report 按钮 | ~72×28 | +6+120 | 665 | gap=6px |
| AI行为 下拉 | ~120×28 | +6+72 | 665 | gap=6px |

**4.3 输入区（y=709，h=36）**

| 控件 | 尺寸 | x | y | 说明 |
|------|------:|---:|---:|------|
| 输入区容器 | 1000×~36 | 443 | 709 | y=661+36+12 |
| 上传按钮 | 29×29 | 455 | 714 | BTN_UPLOAD_PCT(30%)×36=29 |
| 输入框 | ~910×36 | 492 | 709 | 左边=上传+12+间隙 |
| 发送按钮 | 43×43 | 右边缘-43 | 714 | BTN_SEND_PCT(45%)×36=43 |

**控件间相邻关系总结：**
- 标题栏 → 内容区（y=48，无缝衔接）
- 左导航 → 间距(8px) → 左侧面板（x=87+8=95，紧贴间距右边缘）
- 左侧面板 → 右侧面板（x=95+340+8=443，无 splitter）
- 消息区 → 控制行（y=649，紧贴消息区下方+gap）
- 控制行 → 输入区（y=709，紧贴控制行下方+12px）
- 上传按钮 → 输入框（x=455+29+12=496，紧贴上传按钮右边缘+12px间隙）
...
| 区域 | 坐标 | 说明 |
|------|------|------|
| right_panel | x=428, y=48, w=1022, h=752 | Bot Chat 内容区 |
...
| 头像 | 气泡下方 8px 居右 | ICON_LARGE=24×24 | 🔵 图标 |
...
|| 区域 | 坐标 | 尺寸 | 说明 |
|------|------|------|------|
| 弹窗整体 | x=768, y=360, w=1024, h=880 | 居中 | 应用授权弹窗，背景 panel 色，圆角 12px |

**第二层：内容区**

|| 元素 | 绝对坐标 | 尺寸 | 说明 |
|------|---------|---------|------|
| 标题 | x=784, y=376, w=992, h=40 | 顶部 | "📱 应用授权" 文字 |
| Outlook 行 | x=784, y=432, w=992, h=56 | 100% 宽 | 图标+名称+推荐CLI+授权/拒绝按钮 |
| Google Calendar 行 | x=784, y=504, w=992, h=56 | 100% 宽 | 图标+名称+推荐MCP+授权/拒绝按钮 |
| 优先级说明 | x=784, y=576, w=992, h=24 | 底部 | "交互方式优先级: CLI > MCP > COM > Skill" |

**相邻关系：**
- 标题 → Outlook 行：y+16px 间距
- Outlook 行 → Google Calendar 行：y+16px 间距
- Google Calendar 行 → 优先级说明：y+16px 间距
