...
**按钮行为规则：**
- [退出]：点击 → 弹出确认对话框「确定退出向导吗？应用将关闭」，确认 → `exit(0)`
- [上一步]：回到上一个步骤（step > 0 时可见）
- [下一步]：前进到下一个步骤（step < 3 且条件满足时可用）
...
1. **标题区：** 内容区顶部居中放置两行文字。上行为中文标题「请选择您的语言」（18px，粗体），下行为英文副标题「Please select your language」（14px，text_secondary 颜色）。标题区下方有 32px 垂直间距。
...
1. **标题区：** 内容区顶部居中放置一行中文标题「🔧 环境检测与修复」，18px 粗体。标题区下方有 24px 垂直间距。

2. **双面板区：** 标题区下方左右并排两个矩形容器面板，中间有 16px 间距，两个面板总宽度占内容区宽度的 90%，在内容区内部水平居中。
   - **左面板（检测结果）：** 宽度约为双面板总宽的 40%，内部从上到下排列 4 个检测项状态行。每行包含：状态图标（● ✅/⏳/⚠️/❌）+ 检测项名称 + 版本号或状态文字。检测项包括 Node.js、npm、OpenClaw、网络连通性。检测项之间垂直间距 12px。
   - **右面板（修复日志）：** 宽度约为双面板总宽的 58%，内部为一个终端风格日志展示区（monospace 字体，surface 背景色），实时显示检测和修复过程的命令行输出。日志自动滚动到底部，支持选中复制。

3. **按钮区：** 双面板区下方水平居中排列两个按钮：「自动修复」（accent 样式，主要按钮）和「稍后手动处理」（secondary 样式，次要按钮）。两按钮间距 16px，按钮高度 40px。

4. **提示行：** 按钮区下方一行小字提示「扫描结果自动刷新 · 失败时显示错误摘要 + [重试]」，14px，text_secondary 颜色。

**检测项状态语义：**
- `✅ 就绪`：绿色 check-circle + 版本号 + 「已就绪」
- `⏳ 检测中`：蓝色 spinner + 「检测中...」
- `⚠️ 需处理`：黄色 alert-circle + 「需处理」
...
│  │   推荐模型                                                             │  │
│  │   ┌──────────────────────────────────────────────────────────────┐   │  │
│  │   │  ●  MiniMax Text   MiniMax   v2.8   ★ 4.8                   │   │  │  ← 推荐项（默认选中）
│  │   └──────────────────────────────────────────────────────────────┘   │  │
│  │   ┌──────────────────────────────────────────────────────────────┐   │  │
│  │   │     Claude 4 Sonnet   Anthropic  v4.0   ★ 4.9               │   │  │  ← 其他选项
│  │   └──────────────────────────────────────────────────────────────┘   │  │
│  │   ┌──────────────────────────────────────────────────────────────┐   │  │
...
│  │   API Key                                                             │  │
│  │   ┌──────────────────────────────────────────────────────────────┐   │  │
│  │   │  sk-xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx                │   │  │  ← 密码输入框
│  │   └──────────────────────────────────────────────────────────────┘   │  │
│  │   ○ Key 将安全存储，仅本地使用                                         │  │
│  │                                                                       │  │
│  └──────────────────────────────────────────────────────────────────────┘  │
...
[进入 Step 2]
       ...
       ▼
    加载推荐模型列表（默认选中第一项）
    [下一步] 按钮初始状态 = 禁用（API Key 未填写）
       │
       ▼
    用户在 API Key 输入框中输入内容
       │
       ├─── Key 长度 < 10 字符 ──→ [下一步] 保持禁用
       │
       ├─── Key 格式初步校验失败（为空/纯空格）────────────────────────┐
       │                                                             │
       │   输入框下方显示红色错误提示「请输入有效的 API Key」           │
       │                                                             │
       └─── Key 初步格式有效（≥10 字符）──────────────────────────────┘
                                                              │
                                                    [下一步] 变为可用状态
                                                              │
                                    用户点击 [下一步] ──→ 验证 Key（网络请求）
                                                              │
                                            ├─── 验证成功 ──→ 进入 Step 3
                                            │
                                            └─── 验证失败
...
1. **标题区：** 内容区顶部居中放置两行文字。上行为主标题「🤖 选择您的 AI 模型」（18px 粗体），下行为副标题「连接您的 API Key 即可开始聊天」（14px，text_secondary 颜色）。标题区下方有 24px 垂直间距。
...
3. **API Key 输入区：** 模型列表区下方，有 24px 垂直间距。显示标签文字「API Key」（14px 粗体），下方为文本输入框（密码样式，字符显示为 ●），输入框高度 40px。输入框下方有一行小字提示「○ Key 将安全存储，仅本地使用」（14px，text_secondary 颜色）。当 Key 为空时提示文字前导符号为 ○（灰色）；Key 填写后变为 ✓（success 色）。

4. **错误提示区（条件显示）：** 仅在 Key 验证失败时出现。输入框下方显示一行红色错误文字（如「Key 无效，请检查后重试」），14px，danger 颜色。

**[下一步] 按钮启用条件：** 输入框中 Key 字符数 ≥ 10 时可用。点击后触发 Key 验证网络请求，验证通过才进入 Step 3。
...
│  │                                                                       │  │
│  │  ┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐    │  │
│  │  │  💾 本地模型     │  │  💬 接入 IM     │  │  🤖 Leader 模式  │    │  │
│  │  │  Gemma 离线推理 │  │  飞书/TG/Discord│  │  多 Agent 并行   │    │  │
│  │  │  [配置]         │  │  [连接]         │  │  [了解]          │    │  │
│  │  └─────────────────┘  └─────────────────┘  └─────────────────┘    │  │
│  └──────────────────────────────────────────────────────────────────────┘  │
│                                                                              │
...
[进入 Step 3]
    ...
    显示完成页内容（绿色成功色调）
       ...
       ├─── 用户点击 [进入主界面 →] ───────────────────────────┐
       │                                                           │
       │   保存 wizard_completed=true                             │
       │   关闭向导模态覆盖层                                     │
       │   进入主界面（Bot · Chat 模式）                          │
       │                                                           │
       ├─── 用户点击探索能力卡片（可选，不影响主流程）──┐          │
       │                                             │          │
       │   本地模型卡片 → 弹出配置指引               │          │
       │   IM 连接卡片 → 弹出连接配置               │          │
       │   Leader 模式卡片 → 显示说明弹窗            │          │
...
1. **成功标题区：** 内容区顶部垂直居中放置成功标识和标题。上方为大型成功图标 🎉（64px），中间为主标题「恭喜！环境已就绪」（24px 粗体），下方为两行副标题（「您已具备与 AI 对话的能力」+「点击下方按钮进入主界面」），14px，text_secondary 颜色。标题区在内容区内部垂直居中（使用 flex 布局使其占据中部空间）。

2. **主按钮区：** 标题区下方放置一个大型居中按钮「进入主界面 →」，按钮宽度 240px，高度 48px，accent 背景色，白色文字，按钮内文字水平垂直均居中。按钮有 hover 状态（颜色加深）和点击反馈（scale 0.98）。

3. **探索能力区（可选卡片）：** 主按钮区下方，有 32px 垂直间距。区域内水平排列 3 张能力推荐卡片（本地模型 / IM 连接 / Leader 模式），每张卡片宽度约 180px，高度 120px，panel 背景色 + border 边框，圆角 8px。卡片内部：顶部图标（💾/💬/🤖），中间为能力名称（16px 粗体），下方为简短描述（12px，text_secondary），底部为操作按钮（[配置]/[连接]/[了解]）。这些卡片为非阻塞推荐，不影响用户点击「进入主界面」。

**完成后行为：** 用户点击「进入主界面」→ 标记 `wizard_completed=true`（写入配置文件）→ 销毁向导模态覆盖层 → 显示主界面（Bot · Chat 模式）。
