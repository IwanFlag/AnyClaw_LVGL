...
**比例参考尺寸：** 1190 × 646（UI-11 基准尺寸）。SVG 作为比例示意图，实际渲染尺寸由代码决定。viewBox 和 width/height 统一用 `0 0 1190 646`。
...
宽度 82px，高度 605px（646 - 41），背景 `#0A0C10`。图标圆角矩形 w=52, h=52, rx=9，x=11。
...
3. 确定 viewBox=`0 0 1190 646`，建立 SVG 骨架
...
> 父容器：窗口。坐标：x=0，y=TITLE_H=41px，w=NAV_W_PCT=6%（1190px 屏≈71px，1920px 屏≈115px），h=PANEL_H=605px。
...
> ⚠️ 实际代码使用百分比/flex 布局，以下为 1190px 基准屏的典型值。精确值随窗口大小自动缩放。

||| 控件 | 宽 | 高 | x | y | 计算公式 |
|------|---:|---:|---:|---:|---------
| left_nav | 82px | 605 | 0 | 41 | `NAV_W_PCT(7%) × 1190 = 82px` |
| nav_top | 82px | 256 | 0 | 41 | 4×52 + 3×16 = 256px |
| nav_spacer | 82px | flex_grow=1 | 0 | 304 | y = nav_top bottom |
| nav_bot | 82px | 120 | 0 | 526 | 2×52 + 22 = 126px，flex END 贴底 |
...
**Bot 模块通用子元素（LVGL 像素值，基准 1190×646）：**
...
| 左导航 | x=0, y=41, w=82, h=605 | Bot 高亮（SVG 青虫子），6个图标，上下部对齐面板 |
| 左面板 | x=82, y=48, w=276, h=598 | 左侧面板主体 |
| AI伙伴容器 | x=92, y=58, w=256, h=203 | 顶部 SVG 青虫子 + Voice开关 + 右侧 Agent Status + Model |
| Agent Status | x=92, y=240, w=256, h=20 | 绿色LED + Running，紧贴容器底部 |
...
| 控件 | 宽 | 高 | x | y | 计算公式 |
|------|---:|---:|---:|---:|---------|
| 窗口 | 1190 | 646 | 0 | 0 | WIN_DEFAULT_W × WIN_DEFAULT_H |
| 标题栏 | 1190 | 41 | 0 | 0 | TITLE_H_PCT(6%) × 646 ≈ 41px |
| 左导航 | 82 | 605 | 0 | 41 | NAV_W_PCT(7%) × 1190 ≈ 82px |
| 间距 | 7 | — | 82 | — | PANEL_GAP = 7px |
| 左侧面板 | 276 | 598 | 82 | 48 | w = 1190 - NAV_W - PANEL_GAP - RIGHT_PANEL |
| 右侧面板 | 818 | 605 | 365 | 41 | 1190 - 82 - 7 = 818 |
