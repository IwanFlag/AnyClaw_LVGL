...
**比例参考尺寸：** 1190 × 646（UI-11 基准尺寸）。SVG 作为比例示意图，实际渲染尺寸由代码决定。viewBox 和 width/height 统一用 `1190 646`。
...
背景 `#13151E`，圆角 rx=10，边框 stroke=`#3DD68C` @ 4%，stroke-width=1。内边距 pad=24px（四边相同）。

#### 5.4.7 按钮

| 类型 | 填充 | 文字 | 高度 | 圆角 | 最小宽度 |
|------|------|------|------|------|---------|
| 主要（accent） | `#3DD68C` @ 15% | `#3DD68C`, 600 | 36px | rx=11 | 90px |
| 次要（secondary） | `#252A38` | `#8B92A8` | 36px | rx=11 | 90px |
| 危险（danger） | `#EF4444` | `#FFFFFF`, 600 | 36px | rx=11 | 90px |
...
Panel 内 pad=24px，三列合计 200+8+240+20+80 = 548px（居中对齐面板内）。

#### 5.4.11 弹窗（Modal）

遮罩 `#000000` @ 50-60%，背景 `#1A1D27`，边框 stroke=`#3DD68C` @ 15%，圆角 rx=12。

#### 5.4.12 绘图流程

1. 读 Design.md 对应 UI 章节，读布局描述和坐标表
2. 读 UI-11-chat-mode.svg 作为 V1 风格基准
3. 确定尺寸：viewBox=`0 0 1190 646`
4. 画 Header：高度 41px，按规范画 Logo + 窗口控制
5. 画 Content：按 Design.md 坐标填入实际值
6. 画 Footer：高度 52px，按钮 y=610
7. 加版本标记：右下角 `Matcha v1 · UI-XX`，8px，`#3DD68C` @ 30%
