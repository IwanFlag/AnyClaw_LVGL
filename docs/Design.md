...
**结构：** 标题栏(48px) + 横向步骤条 + 内容区(剩余高度) + 底部按钮栏
...
| UI-05 | 环境检测 & 安装 — 左半:检测结果LED + 右半:实时终端日志 + 安装按钮组 |
...
| UI-09 | 确认 & 启动 — 配置摘要卡片 + "Get Started" 主按钮 |
...
三区域结构：左导航栏(56px) + 标题栏(48px) + 内容区(剩余空间)

```
┌────┬──────────────────────────────────────────────────────────────────┐
│ 🧄 │ 🧄 Bot · Chat    [ Chat | Work ]            ●  [⚙] [✕]       │ 标题栏 48px
├────┼──────────────────────────────────────────────────────────────────┤
│    │                                                                  │
│ 🦞 │  ← Bot                                                          │
│ ⚡ │  ← 任务    内容区（跟随左导航切换模块）                            │
...
| 状态 | 样式 |
|------|------|
| 激活 | icon=accent 色 + 左侧 3px accent 竖线 + accent_subtle 背景 |
...
| [ Chat \| Work ] | Chat/Work 模式切换，仅 Bot 模块显示 |
| ● | Gateway 状态灯（绿=Ready / 黄=Busy / 红=Error），hover 显示模型名 |
| ⚙ | 打开设置 |
| ✕ | 关闭到托盘 |
...
| 左导航 | 标题栏显示 | 额外控件 |
|--------|-----------|---------|
| 🦞 Bot | `Bot · Chat` / `Bot · Work` | [Chat\|Work] 胶囊 |
| ⚡ 任务 | `任务` | [+] 新建任务 |
| 📁 资源 | `资源` | — |
...
**Session 面板（左侧 200px）：** Session 列表（上）+ Cron 列表（下），可拖拽调整宽度。
**消息区（右侧）：** 用户消息靠右，AI 回复靠左，最大宽度 70%。
**输入区（底部固定）：** 📎 附件 + 🎤 语音 + 多行输入 + 发送按钮(40px 圆形)。
**模式控制栏（输入框下方）：** AI 模式 Dropdown（自主/Ask/Plan）。
...
**与 Chat 模式的关系：** 右侧面板（步骤流 + 输入框）位置与 Chat 模式的（消息气泡 + 输入框）完全一致。输入框始终在右下角。
**左侧输出区：** Work 模式独有，显示终端/文档/代码输出。
...
```
  ┌────────────────────────────┐
  │  AI 模式: [自主 ▾]         │  ← 输入框下方，Dropdown 三选一
  └────────────────────────────┘

  位置: 输入框下方水平栏，与 [Chat|Work] 切换并排
...
Chat/Work 切换在标题栏，AI 模式在输入框下方。输入区底部布局：

```
  ┌──────────────────────────────────────────────────────────┐
  │ [📎] [🎤] │ 输入消息...                            [▶] │  ← 输入行
  │ [ Chat | Work ]              AI 模式: 自主 ▾             │  ← 模式控制行
  └──────────────────────────────────────────────────────────┘

  输入行: 📎附件 + 🎤语音 + 多行输入 + 字数 + 发送按钮(40px圆形)
  模式控制行: 左侧 Chat/Work 切换 + 右侧 AI 模式 Dropdown
```

> **注意：** 标题栏的 [Chat|Work] 和输入区的 [Chat|Work] 是同一个控件的两种显示位置。
> 标题栏版在 Bot 模块始终显示；输入区版作为辅助入口，与 AI 模式放在一起方便操作。
