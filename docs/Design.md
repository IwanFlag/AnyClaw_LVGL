...
[用户点击左侧导航 Bot → 切换到 Work 模式]
       │
       ▼
    显示主界面：左侧 StepCard 面板 + 右侧输出区 + 控制行 + 输入区
       │
       ├─── 查看左侧 StepCard 步骤流状态 ──→ ● 进行中/✓ 完成/✗ 失败/? 待确认
       │                                                              │
       ├─── 在输入区输入任务描述，按发送 ──→ AI 开始执行，步骤流实时更新
       │                                                              │
       ├─── 点击写操作（✏️）卡片的 [接受/拒绝/回退] ──→ 确认/取消/撤销写入
       │                                                              │
       └─── 任务完成后查看终端输出 ──→ 折叠步骤流，查看最终结果
```

**布局结构（内容区从上到下共 3 层）：**

1. **输出区（flex-grow）：** 占据主区域，显示步骤流/对话气泡/终端输出。垂直滚动。
...
| 元素 | 尺寸 | 位置 | 行为 |
|------|------|------|------|
| 左侧 StepCard 面板 | LEFT_PANEL_W × PANEL_H | 根容器左侧 | 步骤流列表，实时更新 |
| 输出区 | flex-1 | 右侧主区域 | 垂直滚动容器 |
| 步骤流容器 | 100% 宽度 | 输出区顶部 | 垂直排列 StepCard |
| StepCard | 100% × auto | 步骤流内 | 状态图标 + 描述 + 操作按钮 |
| 终端输出块 | 100% 宽度 | 步骤流下方 | 顶部标签，code 字体 |
| 控制行 | 100% × BTN_H | 输出区下方 | 固定高度，按钮组 |
| 输入区 | 100% × CHAT_INPUT_MIN_H | 界面最底部 | 固定高度，自动增长 |
