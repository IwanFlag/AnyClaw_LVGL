...
**安装方式：**
- Auto Install：自动串联 Node.js + OpenClaw 安装（需要网络）
- 本地包：用户指定已安装路径
- 离线包：从 `bundled/` 目录安装，无需网络
...
| 场景 | 处理 |
|------|------|
| 有网 + Auto Install 成功 | 正常流程 |
| 有网 + Auto Install 失败 | 回退到 bundled/ 离线安装 |
| 无网 + bundled/ 存在 | 自动离线安装 |
| 无网 + bundled/ 缺失 | 引导用户手动安装或指定已安装路径 |
| 用户已有安装 | 检测 PATH 或用户指定路径，跳过安装 |
...
| 区域 | 宽度 | 说明 |
|------|------|------|
| 左导航栏 | 56px 固定 | icon-only 模块切换：Bot / 任务 / 资源 + 底部：会话 / 设置 |
| 标题栏 | 全宽 48px | 品牌 + 模块名 + [Chat\|Work] 切换（仅 Bot） + 状态灯 + 窗口控制 |
| 内容区 | 剩余空间 | 跟随左导航切换模块内容 |
| Session 面板 | 200px（可拖拽） | 仅 Bot 模块显示，在内容区左侧 |
| 输入区 | 底部固定 | 📎附件 + 🎤语音 + 输入框 + 发送按钮 + AI 模式 Dropdown |

**左导航栏模块：**

| 模块 | 位置 | 功能 |
|------|------|------|
| 🦞 Bot | 顶部 | AI 对话入口，Chat/Work 双视图 |
| ⚡ 任务 | 顶部 | 任务调度中心（活跃任务 + 定时任务 + 历史） |
| 📁 资源 | 顶部 | 工作区文件 + 知识库 + Skills（Tab 切换） |
| 📂 会话 | 底部 | 点击滑出 Session 面板 |
| ⚙ 设置 | 底部 | 覆盖内容区显示设置页 |
...
| 模式 | 左侧区域 | 右侧区域 | 底部 |
|------|---------|---------|------|
| Chat | Session 面板(200px) | 消息气泡 | 输入框 + AI 模式 |
| Work | Session 面板(200px) | 步骤流 | 输入框 + AI 模式 |
| Work（展开输出） | Work 输出面板 | 步骤流 + 输入框 | 同上 |
