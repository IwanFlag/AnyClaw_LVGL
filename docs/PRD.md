...
| v2.3 | 三 Agent 并行（OpenClaw Leader + Hermes + Claude Code） | 🔧 规划中 |
...
| Step 2 | Agent 安装（OpenClaw 必须 / Hermes 可选 / Claude CLI 可选） | 是 | OpenClaw 未安装则无法聊天 |
| Step 3 | 模型 & API Key | 是 | 未配置则无法聊天 |
| Step 4 | 本地模型（Gemma E4B/26B） | 否 | Skip 后仅使用云端模型 |
| Step 5 | 连接 IM（飞书/Telegram/Discord） | 否 | Skip 后仅桌面端可用 |
| Step 6 | Leader 模式确认（默认开启，并行运行三个 Agent） | 是 | — |
| Step 7 | 个人信息 & 确认（昵称/时区） | 是 | — |
...
|| 区域 | 宽度 | 说明 |
|------|------|------|
| 左导航栏 | 56px 固定 | icon-only 模块切换：Bot / 任务 / 资源 / 设置 |
| 标题栏 | 全宽 48px | 品牌 + 模块名 + 窗口关闭 [✕]（⚙ 已移至左导航栏底部，LED 状态灯已移除） |
| 内容区 | 剩余空间 | 跟随左导航切换模块内容 |
| 左侧面板 | 200px 固定 | 仅 Bot 模块显示，内容区左侧，自动刷新 |
| 控制行 | 右侧面板顶部 | Chat: `[AI行为 ▼]  [Agent ▼]  [AI托管 ▼]  [Text | Voice]`；Work: `[AI行为 ▼]  [Agent ▼]  [AI托管 ▼]`，共用 |
| 输入区 | 底部固定 | Text 模式：`📎附件 + 输入框 + 发送按钮`；Voice 模式：隐藏 |

**左导航栏模块（垂直排列）：**

| 区域 | 模块 | 功能 |
|------|------|------|
| 顶部 | 🦞 Bot | AI 对话入口，Chat/Work 双视图 |
| 顶部 | ⚡ 任务 | 任务调度中心（活跃任务 + 定时任务 + 历史） |
| 顶部 | 📁 资源 | 工作区文件 + 知识库 + Skills（Tab 切换） |
| 底部 | [↔] Chat/Work 切换 | 单按钮图标切换，位于 ⚙ 上方，切换延迟 < 150ms，共享上下文 |
| 底部 | ⚙ 设置 | 覆盖内容区显示设置页，位于最底部 |

> ⚙ 设置图标在左导航栏底部，**不在标题栏**。Chat/Work 切换与 ⚙ 设置上下垂直排列，不并排。

**Bot 模块布局（无分隔条）：**

||| 左侧面板（固定） | 右侧区域 |
|------|---------|---------|
| Chat | Session 列表 + Cron 列表 | 控制行 + 输出区 + 输入区（Text 模式）/ transcript（Voice 模式） |
| Work | StepCard 步骤流 | 控制行 + 输出区 + 输入区 |

**Chat 模式控制行：**
```
[AI行为 ▼]  [Agent ▼]  [AI托管 ▼]  [Text | Voice]
```
...
> 详细尺寸规范见 Design.md §12。

**输出区域内容类型（三种区域，视觉明确区分）：**

| 类型 | 位置规则 | 视觉样式 |
|------|---------|---------|
| AI/用户对话 | 输出区顶部，AI 靠左气泡，用户靠右气泡，像 QQ 那样左右分明 | 气泡样式，气泡内文字，AI 气泡在左侧，User 气泡在右侧 |
| AI 思考过程 | 输出区中部，紧接对话气泡下方，折叠显示 | 折叠块，标题 `[AI 思考]` + 缩进灰色背景 `surface` 色，可点击展开/收起 |
| AI 文件输出 | 输出区底部，终端/代码块，跟在思考块下方 | 代码块/终端样式，带语言标签（`terminal` / `python` 等），背景 `surface` 色，等宽字体 |

> 三种区域上下排列，之间有明确分隔（横线或间距），用户可一眼识别内容类型。

**左侧面板：**
- Chat：Session 列表 + Cron 列表，自动刷新（2s 间隔），固定宽度
- Work：StepCard 步骤流，自动刷新（2s 间隔），固定宽度
...
| 窗口宽度 | 左导航 | Session 面板 | 输入区 |
|---------|--------|-------------|--------|
| > 1000px | 56px | 200px 正常 | 正常 |
| 800~1000px | 56px | 折叠为 📂 抽屉按钮 | 正常 |
| < 800px | 56px（icon缩小） | 隐藏 | 紧凑 |
...
model 固定为 `openclaw:main`，由 Agent 路由到配置的 Agent。
...
**上下文窗口管理：**
- AnyClaw 每次请求仅发送当前消息（+ system prompt），上下文维护由 Agent/Agent 运行时负责
...
语音模式是 Chat 模式下的一个子模式，通过控制行右侧的 `[Text | Voice]` 切换。
...
**两种输入模式：**
- **Text 模式**：🎤录音按钮 + 文字输入框（现有异步语音输入已废弃）
- **Voice 模式**：无输入框，实时语音双向通话

**Voice 模式交互：**
1. 用户点击控制行的 `[Voice]`，切换到语音模式
2. 输出区域显示实时 transcript，双方对话实时滚动（AI 靠左，用户靠右，气泡样式同 Text 模式）
3. 用户说话 → 实时 STT 转文字显示在输出区 + 流式 TTS 语音输出
4. AI 回复实时 STT 显示 + TTS 语音返回，用户可随时打断
5. 再次点击 `[Text]` 切回文字模式，输入框恢复

**技术方案：**
- 实时 STT：WebRTC 语音引擎（如 Vosk/Kaldi）或云端实时 API
- 实时 TTS：流式 TTS（云端 API）或本地 TTS 引擎
- 全双工通话：同时听和说，不用等对方说完
- 语言：跟随界面语言设置，支持中英文混合
...
| 控制行 | Agent 行为控制 | `[AI行为 ▼]  [Agent ▼]  [AI托管 ▼]` |
...
| Agent | 路由到配置的 Provider 和模型 | `openclaw.json` |
