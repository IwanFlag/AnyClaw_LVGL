...
|| 区域 | 宽度 | 说明 |
|------|------|------|
| 左导航栏 | 窗口宽 × 6%，最小 36px | icon-only 模块切换：Bot / 任务 / 资源 / 设置 |
| 标题栏 | 全宽 48px | 品牌 + 模块名 + 窗口关闭 [✕]（⚙ 已移至左导航栏底部，LED 状态灯已移除） |
| 内容区 | 剩余空间 | 跟随左导航切换模块内容 |
| 左侧面板 | 可用宽 × 25%，最小 160px | 仅 Bot 模块显示，内容区左侧，自动刷新 |
| 控制行 | 右侧面板顶部 | `[Agent ▼] [Report] [AI行为 ▼]`，Chat/Work 共用，AI托管下拉和 Text\|Voice 按钮已移除 |
| 输入区 | 底部固定 | `📎附件 + 输入框 + 发送按钮` |
...
| 顶部 | 📁 文件 | 搜索 + 浏览工作区文件列表（仅当前工作区单目录范围） |
| 底部 | [↔] Chat/Work 切换 | 单按钮图标切换，位于 ⚙ 上方，切换延迟 < 150ms，共享上下文 |
| 底部 | ⚙ 设置 | 覆盖内容区显示设置页，位于最底部 |

> File 模块的目录语义与 VS Code“打开文件夹/工作区”一致：任一时刻只绑定一个当前工作区根目录，文件搜索与列表仅作用于该目录及其子目录；默认按目录树层级展示，切换工作区后立即重载。
> File 页面提供工作区切换入口（"切换..."），可直接改当前工作区目录。
> File 页面提供目录树“折叠/展开”按钮，可在全部折叠与全部展开之间切换，同时支持每个目录独立展开/收起。
> File 页面文件项支持单击选中、双击打开（系统默认程序）。
> File 页面提供“刷新”“打开”“打开项”“定位”快捷入口，分别用于重扫工作区、打开工作区目录、打开当前选中项、在资源管理器定位当前选中项。
> File 页面提供状态信息行，实时显示目录/文件计数、搜索匹配数与当前选中项。
> File 页面支持上下文菜单（长按行或点击“菜单”）：新建文件夹、重命名、删除。
...
||| 左侧面板（固定） | 右侧区域 |
|------|---------|---------|
| Chat | Session 列表 + Cron 列表 | 控制行 + 输出区 + 输入区 |
...
|---------|--------|-------------|--------|
| > 1000px | 6%（min 36px） | 可用宽 × 25%（min 160px） | 正常 |
| 800~1000px | 6%（min 36px） | 折叠为 📂 抽屉按钮 | 正常 |
| < 800px | 6%（min 36px，icon缩小） | 隐藏 | 紧凑 |
...
支持全局快捷键操作，方便用户无鼠标快速切换模块、模式和窗口状态。
...
| `Ctrl+2` | 切换到 Files 模块 |
| `Ctrl+Tab` | 循环切换导航模块 |
| `Ctrl+T` | 切换 Chat/Work 模式（Bot 模块内） |
...
|--------|------|
| `Ctrl+,` | 打开设置界面 |
| `Ctrl+Shift+T` | 循环切换主题（Matcha→Peachy→Mochi） |
| `Ctrl+N` | 新建对话（Bot·Chat 模式，清空输入框） |
