...
| Step 6 | Leader 模式确认（默认开启，并行运行三个 Agent） | 否 | Skip 后降级为单 Agent 模式 |
...
**Step 2 交互细节（安装卡片）：**
- OpenClaw 卡片（必选）：状态灯 + 版本号 + `[在线安装（推荐）] [bundled/离线安装] [手动指定路径]`
- Hermes 卡片（可选）：`未安装/已安装` 状态 + `[安装] [卸载]`，安装过程显示进度条与日志
...
| Agent | OpenClaw/Hermes/Claude CLI 安装状态、启用开关、路径与端口配置、卸载入口 |
