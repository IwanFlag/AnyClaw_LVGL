...
| 控制行 | 右侧面板顶部 | `[Agent ▼]  [Open Report]  [AI行为 ▼]`，Chat/Work 共用 |
...
|| 类型 | 视觉区分 |
|------|---------|
| AI/用户对话 | 气泡样式（用户靠右，AI 靠左） |
| AI 思考过程 | 折叠块，标题 `[思考]` + 缩进灰色背景 |
| AI 文件输出 | 代码块/终端样式，带语言标签 |
...
| 控制行 | Agent 行为控制 | `[Agent ▼]  [Open Report]  [AI行为 ▼]` |
