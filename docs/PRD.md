...
| BootCheck | 全面环境体检 | 10 | 启动时自动运行（无需手动触发） |

**启动页面形态（UI-01）：**
- 启动时进入 Boot Check 页面，页面尺寸与主界面一致（同窗口尺寸），不是全屏独占系统桌面。
- 页面展示统一状态：`Pass / Fail / Unknown`，并展示关键运行时版本与可用性。
- 检查项包含：Node.js、npm、Network（google.com）、OpenClaw、Hermes、Claude Code CLI、Disk Space、Port 18789、SDL2.dll、API 联通性（api.minimaxi.com）。
...
|| Step 4 | 完成页（昵称 + 时区 + Leader 模式 + 进阶能力推荐） | 是 | — |

**阶段 B：进阶能力引导（推荐任务清单，非阻断）**

| 能力 | 说明 |
|------|------|
| 飞书 Bot | 连接飞书群，实现手机远程指挥 AnyClaw |
| 桌面控制 | 接管鼠标键盘，执行桌面自动化任务 |
| 语音交互 | 语音输入 + AI 语音回复 |
...
| 跳过项 | 可用功能 | 不可用功能 |
|--------|---------|-----------|
| 飞书 Bot | 桌面端 Chat/Work 全部功能 | 手机远程指挥 AnyClaw |
| 桌面控制 | 云端模型聊天、Work 模式全部功能 | 桌面自动化、无网时 AI 不可用 |
| 语音交互 | 文字聊天全部功能 | 语音输入/输出 |

**Leader 模式（默认开启）：**
- OpenClaw 作为入口接收消息，自动拆分任务并行分发给 Hermes 和 Claude
- Step 4 提供单选：启用 Leader 模式（多 Agent 并行，推荐）/ 跳过（单 Agent，仅 OpenClaw）
- 跳过后切换为单 Agent 模式，可后续在 `[Agent ▼]` 中重新开启
...
6. Step 4（完成页）填写昵称（选填）、选择时区、选择 Leader 模式后点击 Footer「开始使用」即可进入主界面。
...
### 4.7 语音模式（实时语音交互）
- **功能编号：** VOICE-01 **优先级：** P1 **状态：** 🔧 部分实现（V1.1 开发中）

**控制行 Voice 开关（Chat / Work 通用）：**

```
┌──────────────────────────────────────────────────────────┐
│  [Agent ▼]  [AI行为 ▼][AI托管 ▼][Voice开关]  [字数]    │
└──────────────────────────────────────────────────────────┘
```

**Voice 开关：** 控制语音交互模式的开/关。

- **关闭时：** 仅文字输入输出。
- **打开时：** 启用语音闭环：用户语音转文字后进入聊天输出链路，AI 回复先文字入聊天输出，再触发语音播报。

**AI 人物容器喇叭开关（新增强约束）：**

| 控件 | 位置 | 语义 |
|------|------|------|
| 喇叭开关（SPK ON/OFF） | 左侧 AI 人物容器（Agent Card） | 控制 AI 回复是否语音播报 |

- **喇叭关闭（OFF）：** AI 仅文字回复，不触发 TTS。
- **喇叭打开（ON）：** AI 回复为“文字 + 语音”双通道，且文字必须先显示再播报。
- 喇叭开关不影响文本对话主链路，任何状态下都必须保证文本可见。

**参考实现边界（必须遵守）：**

- 语音流程可参考 `D:\workspace\AnyClaw\AI Voice\openclaw_voice` 的交互思路（录音/VAD/STT/LLM/TTS）。
- AnyClaw 主程序实现语言必须为 C/C++（LVGL/Win32 现有栈），不引入 Python 运行时作为产品依赖。
- Python 仅可用于离线调研与算法对照，不可作为发布版主链路。

**打开时的行为（与 `D:\workspace\AI\doc\MiniMax27-Voice语音助手-完整方案.md` 联动）：**

|| 组件 | 说明 |
|------|------|------|
| 麦克风采集 | 持续录音 16kHz 单声道，能量阈值 VAD 检测 | |
| 语音激活检测 | 静音 > 0.2s 且语音段 > 0.3s 时触发识别 | |
| STT 语音转文字 | faster-whisper base（CPU int8，本地离线） | |
| LLM 对话 | MiniMax Chat API（`MiniMax-M2`） | |
| TTS 语音合成 | MiniMax TTS（`speech-2.8-hd`），女声音色 | |
| 音频播放 | sounddevice OutputStream，支持打断 | |
| AI 人物联动 | Voice 开关打开时，AI 人物动画响应（嘴型动画 / 耳朵闪烁 / 情绪表情），详见本节 AI人物联动说明 | |

**文本优先显示（强约束）：**

| 流程节点 | 必须行为 |
|------|------|
| 用户语音 STT 成功 | 先生成用户文本并进入 Chat 输出区（用户气泡），再走与键入一致的消息发送链路 |
| AI 生成回复 | 先将 AI 文本进入 Chat 输出区（AI 气泡/流式区域），再启动 TTS 播报 |
| AI 语音播放 | 语音内容必须来源于已显示文本，不允许“仅语音不落字” |
| STT/TTS 失败 | 保留文本链路，给出错误提示，不阻断 Chat 文本会话 |

**Voice 模式最低验收（VOICE-01-AC）：**

1. Voice 打开后，用户说话完成 12s 内，能在聊天输出区看到对应用户文本（含时间戳）。
2. AI 回复必须先在聊天输出区可见，再进行 TTS 播放。
3. 关闭 Voice 后，任何 TTS 不再自动播放，聊天保持纯文本。
4. Voice 打开状态下，STT 失败时可回退手工文本输入，不出现卡死状态。
5. 当喇叭关闭时，AI 回复必须保持文字输出，但不得触发语音播放。
6. 当界面最小化/隐藏到托盘时，若 Voice 与喇叭均开启，AI 回复应继续后台语音播报，并保持文本记录完整可回看。

**AI 人物联动说明：**

Voice 开关打开时，左侧面板顶部的 AI 人物容器（见 Design.md UI-11 §AI人物容器）内的 AI 人物动画与语音交互联动：

|| 状态 | AI 人物动画 |
|------|------|
| 待机/倾听中 | 耳朵微微闪烁（呼吸灯效果），眼睛眨眼动画 |
| 用户说话中 | 耳朵加速闪烁，眉毛微微上扬（专注表情） |
| AI 正在识别/STT | 眼睛显示省略号动画 (...) |
| AI 正在生成回复 | 嘴型动画（模拟说话），身体轻微上下弹跳 |
| TTS 播放中 | 嘴型持续动画，耳朵保持高频闪烁，情绪高亮 |
| TTS 播放结束 | 渐变为待机动画，耳朵变为缓慢闪烁 |
| 打断/静音 | 耳朵闪烁频率降低，眉毛微微下垂（疑惑表情） |
| 界面最小化/托盘隐藏 | UI 可不可见，但语音状态机仍在后台运行；恢复窗口后应能看到完整文本链路与最新状态 |

> 📋 详细技术架构和实现方案见：`D:\workspace\AI\doc\MiniMax27-Voice语音助手-完整方案.md`
>
> 🔎 交互参考实现：`D:\workspace\AnyClaw\AI Voice\openclaw_voice`（仅作流程参考，AnyClaw 正式实现以 C/C++ 为准）
...
| 控制行 | Agent 行为控制 | `[Agent ▼]  [AI行为 ▼][AI托管 ▼][Voice开关]  [字数]` |
...
**AI托管 下拉：** 切换控制权模式，用户控制（默认）/ AI 托管。详见 §6.3。

**Voice 开关：** 切换语音交互模式的开/关。打开后启用“文本优先”的语音链路（用户语音 STT 文本先入输出区，AI 回复文本先入输出区再 TTS），并联动 AI 人物动画响应（详见 §4.7）。

**字数统计：** 显示当前输入框字符数，达到上限时高亮提示。
...
**首次启动向导更新：**
- Step 1（环境检测）检测 8 项：Network / Node.js / npm / OpenClaw / Hermes / Claude / Config Dir / Disk Space；检测完成后若缺失则提供安装选项（从网络下载 / 本地离线包 / 稍后手动处理）；安装后刷新左侧检测结果
- 向导完成后默认启用 Leader 模式
