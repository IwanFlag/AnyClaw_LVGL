...
v2.0 默认开启 Agent 现有能力范围内的主动行为，邮件/日历等依赖应用授权的预设在接入后自动激活：

| 预设 | 触发 | 默认状态 | 前置条件 |
|------|------|---------|---------|
| 启动体检 | AnyClaw 启动时 | ✅ 默认开启 | 本地检测 |
| 空闲整理 | 用户 30 分钟无操作 | ✅ 默认开启 | 文件读写权限 |
| 记忆维护 | 每天 18:00 | ✅ 默认开启 | MEMORY.md 存在 |
| 日程提醒 | 每天 09:00 | ⏳ 应用授权后自动激活 | 日历应用已授权 |
| 邮件摘要 | 每天 12:00 | ⏳ 应用授权后自动激活 | 邮件应用已授权 |
| 日终总结 | 每天 23:00 | ✅ 默认开启 | 无 |

用户可在 Settings 中增删改预设。

#### 触发方式

| 触发 | 场景 |
|------|------|
| **定时** | Cron 任务（日历、邮件、记忆维护） |
| **事件** | 文件变化、应用授权变更、配置不一致 |
| **空闲** | 用户 30 分钟没操作 → Agent 趁空闲整理 |
| **启动** | AnyClaw 启动时自动体检 |
...
| 组件 | 能力 | 说明 |
|------|------|------|
| 文件树 | 浏览工作区目录结构 | 左侧树状导航 |
| 代码查看器 | 语法高亮 + 行号 | 只读，不做编辑 |
| 终端 | 嵌入式命令行 | Agent 和用户都可执行命令 |
| Chat 面板 | 对话 + 工具输出 | Agent 读代码、提建议、跑命令 |

- **Agent 编程方式：** 用户说"帮我修这个 bug"→ Agent 读文件 → 给出修改方案 → 用户手动修改或 Agent 通过终端执行
