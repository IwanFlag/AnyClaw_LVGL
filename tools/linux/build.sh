...
#  用法: bash tools/linux/build.sh [sdl2_prefix] [version]
#  示例: bash tools/linux/build.sh /opt/sdl2-mingw v2.0.0
# ═══════════════════════════════════════════════════════════════
set -e
...
TOOLCHAIN="${PROJECT_DIR}/tools/linux/toolchain-mingw64.cmake"
...
# 如果 SDL2 交叉编译包存在，使用它
if [ -d "${SDL2_PREFIX}/lib/cmake/SDL2" ]; then
    CMAKE_ARGS="${CMAKE_ARGS} -DSDL2_CROSS_PREFIX=${SDL2_PREFIX}"
    echo "  Using cross-compiled SDL2 at ${SDL2_PREFIX}"
else
    echo "  Using bundled SDL2 (thirdparty/SDL2/)"
fi
...
echo ""
echo "  Test:  bash tools/linux/run-wine.sh"
echo "════════════════════════════════════════════════════════"
