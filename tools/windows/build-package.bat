...
if "%SHOW_HELP%"=="1" (
  ...
  echo.
  echo Usage:
  echo   tools\windows\build-package.bat [--quiet^|--verbose] [--help]
  echo.
  echo Options:
...
if not exist "%EXE_PATH%" (
  if exist "%PROJECT_DIR%\build-windows\bin\AnyClaw_LVGL.exe" (
    set "EXE_PATH=%PROJECT_DIR%\build-windows\bin\AnyClaw_LVGL.exe"
    set "SDL_PATH=%PROJECT_DIR%\build-windows\bin\SDL2.dll"
  )
)
if not exist "%SDL_PATH%" (
  if exist "%PROJECT_DIR%\build-windows\bin\SDL2.dll" (
    set "SDL_PATH=%PROJECT_DIR%\build-windows\bin\SDL2.dll"
  )
)
