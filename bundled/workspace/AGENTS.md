...
├── (tasks/ 已整合至 workspace memory/) ← 开发任务跟踪
├── src/                     ← 应用源码 (C/C++)
├── ui/                      ← UI 定义
...
**检查结果记录**：在 `memory/anyclaw/v2.2.1-dev.md` 中标记状态。
...
开发/修复一个功能
  → 编译通过
  → 复测（Wine 跑应用验证该功能）
  → 更新 memory/anyclaw/v2.2.1-dev.md 状态（⏳→🔧→✅）
  → 更新 memory/anyclaw/v2.2.1-scan.md（如有 Bug）
...
2. 更新 `MEMORY.md` 进度
3. 写会话记录 `memory/YYYY-MM-DD.md`
4. `git add -A && git commit && git push`

**每次完成一个功能或 Bug 后**：
- 更新 `memory/anyclaw/v2.2.1-dev.md` / `memory/anyclaw/v2.2.1-scan.md` 状态
- 更新 `MEMORY.md` 进度
- `git add -A && git commit && git push`
- 然后才开始下一个
...
> **每次新会话启动时，先读本节了解进度，再读 `memory/anyclaw/v2.2.1-dev.md` 获取完整任务清单。**

### 会话启动顺序

1. `MEMORY.md`（workspace 根目录）— 长期记忆、项目现状、进度、阻塞项
2. `memory/anyclaw/v2.2.1-dev.md` — 任务清单，找到第一个 ⏳ 状态的任务
3. 本文件 — 开发规范、编译命令、提交格式

### 工作循环

1. 在 `memory/anyclaw/v2.2.1-dev.md` 中找到第一个 ⏳ 的任务
2. 改代码 → 编译验证 → 改文档 → 验证一致性 → commit + push
3. 每完成一个任务，更新 `memory/anyclaw/v2.2.1-dev.md` 状态 ⏳ → ✅
4. 每次会话结束，写一份会话记录到 `memory/YYYY-MM-DD.md`
...
| 阶段三 | Bug 检查与修复 | ✅ 6 个 bug 已修（详见 `memory/anyclaw/v2.2.1-scan.md`） |
...
2. **每个任务完成标准**：代码改完 → 文档同步（Design.md / memory/anyclaw/）→ 编译验证 → commit + push
...
| `docs/PRD.md` | 产品需求 |
| `memory/anyclaw/v2.2.1-dev.md` | **完整任务清单（必读）** |
| `memory/anyclaw/v2.2.1-scan.md` | Bug 扫描记录 |
